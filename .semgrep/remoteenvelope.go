...
// Bypasses the first version of this rule missed entirely: it enumerated the
// assignment shapes, so a call that is never assigned walked straight through.
func readRemoteReplyViaReturn(msg reply) (*errcode.Error, bool) {
	// ruleid: remote-envelope-must-use-fromreply
	return errcode.Parse(msg.Data)
}

func readRemoteReplyViaForward(msg reply) error {
	// ruleid: remote-envelope-must-use-fromreply
	return relay(errcode.Parse(msg.Data))
}

func relay(e *errcode.Error, ok bool) error {
	if ok {
		return e
	}
	return nil
...
// A dot-import makes the call read as a bare Parse(...). semgrep resolves a
// named alias back to its package but not a dot-import, so this shape has its
// own pattern — and its own fixture, because a pattern edit could drop it.
