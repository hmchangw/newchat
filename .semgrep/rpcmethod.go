...
// compiler already refuses that. But an untyped string constant is
// assignable to both string and RPCMethod, so a plain literal in the method
// position compiles silently and degrades to rpc_method="_OTHER" at runtime.
func registerCallSites(r *natsrouter.Router) {
	// ruleid: rpcmethod-must-be-a-vocabulary-constant
