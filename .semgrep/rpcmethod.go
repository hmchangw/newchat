...
func registerCallSites(r *natsrouter.Router) {
	...
	// Client lane: RecordRPCClientCall records rpc.client.call.duration and
	// takes the same RPCMethod, so its method argument needs the same
	// constraint at every instrumented outbound call site.
	// ruleid: rpcmethod-server-route-must-name-a-vocabulary-constant
	c.metrics.RecordRPCClientCall(ctx, "get_message", time.Since(started), err)

	// ruleid: rpcmethod-server-route-must-name-a-vocabulary-constant
	c.metrics.RecordRPCClientCall(ctx, natsmetrics.RPCMethod("typo"), time.Since(started), err)

	c.metrics.RecordRPCClientCall(ctx, natsmetrics.MethodGetMessage, time.Since(started), err)

	// A publisher under a different field name, timed without an inline
	// time.Since, given a bare literal. Every heuristic the old name forced
	// missed this shape; the unambiguous name catches it with no heuristic.
	// ruleid: rpcmethod-server-route-must-name-a-vocabulary-constant
	c.telemetry.RecordRPCClientCall(ctx, "get_message", elapsed, err)

	// ruleid: rpcmethod-server-route-must-name-a-vocabulary-constant
	c.telemetry.RecordRPCClientCall(ctx, method, elapsed, err)

	// The same shape with a direct selector stays clean.
	c.telemetry.RecordRPCClientCall(ctx, natsmetrics.MethodGetMessage, elapsed, err)
