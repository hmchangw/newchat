...
export function NatsProvider({ children }) {
  ...
  const watchClosed = useCallback((nc, myGen, onClosed) => {
    nc.closed().then((err) => {
      onClosed?.()
   ...
   */
  const startHomeProbe = useCallback((portal, myGen, inboxPrefix) => {
    ...
    const attempt = async () => {
      ...
      let home
      try {
        home = await natsConnect({ servers: portal.natsUrl, authenticator, inboxPrefix })
      } catch {
      ...
      ncRef.current = home
      setFailoverMode(false)
      // The reverted link needs its own permission watch — the peer's ends with
      // the connection it was bound to.
      const homeStatus = watchInboxPermission({
        nc: home, inboxPrefix, gen: myGen, genRef: connectGenRef, setError, setConnected,
      })
      watchClosed(home, myGen, () => homeStatus?.stop?.())
  ...
  const connectToNats = useCallback(async (opts) => {
    ...
    try {
      ...
      let homeErr = null
      try {
        nc = await natsConnect({ servers: portal.natsUrl, authenticator, inboxPrefix })
        ...
        for (const peer of shufflePeers(await fetchPeerSites(), portal.siteId)) {
          try {
            nc = await natsConnect({ servers: peer.natsUrl, authenticator, inboxPrefix })
      ...
      watchClosed(nc, myGen, () => statusIter?.stop?.())

      // On a peer: keep asking whether home is back, and swap over when it is.
      if (onFailover) startHomeProbe(portal, myGen, inboxPrefix)
    } catch (err) {
      stop()
      throw err
    }
