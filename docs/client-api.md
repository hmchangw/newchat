...
| `appName` | string | Optional. Bot/app display name from `apps.name`, set when the member is a bot — an account ending `.bot`, or a subscription flagged `isBot`. Mutually exclusive with `engName`/`chineseName`. |
