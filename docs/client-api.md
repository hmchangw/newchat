...
| `tcount` | number | Optional. Number of non-deleted replies on a thread parent. Exact when the server can recount the thread within its scan limit; when it cannot — the thread is longer than the limit, or its deleted replies alone fill it — the count is adjusted per reply without recounting, so it is approximate, off by at most the replies that raced or retried since the last re-anchor, and periodically re-derived from an exact count. |
...
| `message could not be accepted, please retry` | `unavailable` | — | An infrastructure failure (store or canonical-publish error) that was still failing on the **last** JetStream delivery. Sent once, at the end of the retry budget, because the message is dropped after it — see the delivery guarantee below. Earlier deliveries are retried silently. Retry with the same `id`: stored history is keyed on it, so a retry can never leave two copies in history. It is **not** a duplicate-*delivery* guarantee — `MESSAGES-CANONICAL` sets no `Duplicates`, so the server's 2-minute default applies and has long closed by the time this reply reaches you. In the uncommon case where the original publish did land and only its acknowledgement was lost, the retry is fanned out and notified a second time. |

**Delivery guarantee:** every validation/authorization failure — including a `siteID` mismatch and a malformed `msg.send` subject — is replied to the client on the response subject and the JetStream message is acked (not retried). The error reply requires a routable response subject, so it can only be sent when the `{account}` segment is recoverable from the subject and the payload carries a valid hyphenated-UUID `requestId`; if neither is recoverable (a truly malformed subject or missing/invalid `requestId`) no reply is possible and the client falls back to a request timeout. **Infrastructure failures** (store/publish errors) are nak'd and **redelivered by JetStream**, producing no reply while deliveries remain — with one exception at the end: on the **final** delivery, when the message is about to be dropped for good, the sender receives a single `unavailable` reply rather than nothing at all. A client that gets no reply is therefore still waiting on a send that may yet succeed; a client that gets `unavailable` knows the send is over. One further exception: a failed **room-metadata** read is not nak'd. It only feeds the large-room cap, which is spam control rather than access control, so it fails open and the send proceeds. The subscription check that authorizes the send still fails closed.
...
| `newTcount` | number | Reply count for the parent message. Replaces any locally-computed count — do not delta. Exact when the server can recount the thread within its scan limit; when it cannot (the thread is longer than the limit, or its deleted replies alone fill it) the count is adjusted per reply without recounting, so it is approximate and periodically re-derived from an exact count (same contract as `tcount` on [Message](#message-schema)). |
| `newThreadLastMsgAt` | string (ISO 8601) | Optional. The thread's stamped last-reply time — normally the most recent surviving reply. Absent only when no reply survives. On `reply_deleted` past the scan limit the server does not re-derive it — resolving the new newest reply needs the recount that path skips — so it repeats the timestamp already stamped, which may still name the deleted reply until the next re-anchor. |
