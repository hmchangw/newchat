...
- **Room names carry no special meaning.** No endpoint filters, drops, or suppresses a subscription based on its room's name — the `Del-` prefix that legacy soft-deletes used is not interpreted anywhere. The deployment guarantees no soft-delete rename reaches the database, so a room named `Del-anything` is returned like any other room. Enrichment is purely additive: every subscription matched by a query is returned, and a room the server cannot resolve (not found, or a degraded cross-site RPC) simply arrives with **no `room` object** rather than removing the row. One consequence: a `subscription.list` page is no longer shortened after pagination, so a full page is always exactly `limit` rows.
...
**Active set:** an active subscription is a non-muted DM or channel, **or** a botDM that is both non-muted **and** subscribed (`isSubscribed: true`). Unsubscribed botDMs and muted rooms of any type are excluded from the count. The count reads subscription state only — it consults no room document, and no room name is filtered.
