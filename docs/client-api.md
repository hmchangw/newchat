...
- **Cross-site federation:** if the user's home site differs from the handler's site, the handler emits an `OutboxEvent` on the OUTBOX stream and `outbox-worker` forwards the cross-site `thread_read` event (at-least-once) to `chat.inbox.{userSite}.external.thread_read` with payload `{account, roomId, threadRoomId, lastSeenAt, newThreadUnread, timestamp}` (`newThreadUnread` is the post-`$pull` array, omitted when empty; timestamps as `int64` UnixMilli). The destination `inbox-worker` applies `lastSeenAt`+`updatedAt`+`hasMention=false` to the local ThreadSubscription with an `$lt` order-safety guard, and — only when that guard matches — mirrors `newThreadUnread` onto the local Subscription's `threadUnread` ($set when non-empty, $unset when empty) so out-of-order delivery cannot regress the thread's read position or badge state.
