...
| `message could not be accepted, please retry` | `unavailable` | — | An infrastructure failure (store or canonical-publish error) that was still failing on the **last** JetStream delivery. Sent once, at the end of the retry budget, because the message is dropped after it — see the delivery guarantee below. Earlier deliveries are retried silently. The send did not happen; the client may safely retry with the same `id` (the canonical publish is deduplicated on it). |
