...
- **Room names carry no special meaning.** No endpoint filters, drops, or suppresses a subscription based on its room's name — the `Del-` prefix that legacy soft-deletes used is not interpreted anywhere, so a room named `Del-anything` is returned like any other. Enrichment only adds: every subscription a query matches is returned, so a full `subscription.list` page is always exactly `limit` rows.
