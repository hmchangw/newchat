...
- Soft-deleted rooms (a `Del-` name prefix) are treated **identically whether the room is local or cross-site**, but differ by endpoint shape:
  ...
  - **`subscription.count`**: the subscription is **dropped**, local and cross-site alike, in the Mongo query — the filter reads the `Del-` prefix from the subscription's own denormalized room name, which a rename fans out to every subscriber (including across sites). No `GetRoomsInfo` lookup and no pagination are involved, so unlike the list paths the count needs no post-filtering pass.
