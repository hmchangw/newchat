...
- Soft-deleted rooms (a `Del-` name prefix) are treated **identically whether the room is local or cross-site**, but differ by endpoint shape:
  - **List paths** (`subscription.list`, `subscription.getChannels`): the subscription is **dropped**. Local rooms are filtered in the Mongo query; cross-site rooms are dropped after the per-site `GetRoomsInfo` lookup reveals the `Del-` name — this happens post-pagination, so a page can be shorter than `limit` (`hasMore` is computed from the database page, before the cross-site drop).
  - **`subscription.count`**: **not filtered.** The count applies no `Del-` filter at all — the deployment guarantees no soft-delete rename reaches the database, so the count reads only subscription-local state and issues no room lookup. If a `Del-` room ever did exist, it would be counted here while the list paths dropped it, and a client folding its badge from the list would disagree with the server.
