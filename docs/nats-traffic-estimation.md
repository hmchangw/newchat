...
The presence health-check row is a **client→server publish** (`RegisterVoid`, §2.3), listed
here rather than in R/R (§6.3) because it is fire-and-forget with no reply — it is ingress,
not a server→client subject.
...
`presence.state.*`. The health-check row is **client-published and server-terminated**: each
connection publishes hello/ping/activity/bye to its own presence subject, and
`user-presence-service` writes the liveness key to Valkey without re-broadcasting — its
`Ping` handler publishes a `PresenceState` **only** on the first-sight (offline→online)
...
|------------------------|-------------------------------|
| All core deliveries (message, metadata, member-change, presence) — plus **presence keepalive ingress**, which is per-connection | Message ingress (user sends from one client) |
