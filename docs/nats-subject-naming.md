...
| Subject Pattern | Publisher | Consumer | Purpose |
|-----------------|-----------|----------|---------|
| `chat.msg.canonical.{siteID}.created` | message-gatekeeper | broadcast-worker, notification-worker, unread-worker, search-sync-worker | New message persisted |
| `chat.msg.canonical.{siteID}.updated` | history-service | broadcast-worker, unread-worker, search-sync-worker | Message edited |
