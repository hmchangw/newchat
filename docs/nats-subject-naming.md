...
| Subject Pattern | Publisher | Consumer | Purpose |
|-----------------|-----------|----------|---------|
| `chat.msg.canonical.{siteID}.created` | message-gatekeeper | broadcast-worker, notification-worker, room-state-worker, search-sync-worker | New message persisted |
| `chat.msg.canonical.{siteID}.updated` | history-service | broadcast-worker, room-state-worker, search-sync-worker | Message edited |
