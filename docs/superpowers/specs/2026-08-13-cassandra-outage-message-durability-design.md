...
`jsretry.MinWindow`, and the implied delivery count via `jsretry.DeliveriesFor` — both
shared helpers, so the window this service measures against and the budget any other
consumer sizes against cannot disagree about the tail rule. With `DefaultBackoff` a 1h
window is first crossed on the **17th delivery** (logged at startup so the mapping is
visible, not implied).

That number is schedule-dependent and has moved twice already: the 34th delivery while
`DefaultBackoff` ended in a 2m tail, the 11th once it gained a 10m tail, and the 17th
now that the measure is the guaranteed window rather than the nominal one. Nothing in
the service changed for any of them, which is the point of expressing the threshold as a
duration — but it does mean the count is a derived fact to read off the startup log,
never a constant to hard-code.

`MinWindow` sums the **jittered minimum** of each entry, not the nominal value. Equal
jitter draws every wait from `[d/2, d]`, so nominal arithmetic overstates any window by
up to 2x — an earlier revision of this design measured nominally and a 1h setting bought
roughly 45m of real retrying. Measuring the floor makes the configured duration a
guarantee: `INVALID_RETRY_WINDOW=1h` means *at least* an hour of actual retrying before
...
things that can fail a message — a Mongo lookup or a downstream publish, most of all. (A
thread parent that has not landed no longer contributes much: #417 caps that wait at
`parentResolveAttempts` deliveries and then salvages the reply, so it can no longer spend
a message's whole budget.) So a message pre-aged by unrelated failures arrives at its *first*
...
- `jsretry.MinWindow` / `DeliveriesFor`: the guaranteed window and the delivery count
  sized against it, both walking the schedule through the same tail rule.
