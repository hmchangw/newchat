...
- `message-worker/integration_test.go`: `EnsureIndexes` co-creates both unique
  keys with `room-service`'s spec through the non-destructive
  `mongoutil.EnsureIndex` (asserted on an index-less database), the gates
  confirm them before each dependent write, and a conflicting spec is refused,
  never repaired.
