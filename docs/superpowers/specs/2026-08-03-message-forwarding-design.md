...
- **Client body override.** An optional `forwardedContent` on the send request replaces `ForwardedMessage.Msg`. Motivation is client-side redaction, not authoring: a source body may reference a restricted image that the destination room's members cannot access, so the client substitutes a safe body before the snapshot is persisted (the snapshot is immutable, so this cannot be corrected after the fact). The source is still fetched and every rule above still runs against it; only the body is substituted, and it is applied *after* the reject rules so an override can replace a real body but never manufacture one for a source that had none. Deliberately carries **no provenance marker**: the redacted body is the one every reader should see, so flagging it as client-supplied would work against the purpose. The cost is that a fabricated body is equally indistinguishable, under the source author's identity — accepted knowingly. Everything else on the snapshot stays server-derived.
