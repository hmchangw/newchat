...
> **Status: superseded — kept as a design record, not as work to do.** The page-cache decorator described here was replaced by the per-bucket design in [`2026-08-05-history-per-bucket-cache.md`](2026-08-05-history-per-bucket-cache.md), which is what shipped. Read this one for the rationale it still carries — the serialization choice, the fail-open posture, and the appendix that argues for per-bucket over per-page — not as a description of the current code.
