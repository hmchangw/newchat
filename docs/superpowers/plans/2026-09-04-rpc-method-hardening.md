...
- **The nine uninstrumented outbound RPC call sites** in `user-service`, `search-service` and `notification-worker`. Six other outbound sites *are* instrumented through `Publisher.Request`, which records `rpc.client.call.duration`; those are in scope for the semgrep rule and were added to it. What is deferred is adding the instrument to the nine that record nothing.
- **Verb synonym drift** (`remove_member` vs `delete_emoji`, `batch_get_room_previews` vs `list_members`). Worth a shrink of the verb set and a picking rule in the contract, but it is a vocabulary-wide edit, not part of making the guard mechanical.
