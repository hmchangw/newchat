...
| Subject builder | Handler | `rpc_method` |
|---|---|---|
| `MsgHistoryPattern` | `s.LoadHistory` | `list_channel_messages` **(SLO-4)** |
| `MsgThreadPattern` | `s.GetThreadMessages` | `list_thread_messages` **(SLO-5)** |
| `MsgNextPattern` | `s.LoadNextMessages` | `list_next_messages` |
| `MsgSurroundingPattern` | `s.LoadSurroundingMessages` | `list_surrounding_messages` |
...
back as exceptions to `<verb>_<object>`. They become `list_channel_messages` and
...
| File | What to change |
|---|---|
| `pkg/natsmetrics/rpcsemconv_test.go` | 15 uses of `OperationHistoryRead` / `OperationRoomRead` → `MethodListChannelMessages` / `MethodOpenRoom` |
...
`rpc_method="list_channel_messages"` in the SLO-4 and SLO-5 PromQL blocks. No
