...
- **Writes by other services** (applied by `message-worker` / `bot-message-worker`, not history-service): backstopped by the TTL only. This is broader than the cross-site/federated edits originally scoped here, and it is **not** an edge case: every thread reply runs `setParentTcountAndTlm`, and a first reply also runs `UpdateParentMessageThreadRoomID`, both targeting `sizer.Of(parentCreatedAt)` — the *parent's* bucket, which is sealed whenever the parent predates the current window. `tcount`, `thread_last_msg_at` and `thread_room_id` are all in `baseColumns`, so a cached read serves the pre-reply values. chat-frontend gates the thread affordance on `tcount > 0`, so a first reply on such a parent renders no "N replies" button at all.
  ...
  Consequently the claim "a sealed bucket changes only through the mutation paths (edit, delete, pin, react)" is **false**, and the invalidation set above is not complete. Recorded as an accepted gap by `TestPerBucketCache_ThreadParentMutatedByAnotherService_StaysStale`, and listed with the other three on `config.Config.BucketCacheOptIn`.

  A shared-tier DEL from the workers would not close it: `Cache.Get` returns on an L1 hit before consulting L2, so a replica already holding the bucket keeps serving the stale row. Closing it needs cross-replica L1 invalidation (#251) — pub/sub fan-out, or an L1 TTL short enough to bound the window — which subsumes the DEL. Until then the cache stays off by default (`HISTORY_BUCKET_CACHE_ENABLED=false`).
