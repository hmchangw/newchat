...
> **Amended after implementation:** the in-process L1 this plan specifies was
> removed before merge — it made `Bust` unable to reach sibling replicas (#251).
> The cache is now a single shared Valkey tier. See the **Tiers** bullet under
> Design decisions; the task list below still describes the original two-tier
> build and is kept as the historical record.
...
- **Tiers:** one — Valkey (`cachemetrics.For("history_bucket","valkey")`). Fail-open throughout. **Superseded:** this originally specified an in-process L1 over Valkey. The L1 made `Bust` unable to reach sibling replicas (#251) — a correctness hole traded for a network hop, and the hop is the cheap half, because an L1 caches the *encoded* blob so every `Get` pays the gob decode either way (`BenchmarkDecode`: ~113µs for 10 rows, ~169µs for 50, i.e. ~100µs of fixed gob type-descriptor parsing per call regardless of tier). Dropping it makes `DEL` authoritative fleet-wide, matching `pkg/badgecache` and `pkg/roomsubcache`; `pkg/roommetacache` keeps an L1 and documents the same staleness it cannot close.
...
- **Invalidation:** **no generation counter** — one key per bucket, so a plain `DEL`. New messages need no bust (current bucket never cached). Edit/delete/react/pin bust exactly `sizer.Of(msg.CreatedAt)`; a delete that recomputes the thread parent's tcount also busts `sizer.Of(*msg.ThreadParentCreatedAt)`. With a single shared tier the `DEL` is authoritative for every replica immediately — there is no per-process copy to reconcile.
- **Writes by other services** (applied by `message-worker` / `bot-message-worker`, not history-service): backstopped by the TTL only. This is broader than the cross-site/federated edits originally scoped here, and it is **not** an edge case: every thread reply runs `setParentTcountAndTlm`, and a first reply also runs `UpdateParentMessageThreadRoomID`, both targeting `sizer.Of(parentCreatedAt)` — the *parent's* bucket, which is sealed whenever the parent predates the current window. `tcount`, `thread_last_msg_at` and `thread_room_id` are all in `baseColumns`, so a cached read serves the pre-reply values. chat-frontend gates the thread affordance on `tcount > 0`, so a first reply on such a parent renders no "N replies" button at all.

  Consequently the claim "a sealed bucket changes only through the mutation paths (edit, delete, pin, react)" is **false**, and the invalidation set above is not complete. Recorded as an accepted gap by `TestPerBucketCache_ThreadParentMutatedByAnotherService_StaysStale`, and listed with the other two on `config.Config.BucketCacheOptIn`.

  With the single shared tier a `DEL` from those writers now *would* close it; the blocker is that neither worker has a Valkey client or the key builder (which lives under `history-service/internal/`, so it would move to `pkg/`). Until that lands the cache stays off by default (`HISTORY_BUCKET_CACHE_ENABLED=false`).

**Tech Stack:** Go 1.25, `pkg/valkeyutil`, `pkg/cachemetrics`, `pkg/msgbucket`, `encoding/gob`, `gocql`, `pkg/testutil` (Cassandra + Valkey), testify.
