...
5. Moves the presence classification into `pkg/model` — see the spec's [What the presence side still owes this](../specs/2026-08-10-priority-contact-presence-gating-design.md#what-the-presence-side-still-owes-this) for why it is deferred until then.
6. Sizes the affected population in production **first**, and follows the consumer-first sequencing — both specified in the spec's [Rollout](../specs/2026-08-10-priority-contact-presence-gating-design.md#rollout) section. Do not restate the sizing query or the sequencing rules here; that section is the single source and the two drifted apart once already.
