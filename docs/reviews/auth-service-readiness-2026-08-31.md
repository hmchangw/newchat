...
- `high` — Unknown-`kid` JWKS refetch amplification. `Validate` calls `verifier.Verify` (`pkg/oidc/oidc.go:133`); go-oidc's `RemoteKeySet` has **no minimum refresh interval** — a cache miss goes straight to `keysFromRemote` and re-fetches the IdP's JWKS (`go-oidc@v3.17.0/oidc/jwks.go:196-235`). `inflight` coalesces only the fetches that overlap, and it is nil'd the moment one completes (`:216-222`), so the next unknown `kid` starts a fresh fetch immediately. **The ceiling is therefore one outstanding fetch, not one per request** — a continuous back-to-back loop at whatever rate the IdP answers, each attempt also holding the caller's request open for up to the 10 s timeout, at zero cost to the attacker. With finding 1 there is nothing throttling the submission rate.
...
- `high` — Bound JWKS refetch with a **minimum refresh interval plus a short negative cache of recently-rejected `kid`s**: at most one refetch per window, and an unknown `kid` seen inside that window fails without touching the IdP. A token bucket keyed on verification failures in front of `Validate` is a reasonable second layer. **Do not** pre-parse the header and hard-reject a `kid` missing from the cached key set — an earlier draft of this report recommended exactly that, and it breaks legitimate rotation: a newly rotated IdP key *is* an unknown `kid`, and refetching on that miss is the behaviour the OIDC spec prescribes (`jwks.go:167-171`). Rejecting it turns every key rotation into an auth outage. The interval bounds the abuse; the refetch on miss has to survive it, at a worst-case rotation delay of one window.
...
| 5 | `high` | **Bound unknown-`kid` JWKS refetches — with an interval, never a hard reject** | Performance | `Validate` → `verifier.Verify` at `pkg/oidc/oidc.go:133`; go-oidc's `RemoteKeySet` has **no minimum refresh interval** — a cache miss goes straight to `keysFromRemote` (`go-oidc@v3.17.0/oidc/jwks.go:196-235`), and `inflight` coalesces only overlapping fetches, freed as soon as one returns | A caller submitting syntactically valid JWTs with **random `kid` values drives a continuous back-to-back 10 s-timeout fetch loop against the IdP, at zero cost to the attacker** (bounded to one outstanding fetch, not one per request). With item 4 unfixed, each such request also holds a slot. Add a minimum refresh interval plus a negative-`kid` cache — refetching on an unknown `kid` must keep working, or IdP key rotation becomes an auth outage. |
