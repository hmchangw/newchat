...
|---|-----|--------|-----|----------|-----|
| 1 | `high` | **Install the handler timeout — switch to `natsrouter.DefaultGuarded`** | Architecture / Perf | cap applied alone at `main.go:112-116`; `pkg/natsrouter/guard.go:57-66` | The helper exists **precisely** so "a service can't apply only half of the overload protection — the cap and the timeout are otherwise two separate calls that are easy to forget one of." This service forgot one. Handler contexts have no deadline, so a request runs until resty's 30s client timeout plus up to 5s of token exchange. **A NATS caller gives up in ~2s**, so with a degraded upstream all 100 slots stay occupied ~35s doing work no one reads, and every other caller gets "service busy". The two knobs are unrelated numbers that must agree. |
| 2 | `high` | **Mount `natsrouter.GuardConfig` instead of re-declaring `MAX_CONCURRENCY`** | Architecture | `main.go:41` (`envDefault:"100"`) vs the owning package `pkg/natsrouter/guard.go:21` (`envDefault:"256"`) | CLAUDE.md §6's shared-knob rule, and **the divergence is already real**: six services (`room-service`, `media-service`, `history-service`, `search-service`, `room-worker`, …) mount the config and get 256; this one silently gets 100. Fixing it is the same edit as item 1. |
| 3 | `medium` | **Size the idle connection pools** | Performance | `translator_stream.go:51` and `token.go:64` call `restyutil.New` without `WithMaxIdleConns`; rationale at `pkg/restyutil` | Both clients are single-host at `MAX_CONCURRENCY=100`, and the stdlib keeps **2 idle connections**, so ~98 of 100 concurrent translate calls pay a fresh TCP+TLS handshake **to a third-party endpoint**. `media-service/main.go:112` and `upload-service/main.go:191` already do this. |
| 4 | `medium` | **Test the 5s token-exchange clamp** | Test coverage | `token.go:60-62`, uncovered | Every test passes `5*time.Second`, so **the branch that actually runs in production** (`TRANSLATION_HTTP_TIMEOUT` default 30s → clamped to 5s) is untested. The whole point of that clamp is bounding how long a hung accessToken endpoint holds `p.mu` and every waiting translate — the exact interaction item 1 makes worse. |
| 5 | `medium` | **Assert the documented error taxonomy through the handler** | Test coverage / Integration | wrap at `handler.go:39`; `handler_test.go:72-86` feeds only a plain `errors.New`; contract at `docs/client-api.md:6317-6318` | `unavailable`/`upstream_unavailable` and `too_many_requests`/`rate_limited` are **promised to clients**, and nothing tests that a typed upstream error survives the `fmt.Errorf` wrap on the path clients actually take — only inside `translator_stream_test.go`. |
| 6 | `medium` | **Cover token-provider transport failure** | Test coverage | `token.go:113-115`, uncovered | An unreachable accessToken host is **the most likely production failure** and the only one with no test; non-200 and malformed-body cases are covered. |
| 7 | `medium` | **Split `translateOnce`** | Maintainability | `translator_stream.go:87-174` (88 lines) | Four jobs — request construction, HTTP status classification, SSE framing and merge, post-loop error taxonomy — with a labeled loop, a nested `switch`, and a deferred `readErr` ordering subtlety that needs its own comment at `:150-152`. Adding one upstream behaviour (a `[HEARTBEAT]` sentinel, a size cap) means editing the middle of this loop. |
| 8 | `low` | **Tidy the SAST suppression** | Code quality | flagged by D1 as the one sloppy suppression | CLAUDE.md requires `// #nosec <RULE> -- reason` directly above the statement, and notes that golangci-lint's `//nolint:gosec` does **not** suppress standalone gosec. Worth getting exactly right in the one service that otherwise passes every gate. |
| 9 | `low` | **Fix the `pkg/model` tag deviation** | Integration | flagged by D5 | CLAUDE.md §3: all model structs get both `json` and `bson` tags. |
| 10 | `low` | **Document the undocumented operational knob** | Integration | flagged by D5 | Small, but this is the service whose client-API documentation is otherwise the cleanest in the repo — worth keeping it that way. |

**Context for the fleet report.** This service is the control case. It is not the largest or the most complex, but it is the only one where the coverage gate passes, and the reason is visible in its shape: no `main()`-heavy wiring, dependencies injected as interfaces, and a stateless request/reply surface. That is exactly the structural difference the fleet-wide coverage finding turns on — every other service's shortfall is concentrated in an untestable `main()`, not in untested logic. Its two `high` findings are also instructive in the other direction: both are shared-knob and shared-helper discipline, the same systemic pattern that shows up across twelve services.
