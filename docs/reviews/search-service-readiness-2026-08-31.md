...
| medium | **No CCS *degraded* test**, and the degradation is not decodable as written: `rawResponse` reads `_shards.total` and nothing else — not `_shards.failed`, and not the `_clusters` block that carries the authoritative cross-cluster verdict — so **a partial CCS result is returned as complete with an undercounted `Total`**. Note `_shards.skipped` is *not* that signal: it counts can-match pruning, which is routine for range-filtered reads over monthly `messages-*` indices | `integration_ccs_test.go:282`, `:373`; `response.go:21-23` |
...
- `medium` — **Decode the `_clusters` block (`skipped`/`partial`/`failed`) plus `_shards.failed` in `rawResponse`, and surface it** — *not* `_shards.skipped`, which counts can-match pruning and would flag ordinary range-filtered searches as degraded. The scenario below is exactly why `_clusters` is the one that matters: a remote cluster that is unreachable under `skip_unavailable: true` contributes no shards at all, so `_shards.failed` stays 0 and only `_clusters.skipped` records it (under `skip_unavailable: false` the search errors outright instead of returning partial). Then add a CCS test that stops the remote container mid-suite and asserts the degraded result is flagged rather than reported complete.
...
| 8 | `medium` | Decode `_clusters.skipped`/`partial`/`failed` (plus `_shards.failed`) and surface CCS degradation | Test coverage / Integration | `response.go:21-23` | **A partial cross-cluster result is currently returned as complete, with an undercounted total.** The user cannot tell a degraded search from a real one. `_shards.skipped` is not the signal — it counts can-match pruning, and an unreachable remote contributes no shards to count. |
