...
Deduplicated across dimensions, ordered by severity, then impact ÷ effort.

1. **critical · federation** — Stop silent cross-site event loss: set inbox-worker's INBOX consumer to `MaxDeliver=-1` (or large bound + DLQ with alerting) — `inbox-worker/main.go:983-988`, `pkg/stream/consumer.go:18`. *Why:* any destination-side outage or dependency wait >~13 min permanently diverges membership/subscription state between sites, and the reconciliation backstop doesn't exist. Small config-level change, largest data-integrity payoff.
2. **high · Cassandra + hot path** — Kill the O(N²) thread-reply count: replace the full-partition scan in `pkg/threadcount/count.go:33-66` (called from `message-worker/store_cassandra.go:374-387`) with a counter/incremental mechanism; at minimum add a row-count circuit breaker. *Why:* popular threads degrade quadratically and end in timeout → NAK redelivery storms aimed at the hottest replicas.
3. **high · config** — Add startup guards for the three silent-mismatch knobs: Cassandra marker-row check for `MESSAGE_BUCKET_HOURS`; single-source or publish-time validation for `ALL_SITE_IDS` (reject/alert on publish to an unconfigured destination); Mongo-doc cross-check for `ROOM_KEY_RETIRED_TTL` writers. *Why:* each mismatch loses data or strands federation with no error anywhere.
4. **high · stability** — Wrap message-gatekeeper's processor in `jobguard.Run` (`message-gatekeeper/main.go:195-202`). *Why:* it is the only worker where a poison message on the client-facing ingest path crash-loops the pod; every sibling worker already has the guard. Trivial effort.
5. **high · edge** — Put admission control (`ginutil.MaxConcurrency`) + rate limiting on auth-service `/api/v1/auth` (`auth-service/routes.go:12`). *Why:* reconnect storms hit the single service every client needs, amplified by a 10s IdP timeout.
6. **high · hot path / Mongo** — Tame per-message fan-out amplification: debounce/batch the `badge.count.batch` RPC (`notification-worker/handler.go:259,320-361`); coalesce `AdvanceSubscriptionLastSeen` and take the `@all` UpdateMany off the ack-critical path (`broadcast-worker/handler.go:261,272`, `store_mongo.go:197-205`). *Why:* cost scales room-size × message-rate on the two hottest write paths (user-service and `subscriptions`).
7. **high · Cassandra + Mongo** — Bound thread growth on both stores: bucket or cap `thread_messages_by_thread`; cap the `threadUnread` array (`message-worker/store_mongo.go:183-194`); add max-partition-bytes and array-size p99 alerts now. *Why:* both grow without limit and sit on hot paths (the gatekeeper per-send read decodes `threadUnread` today — also add the missing projection, `message-gatekeeper/store_mongo.go:36-46`).
8. **high · federation** — Codify OUTBOX/INBOX retention (assert `MaxAge`/replicas in the bootstrap verify path or a checked-in IaC manifest) and alert on OUTBOX oldest-message age (`outbox-worker/bootstrap.go:43-46`). *Why:* today retention is an unverified precondition where both directions (too short / absent) lose data or disk.
9. **medium · federation + stability** — Enforce inbox-worker single-replica at deploy level (or add the watermark guard to membership applies) before anyone scales it out (`inbox-worker/main.go:857-903`); classify never-succeeding forwards as Permanent-with-page in outbox-worker (`outbox-worker/handler.go:59`) and alert on ordered-lane oldest-ack-pending age.
10. **medium · stability + observability** — Set a client-wide Mongo timeout in `pkg/mongoutil`; add consumer-lag/stream-depth gauges (or bless the NATS exporter as a hard prod dependency); fix the notification-worker shutdown close-channel race (`notification-worker/main.go:378-399`); add hot-thread and large-room-burst scenarios to the load-test workload model so the top two findings are exercised before production finds them.
