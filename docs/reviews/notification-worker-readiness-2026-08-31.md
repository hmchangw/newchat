...
| 8 | `medium` | **Make `Snapshot`'s chunks concurrent, or give each chunk its own budget** | Performance / Integration | `usersettings.go:96`, loop `:105` | This is the gate that enforces mute, and its chunks share one 2 s deadline. On a large `@all` fan-out a slow secondary means **the tail chunks never run and those recipients fail open — muted users get pushed.** The comment at `:100-103` acknowledges it. |
