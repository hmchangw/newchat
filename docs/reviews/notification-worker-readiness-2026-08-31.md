...
### Findings
- `critical` — Service coverage is **59.0% (373/632 stmts)**, below the 60% line this audit scores as `critical` (CLAUDE.md §4 requires 80%) and far below the 80% merge floor. Per-file: `handler.go` 96.7% (202/209), `main.go` **2.0% (4/196)**, `usersettings.go` 51.1% (23/45), `threads.go` **0% (0/15)**, `emit.go` 75%, `presence.go` 79.4% — `/tmp/.../scratchpad/pr/coverage_by_service.txt`
...
- `medium` — Make the settings chunk loop degrade uniformly instead of silently un-muting whichever recipients land in the last chunks — but **give each chunk its own budget rather than copying `bulkPresenceSource`'s fan-out**, which is what an earlier draft of this report recommended. Two reasons. `bulkPresenceSource` spawns a goroutine per chunk with **no semaphore** (`presence.go:69-72`), so the shape being copied is an unbounded fan-out. And the multiplication is service-wide, not per message: `MAX_WORKERS` is 100, so a large `@all` at 512 per chunk turns into (workers × chunks) concurrent Mongo queries against a `MaxPoolSize` of 150 per pod (`pkg/mongoutil/poolconfig.go:25`) — the excess then blocks on pool checkout, inside the same 2 s deadline it was meant to beat, and starves every other Mongo reader in the pod. A per-chunk timeout fixes the uneven degradation with no added concurrency at all; if parallelism is still wanted, bound it with a semaphore sized against the pool and `MAX_WORKERS` together. The code's own comment (`usersettings.go:100-103`) already argues the sequential shape is right for a single indexed `$in` — the defect is the shared deadline, not the sequencing.
...
|---|-----|--------|-----|----------|-----|
| 1 | `critical` | **Track the invalidation goroutine in a `WaitGroup` and join it before `close(invalCh)`** | Quality / Arch / Perf / Integration | goroutine `main.go:304-325`; `invalIter.Stop()` `main.go:416`; `close(invalCh)` `main.go:420` | Found independently by **four of six experts**. `Stop()` does not wait for a message already returned by `Next()`, so `invalCh <- evt.RoomID` can execute after the close and **panic the process during graceful shutdown**. A send on a closed channel panics even from a `select` with a `default`. Every other goroutine here has a tracked termination path. |
| 2 | `critical` | **Cover the migrated-event Ack-drop branch** | Test coverage | `main.go:366-373`; cf. `migration_test.go:14` | `natsutil.IsMigrationLiveHeader` → `Ack()` + `notifySuppressed` lives inside `main()` and never executes under test. **An inverted condition or a dropped `Ack()` would re-notify every migrated message and no test would fail** — a user-visible notification storm with no regression guard. |
| 3 | `high` | **Fix `bootstrapStreams` to use the declared `.>` subject for MESSAGES-CANONICAL** | Arch / Integration | `main.go:186`, `:195`; `bootstrap.go:27-36`; `pkg/stream/stream.go:22-27` | It creates the stream with the `.created` leaf only, while `pkg/stream.MessagesCanonical` and `message-gatekeeper` declare the wildcard. CLAUDE.md is explicit that a bootstrap helper sets **only** `Name + Subjects` from `pkg/stream` — narrowing a stream four services share is how a dev environment silently diverges from production. |
| 4 | `high` | **Give the federated user-settings replica a backfill path** | Integration | `inbox-worker/main.go:280-294`; `pkg/model/event.go:212-222`; consumed at `handler.go:270` | `inbox-worker` applies settings with a non-upsert `UpdateOne` ("a missing user is a silent no-op"), and `UserAccountUpdated` carries no `settings`. A `user_settings_updated` arriving before the account upsert — or a user replicated to a site added after their last settings write — leaves **no settings doc, so the mute gate fails open and the user is pushed**. |
| 5 | `high` | **Extract `main()`'s wiring into testable seams** | Test coverage / Maint | `main.go:81-438` (361 lines, 2.0% covered), kill switches at `:215-243` | `main.go` is 196 statements at 2% and holds the three suppression kill switches that decide whether the mute gate runs at all. **Nothing proves `USER_SETTINGS_ENABLED=false` yields pre-enforcement behaviour end to end** — `config_test.go:78` asserts only the parsed bool. This is also most of the path from 59.0% to the 80% floor. |
| 6 | `high` | **Split `HandleMessage` (252 lines, CC≈30) along its six stages** | Maintainability | `handler.go:109-354` | Decode, cache invalidation, thread-parent resolution and retry budget, member filtering, settings/presence gating, badge fan-out and batch emission in one body. It is the reason `usersettings.go`/`threads.go` coverage is reachable only through a full-message fixture. |
| 7 | `medium` | **Run the three dependency round trips concurrently** | Performance | settings `handler.go:270`, presence `:271`, badges `:286` | All three inputs are fully known when the member loop ends at `:268`, yet the calls run strictly serially: **~9 s of worst-case dependency wait per message where ~5 s would do.** `presence.go:72` already solves the same shape concurrently. |
| 8 | `medium` | **Give each `Snapshot` chunk its own budget** (concurrency only if bounded) | Performance / Integration | `usersettings.go:96`, loop `:105`; `presence.go:69-72`; `pkg/mongoutil/poolconfig.go:25` | This is the gate that enforces mute, and its chunks share one 2 s deadline. On a large `@all` fan-out a slow secondary means **the tail chunks never run and those recipients fail open — muted users get pushed.** The comment at `:100-103` acknowledges it. A per-chunk deadline fixes it without adding concurrency; copying `bulkPresenceSource` would import an unbounded goroutine-per-chunk fan-out, multiplied by `MAX_WORKERS` against a 150-connection pool. |
| 9 | `medium` | **Drop the log-and-return double-log; convert the per-message path to `*Context` slog** | Code quality | `handler.go:332` then `jsretry.Settle` `main.go:375`; ctx-less at `handler.go:247`, `:387`, `:497`, `presence.go:76-95`, `main.go:312`, `:320` | `SettleQuiet` exists exactly for already-logged paths. And the four presence warn lines carry **no correlation ID at all**, so a presence failure is the one event that cannot be joined to its message. |
| 10 | `medium` | **Wire `logctx.Configure`, or remove the `ConsumeContext` call** | Code quality | `main.go:363`; cf. `broadcast-worker/main.go:99`, `:125`, `:133` | Half the helper's contract is wired: the limiter stays `denyAll` and the `obs`-installed handler is not the logctx wrapper, so **the admitted `X-Debug` rung is dead on arrival** and sub-INFO records never emit. |

**Also worth doing, below the cut.** Stop the unit test that starts a real in-process NATS server (`parent_fetcher_test.go:25-38`) — every other collaborator here is stubbed through a narrow interface, and CLAUDE.md forbids real NATS in unit tests. Route the invalidation consumer's hand-rolled config through `stream.DurableConsumerDefaults` (`main.go:288-293`) so its `BackOff` cannot disagree with `AckWait`. Give the mention-name lookup the `userstore.Resilient` L2+breaker wiring its peers use (`main.go:243` vs `broadcast-worker/main.go:295`) — the service already holds both a Valkey client and a breaker config. And either honour `showPreviewsInNotifications` or remove it: it is validated, stored, watermarked and federated to every site, and **no service reads it** — `handler.go:297` puts the full message body into the push event unconditionally.
