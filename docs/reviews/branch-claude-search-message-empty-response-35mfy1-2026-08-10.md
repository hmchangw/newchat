...
**1. MEDIUM — `logEmptyResult` emits without `request_id` (request-ID propagation rule).** `search-service/handler.go:183-192`. Verified the plumbing: `c.WithLogValues("request_id", ...)` (handler.go:137, 206) stores an enriched `*slog.Logger` under `errcode`'s `loggerCtxKey` (`pkg/errcode/logctx.go:18-20`), which is consumed **only** by `errcode.Classify` via `loggerFrom`. `slog.WarnContext`/`slog.Log` use `slog.Default()`; the `logctx.Handler` wrapper only gates sub-INFO admission — it does not merge ctx log values. So neither the WARN nor the FLOW line carries `request_id` (trace_id via the o11y SDK handler only when `O11Y_ENABLED=true`, default false). Repo convention is explicit args: `message-gatekeeper/handler.go:150`, `natsrouter/middleware.go:85-94 requestAttrs`. Fix is one pair — the derived timeout ctx preserves values, so add `"request_id", natsutil.RequestIDFromContext(ctx)` inside `logEmptyResult`.

**2. MEDIUM — `search-sync-worker` "build action" error log cannot identify the failing message.** `search-sync-worker/handler.go:92` (`slog.Error("build action", "error", err)`; same at :86 "decode payload"). The new `parseMemberEvent` errors route here, then the message is **Acked** (poison-drop) — the log line is the only trace, and it carries no `msg.Subject()`, no collection name, and discards `ctx` (plain `slog.Error`, not `ErrorContext`, losing trace correlation the consumer span provides). An operator seeing "missing payload envelope" cannot tell which stream/subject/site published unwrapped — precisely the misconfiguration this branch fixed. Log site is pre-existing, but the branch adds the failure modes that make it operable-critical. Add `"subject", msg.Subject()` (+ collection) and use `slog.ErrorContext(ctx, ...)`.

**3. LOW — user search input logged on an always-on WARN path.** `search-service/handler.go:186`. `query` is user content; in the zero-shards branch the query has zero diagnostic value — the fault is index config. Suggest dropping `query` (keep `account` or not) from the WARN branch; the FLOW branch is fine (X-Debug-admitted, analogous to gated payload capture).

**4. LOW — log-only signal where a metric would be alertable.** The zero-shards condition is a standing misconfiguration; the WARN is continuous while broken and greppable, but alerting typically rides metrics — `metrics.go` already has the plumbing, and a tiny counter would give a standing alert without log pipelines. Note: do NOT overload the `status` label (`allowedStatusLabels`, metrics.go:149, pins cardinality; the request succeeds, "ok" is correct). Team decided against it; reasonable, flagging as follow-up.
