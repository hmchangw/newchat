...
1. **medium** — Log the inherited cap: add `history_mode` + `history_shared_since` to the room-service member.add flow log (room-service/handler.go:992) and room-worker's resolution debug log (room-worker/handler.go:907). *Why:* capped adds are currently undiagnosable from logs; the first support ticket needs JetStream archaeology. (Observability)
2. **medium** — Validate `history.mode` at the room-service boundary: `BadRequest` on anything other than `""`/`"none"`/`"all"`; update `docs/client-api.md` error cases in the same change. *Why:* unknown strings silently grant share-all — permissive failure of a visibility control. (Go expert + Bug & security)
3. **medium** — Collapse the duplicated share-all predicate into `model.HistoryConfig.SharesAll()` used by room-service/handler.go:906 and room-worker/handler.go:141. *Why:* two independently encoded copies of a security-relevant predicate can drift. (Go expert)
4. **medium** — Harden the forged-value test: add table cases `{mode none, forged client value → nil}` and `{forged value + capped requester → requester's cap}` to `room-service/handler_test.go:980`. *Why:* pins the unconditional server-set reset against refactor drift. (Test-automation + Bug & security)
5. **low** — Thread `ctx` into `historySharedSincePtr` and switch the interior `slog.Error` to `slog.ErrorContext` (room-worker/handler.go:1140,1148-1149). *Why:* request-ID correlation per CLAUDE.md §3; signature was already being changed. (Go + Observability)
6. **low** — Defense-in-depth on the malformed-event branch: mode `"none"` with `timestamp <= 0` should fall back to `inherited` (when non-nil) instead of nil/unrestricted (room-worker/handler.go:1146-1151). (Bug & security)
7. **low** — Fix the doc comment: `model.RoomMemberEvent` → `model.InboxMemberEvent` (room-worker/handler.go:139-140). (room-worker generalist + Go expert)
8. **low** — Note in the PR description: deploy order room-worker → room-service (old worker ignores the new field, leaving the escalation window open), and the documented `history.mode` default correction from `"none"` to `"all"`. (Bug & security + room-service generalist)
9. **low** — Hoist the loop-invariant `historySharedSincePtr` call out of the per-member loop in room-worker/handler.go:1166-1170. (Performance)
10. **nitpick** — Add a negative-value case to `TestHistorySharedSincePtr_InheritedCap`; consider trimming the committed plan/spec working docs from the PR. (Test-automation + Go expert)

Backlog (pre-existing, not this PR): split seams for room-service (29 RPCs, ~50-method store; Teams surface is the natural cut) and room-worker (2,546-line handler.go — membership vs. key lifecycle); verify the documented admin-bypass claim on Add Members against `errNotRoomMember` behavior (docs/client-api.md:1239).
