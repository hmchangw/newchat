...
|---|-----|--------|-----|----------|-----|
| 1 | `high` | **Resolve the `LogDispatcher` question explicitly** | Architecture | injected in production wiring at `main.go:61`; `handler.go:54-63` logs and returns nil; it is the only implementation | **Every push event is acked and dropped, and no APNs/FCM call exists anywhere in the service.** Either wire a real dispatcher, or make the placeholder impossible to ship unnoticed — fail startup unless an explicit `PUSH_DISPATCHER=log` opt-in is set, and say so in the deploy files. What must not persist is a binary whose name promises delivery and whose behaviour is a log line. |
| 2 | `high` | **Wrap the handler in `jobguard.Run`** | Architecture | `main.go:84-87` calls the handler directly; **eight** peers do this — `broadcast-worker`, `message-worker`, `notification-worker`, `inbox-worker`, `outbox-worker`, `roomlist-worker`, `hr-sync-worker`, `search-sync-worker` | An unrecovered panic **kills the process and crash-loops on redelivery** — the poison message takes the pod down repeatedly. This is the single most uniformly-applied convention in the repo, and this service is the one that skipped it. |
| 3 | `high` | **Log and signal `iter.Next()` failures** | Architecture | silent return at `main.go:78-81`, `:91-93`; health probe reports only NATS status; `notification-worker/main.go:346` records `LoopFailed` | The consume loop returns silently on any iterator error with **no log, no metric and no health impact**, so the pod stays "healthy" while consuming nothing. A push backlog would be invisible until users complained. |
| 4 | `high` | **Count the consume-loop goroutine in the WaitGroup** | Performance | `main.go:76-89`, `:99-110`; `notification-worker/main.go:337-342` **names this exact window in a comment** | Between `iter.Next()` returning a message and the `wg.Add(1)` two lines later, `wg.Wait()` can observe zero in-flight, `nc.Drain()` proceeds, and **the handler then acks on a drained connection** — a lost push on every rolling restart that lands in the window. |
| 5 | `critical` | **Raise coverage from 26.9% and add an integration test** | Test coverage | `main` and `run` both 0.0% (`covfunc.txt:2481-2486`); no `integration_test.go`, no `TestMain` | Below the 60% critical line. Nothing exercises the real consumer against `testutil.NATS(t)`, so **the durable name, the `FilterSubject` binding to `PushInputWildcard`, and the real NAK-redelivery path are all unverified.** `notification-worker/integration_test.go` is the peer precedent, and CLAUDE.md §1 lists the file in the per-service layout. |
| 6 | `high` | **Make the fake record the NAK delay, and assert it** | Test coverage | `handler_test.go:53`, `:92` — `NakWithDelay(_ time.Duration)` discards the duration | The repo's central JetStream invariant is **never a bare or zero-delay NAK**, because it burns `MaxDeliver` in milliseconds. Here that invariant is untested: the test would stay green if the code regressed to `msg.Nak()`. |
| 7 | `medium` | **Add a per-dispatch deadline** | Performance | message context passed straight through at `handler.go:36` | With a real dispatcher (item 1), **a hung APNs/FCM call holds a semaphore slot indefinitely and silently blows past `AckWait`** (30s default, `pkg/stream/consumer.go:19`) — turning one slow upstream into redelivery storms. Fix this *before* wiring the real dispatcher, not after. |
| 8 | `medium` | **Switch to `jsretry.LowLatencyBackoff`** | Performance | `handler.go:45`; `pkg/jsretry/jsretry.go:47-81`; `broadcast-worker/consumeloop_test.go:545` | User-visible push delivery currently retries on the non-latency-sensitive schedule (first retry 1s) where `LowLatencyBackoff` (200ms) is documented for exactly this class of delivery worker. |
| 9 | `medium` | **Propagate the correlation context from the producer** | Integration / Quality | `handler.go:28-52` ignores `msg.Headers()`; producer propagates them at `notification-worker/main.go:368` | `notification-worker` publishes with `logctx` headers and reads them back on its own consume path; this service drops them, so **a push cannot be correlated back to the message that caused it.** That is also why no log line here carries a `request_id` (`handler.go:31`, `:38`, `:43`, `:49`, `:58`). |
| 10 | `medium` | **Replace the hand-rolled settle tree with `pkg/jsretry`; add `bootstrapStreams`** | Maintainability / Arch | `handler.go:28-52` vs `pkg/jsretry/jsretry.go:86-141`; no `Bootstrap bootstrapConfig` at `main.go:21-31`, `:65` | The permanent/transient policy is duplicated and **must now be changed in two places.** And with no bootstrap helper the service cannot stand up against a fresh NATS and never verifies its input stream exists — mitigated, not excused, by `CreateOrUpdateConsumer` failing fast. `notification-worker/bootstrap.go:33-40` creates `PUSH-NOTIFICATION-{siteID}` as its *output*. |

**A correction to the record.** An earlier premise in this audit — that the service has no `deploy/` artifacts — was wrong, and the reviewing agent checked and corrected it. It **does** ship `deploy/user/{Dockerfile,docker-compose.yml,azure-pipelines.yml}` and a `deploy/bot/` variant; it simply uses the nested per-variant layout that `broadcast-worker`, `notification-worker` and `roomlist-worker` also use, rather than a flat `deploy/`. No deploy-artifact finding stands against this service.

**Also worth doing.** Comment the two `_ = msg.Ack()` discards or check them, as line 48 already does — CLAUDE.md §3 requires a comment for an intentional discard, and having two of three sites silent is worse than either convention applied consistently. Give the fake an injectable `ackErr` so the failed-ack branch (`handler.go:48-51`) is reachable. And convert the four near-identical single-scenario tests into the table-driven form CLAUDE.md prefers, with names following `Test<Type>_<Method>_<Scenario>` — `TestDispatchSuccessAcks` and `TestMalformedJSONAcks` follow neither.
