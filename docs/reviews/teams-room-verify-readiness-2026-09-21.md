...
- [high] The convergence check compares only *cardinality*, so a roster swap silently passes — `teams-room-verify/runner.go:156` — `res.SubscriptionCount != accountsPresent(c.Members)` is a count equality. When `teams-chat-member-sync` replaces member A with B (`teams-chat-member-sync/store_mongo.go:66-71` rewrites `members`, re-raises `needCreateRoom`, bumps `updatedAt`), the count is unchanged; if `room-worker` has not yet applied the new creation event the room still holds A's subscription and no B subscription, and verify reports `ok`. The wire contract itself cannot do better — `model.TeamsRoomVerifyResult` carries `SubscriptionCount int` and no account set (`pkg/model/teams.go:130-136`).
- [high] A chat with `needCreateRoom` still pending is verified and can have `needVerify` cleared — `teams-room-verify/store_mongo.go:34-36` — `ListChatsNeedingVerify` filters `{"needVerify": true}` only and does not even project `needCreateRoom`, so the two pipeline stages are not mutually exclusive. The `updatedAt` CAS does not close this: `teams-room-creation/store_mongo.go:61` sets `{needCreateRoom:false, needVerify:true}` *without* bumping `updatedAt`, so a ref read before that write still matches at `teams-room-verify/store_mongo.go:55`. Combined with the finding above, verify can clear the flag on a room it knows is about to be re-reconciled.
- [medium] `chats_ok` over-reports because the CAS result is discarded — `teams-room-verify/store_mongo.go:58` — `if _, err := s.writeChats.BulkWrite(...)` drops `ModifiedCount`, while `runner.go:161` has already counted every ref as `ok`. In exactly the scenario the CAS exists for (member-sync rewrote the chat mid-pass) the summary at `runner.go:211-218` reports a clean run for chats that stayed flagged.
- [medium] The cross-service trace and request-ID chain is broken at the only HTTP hop in the pipeline — `teams-room-verify/main.go:31` — the job installs a bare JSON handler and never calls `obs.Init`, so `otel.GetTextMapPropagator()` stays a no-op (`pkg/obs/obs.go:225`) and the otelhttp transport that `restyutil.New` installs (`pkg/restyutil/restyutil.go:70`, used at `client.go:23`) emits no `traceparent`. No `X-Request-ID` is set either, though the callee honours one (`pkg/ginutil/middleware.go:22-26`) and wires full o11y middleware (`teams-room-inspector/main.go:53`). Every mismatch WARN is therefore uncorrelatable with the inspector's own logs.
- [medium] The chat-id→room-id derivation that the whole audit rests on is hand-duplicated across two services — `room-worker/teamsroomcreate.go:62` and `teams-room-inspector/handler.go:68` both call `idgen.DeterministicID([]byte(chatID))`, and both comments state the two "must be kept in step by hand". There is no shared helper in `pkg/model/teamsroom.go`; a change on one side turns every chat into a false `missing_room` with no compile error.
- [medium] The flagged set drains only for rooms whose membership never diverges from Teams — `runner.go:156` — migrated rooms are explicitly live (`room-worker/teamsroomcreate.go:70-71`) and nothing forbids a native join/leave on an `OriginTeams` room (repo-wide grep: no `OriginTeams` guard in room-service). Any such room mismatches forever, and `ListChatsNeedingVerify` (`store_mongo.go:34`) is an unbounded full scan with no limit that reloads every one of them, with its full `members` array, on every run.
- [low] Concurrent inspector calls thrash the connection pool — `client.go:22-23` — one shared Resty client serves up to `MAX_WORKERS` (default 8) concurrent batches, often to one host, on a transport left at the stdlib's `MaxIdleConnsPerHost=2`; `restyutil.WithMaxIdleConns` exists for exactly this and is not used.

### Recommendations

- [high] Add `"needCreateRoom": bson.M{"$ne": true}` to the filter at `store_mongo.go:34` so a chat awaiting (re-)creation is never audited, and have `teams-room-creation` bump `updatedAt` in `MarkRoomsCreated` so verify's CAS token actually covers that write.
- [high] Make the comparison set-based: add `SubscribedAccounts []string` to `model.TeamsRoomVerifyResult` (`pkg/model/teams.go:130`) and diff it against the roster in `runner.go:156`, logging the symmetric difference. Cardinality equality cannot detect a swap, which is the most common roster change.
- [medium] Return `ModifiedCount` from `MarkVerified` (`store_mongo.go:45`) and count `ok` from it in `runner.go:166`, so the summary reports cleared chats rather than attempted ones.
- [medium] Move the derivation to one exported helper (e.g. `model.TeamsRoomID(chatID)` in `pkg/model/teamsroom.go`) and call it from `room-worker/teamsroomcreate.go:62` and `teams-room-inspector/handler.go:68`.
- [medium] Wire `obs.Init` in `main.go` (as `teams-hr-sync` and the inspector do) and generate a per-run id via `idgen.GenerateRequestID()` carried in the context, so the Resty hop propagates `traceparent` + `X-Request-ID` and a run is traceable end to end.
- [medium] Bound the scan: page `ListChatsNeedingVerify` by `_id` (the `needVerify_pending` index at `teams-chat-sync/store_mongo.go:65` already supports it), and give a chat that has mismatched for N runs a terminal state so a permanently divergent live room stops being rescanned forever.
- [low] Pass `restyutil.WithMaxIdleConns(cfg.MaxWorkers)` at `client.go:23`.
