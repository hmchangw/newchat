...
|---|-----|--------|-----|----------|-----|
| 1 | `critical` | **Close the coverage gap and add the missing integration test for `GetByAccount`** | Test coverage | 58.6% (239 stmts); `run` 0.0%; all four store funcs 0.0%; `integration_test.go` covers only `ListEmployees` (`:27`, `:106`) and `EnsureIndexes` (`:115`) | Under the CLAUDE.md 60% line. `handler.go`/`cache.go` are 92–100%, so the whole shortfall is `main.go` plus a store whose **login cache-miss fallback projection is never validated against real Mongo** — including whether `roles` decodes into `[]model.UserRole`. That projection deliberately omits `_id`/`employeeId`; nothing pins it. |
