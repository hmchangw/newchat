...
| 5 | `high` | **Index `hr_employee.account`** | Performance | `store_mongo.go:73-75`; `_id` is `employeeId` per `teams-hr-sync/transform/transform.go:47-50` | `account` is an unindexed secondary field with **no `EnsureIndexes` anywhere** in `hr-sync-worker`, `teams-hr-sync` or here, and no init hook in `docker-local/compose.deps.yaml:45-47`. Every page drives a COLLSCAN: at a 100k directory and `$top=500`, ~200 full scans per run. **The single highest-value change in this audit.** |
