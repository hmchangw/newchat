...
| Sev | Finding | Evidence |
|-----|---------|----------|
| high | **The shipped defaults trip the service's own coupling check.** `BULK_BATCH_SIZE=500` at `PIPELINE_DEPTH=2` needs `MaxAckPending >= (depth+1)×batch = 1500`; the shared default is **1000**, so `checkBatchAckCoupling` fires on the out-of-the-box tuning and every production pod logs a warning about its own defaults at startup — and it **only `slog.Warn`s**. What that costs is **pipeline overlap, not batch size**: with no ack headroom for a batch to fill while `depth` are in flight, filling stalls at the ceiling until an in-flight flush completes — and `FlushBatch` settles a whole batch at once, releasing exactly one batch's worth of budget, so the next batch does still reach the cap and fire the size trigger. The pod alternates between "2 in flight, nothing filling" and "1 in flight, 1 filling" instead of holding `depth` in flight *plus* one filling, which is the concurrency `PIPELINE_DEPTH` was set for. The interval flush takes over only when a bulk round-trip outlasts the 5 s `BULK_FLUSH_INTERVAL` — which the next finding makes possible. **`checkBatchAckCoupling`'s own warning text overstates the effect** ("the size-based flush can never fire … batches stay undersized"); an earlier draft of this report repeated that wording as fact. Correct the message along with the default. Only the dev compose overrides `MAX_ACK_PENDING`, **so dev never sees the warning** | `main.go:82`, `:93`, `:163-170`, `:525-568`, `:575-587`; `handler.go:229`, `:269-300`; `pkg/stream/consumer.go:22`; `deploy/docker-compose.yml:40` |
...
### Recommendations
- `high` — Raise the `CONSUMER_MAX_ACK_PENDING` default (or lower `BULK_BATCH_SIZE` to ~300) so the **shipped defaults satisfy `(depth+1)×bulk`**, and fix `checkBatchAckCoupling`'s warning text, which claims an effect the pipeline does not have. Promoting the check to a fatal `os.Exit(1)` is defensible **only after** the defaults are fixed — do it in that order, or every pod with env unset stops booting. And weigh it: what the violation costs is reduced overlap, not incorrect output, so refusing to start is a stronger response than the symptom warrants and is the owners' call, not an obvious follow-on.
...
| 3 | `high` | Make the defaults satisfy the batch/ack coupling, and fix the check's overstated warning text (fatal only after, if at all) | Performance | `main.go:82`, `:93`, `:163-170` | **The shipped default tuning breaks its own invariant**, so every pod warns about its own defaults at startup and runs with the pipeline overlap `PIPELINE_DEPTH` was set to buy: a batch cannot fill while `depth` flushes are in flight. Batches still reach full size — a completed flush releases exactly one batch's budget — so the cost is throughput, not undersized writes. Dev compose overrides it, which is precisely why nobody has noticed. |
