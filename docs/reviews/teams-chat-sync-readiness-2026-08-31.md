...
|---|-----|--------|-----|----------|-----|
| 1 | `high` | **Pin the `nextLink` origin in the chats pager** | Integration | `pkg/msgraph/chats.go:102`, `:115`, token attached at `:136`; the correct pattern at `pkg/msgraph/msgraph.go:683-688` | The users pager in the same package pins it and **says why in a comment**: "a tampered nextLink must not exfiltrate the token to another host". A tampered link sends an app-only `Chat.Read.All` token to an arbitrary host. **The one finding I would block a release on.** Hoist it into a shared `nextPage(origin, next)` helper so a third pager cannot forget, and test that a foreign host is rejected. |
| 2 | `high` | **Flip `GRAPH_TLS_INSECURE_SKIP_VERIFY` to `envDefault:"false"`** | Code quality | `main.go:51`; compose sets `:-false` at `deploy/docker-compose.yml:18`; `main_test.go:32` pins the `true` default | Code and shipped compose deliberately disagree. This is the **enabling half of finding 1** — with verification off, injecting a `nextLink` is easy. The fleet is split (`teams-hr-sync`, `user-presence-service/sync` default false; `teams-user-sync`, `teams-chat-member-sync` default true) — align all four in one PR. |
| 3 | `high` | **Extract `main.go`'s wiring behind a `newDeps(ctx, cfg)` seam** | Test coverage | `run()` at `main.go:91-143`, 0.0% covered | 67.6% is under the 80% floor. `run()` is the whole gap: the Mongo-connect failure, the **`EnsureIndexes`-fails-and-continues branch** (item 4) and the client-construction error are all untestable today. Roughly 15 points of the shortfall is measurement artifact — the store functions *are* covered, by integration tests the unit profile cannot see. |
| 4 | `medium` | **Make `EnsureIndexes` failure fatal, or at minimum alertable** | Architecture | `main.go:114-116`; indexes at `store_mongo.go:44-68` | This service **owns** the three partial indexes that `teams-chat-member-sync`, `teams-room-creation` and `teams-room-verify` scan on. If creation silently fails every run, three downstream jobs degrade to collection scans with **no alert and no failed run to notice**. |
| 5 | `medium` | **Stop the failure storm on SIGTERM** | Performance | dispatch loop `syncer.go:170-176`; per-user error log `:161-166`; summary `:186` | The loop never checks `ctx.Done()`, so on cancellation every remaining user is fed to a worker, fails immediately on the dead context, and is counted a failure with an ERROR log — **the CronJob records a failure for what was a routine pod eviction.** `pkg/msgraph`'s throttle gate already handles cancellation correctly, which makes the gap more visible. Treat `context.Canceled` as a skip. |
| 6 | `medium` | **Give this job its own server-selection timeout and a bounded retry** | Performance | `pkg/mongoutil/poolconfig.go:49` (2s default); writes at `syncer.go:225`, `:230`; Graph retry only on 429/503 at `pkg/msgraph/chats.go:151-153` | The 2s default is explicitly tuned for **cache-fronted request services** that "recover on the breaker's next probe". This batch job has no cache and no breaker: a replica-set election lasting >2s fails every in-flight user, and because the watermark holds, those users wait a full CronJob interval. A transient Graph 500 likewise fails the user outright. |
| 7 | `medium` | **Fix the test fixture's unreachable configuration** | Test coverage | `newTestSyncer` at `worker_test.go:32` leaves `DefaultSiteID` unset; production requires it at `main.go:39` | Nine of eleven `TestRun_*` cases run a configuration production **makes unreachable**. The result is that the defensive empty-vote skip is heavily tested while the actual production behaviour — default siteID applied — rests on a single case at `worker_test.go:241-265`. |
| 8 | `medium` | **Deduplicate chat upserts within a run** | Performance | per-member re-upsert `syncer.go:34-42`, `:224-229`; guard test `worker_test.go:119-157` | Every member's worker re-upserts the same chat, so a 24-member group is written **24 times per run** with the full roster re-serialized each time — a 25× multiplier at the inline threshold. Keep the per-member durability guarantee that the regression test pins; skip only the redundant write when the chat was already written successfully *this run* by a member who also succeeded. |
| 9 | `medium` | **Collapse the triplicated upsert narrative to one comment** | Maintainability | `store.go:23-28`, `store_mongo.go:95-99`, `:112-121` | The three-branch rule (oneOnOne / small-inline / large-deferred) is narrated in full three times. All three must be edited together and **nothing enforces that.** Keep the one at `chatUpsertModel`; reduce the others to a pointer. |
| 10 | `medium` | **Extract the worker-pool skeleton into `pkg/batchrun`** | Maintainability | `syncer.go:131-189` and `teams-chat-member-sync/syncer.go:113-158` | `summary` + `atomic.Int64` counters + unbuffered `jobs` + `MaxWorkers` goroutines + `close`/`Wait` + "N of M failed" is duplicated near-verbatim. Two copies is tolerable; this is a family of six jobs, and **this is the point to extract** — generic over the job type. |

**Also worth doing.** Move `inlineMemberThreshold = 25` (`syncer.go:94`) into `pkg/model` beside `TeamsRoomVerifyMaxChatIDs`, citing Graph's documented cap: its own comment says exceeding it silently loses members, yet it is a bare local const with no counterpart in `teams-chat-member-sync`. Wrap the bare `return err` at `main.go:98`. Cap the per-user Graph accumulation so one pathological user cannot balloon a worker's heap. Rename `worker_test.go` — it tests `syncer.go` and no `worker.go` exists. And add the CronJob carve-out to CLAUDE.md §6's `pkg/shutdown.Wait` rule, which currently makes six correct services read as non-compliant.
