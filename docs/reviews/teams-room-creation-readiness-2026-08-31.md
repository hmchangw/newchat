...
|---|-----|--------|-----|----------|-----|
| 1 | `critical` | **Unit-test `newJetStreamPublisher` against a fake JetStream** | Test coverage | `publisher.go:31-37`, 0.0% covered; round trip proven only at `integration_test.go:101-103` | Coverage is **55.9%, below the 60% critical line**. More to the point, the `Nats-Encoding: zstd` header and the `natsutil.DecodePayload` round trip **are the cross-service wire contract**, and today only Docker proves them. Assert the header and the body round trip with no container. |
| 2 | `high` | **Handle the oversize batch instead of retrying it forever** | Performance | failure logged at WARN `runner.go:84-88`; static `BatchSize` default 100 at `config.go:26` | A batch too large for `max_payload` fails, is logged, and is **retried identically on every subsequent run forever** — nothing splits it, dead-letters it or alerts. The chats simply never leave `needCreateRoom=true`, and with no metrics (item 3) the stall is **invisible**. Detect `nats.ErrMaxPayload` and split, or refuse to build a batch over a configured marshalled-size cap. |
| 3 | `medium` | **Wire `pkg/obs.Init` and emit three counters** | Architecture | `main.go:63-66` (`noop.NewTracerProvider()`) | CLAUDE.md §1 requires the o11y SDK wired once per service. Chats listed, batches published, batches failed. **A silent CronJob whose batches all fail looks identical to a healthy no-op run** — which is precisely the state item 2 produces. |
| 4 | `medium` | **Log `MarkRoomsCreated`'s matched/modified counts** | Performance | result discarded `store_mongo.go:63`; counts available at `pkg/mongoutil/collection.go:169` | A CAS that matches nothing is indistinguishable from a clean clear. A chat whose `updatedAt` churns every run (member-sync racing) **stays flagged and republished indefinitely with zero signal.** Emit a counter for `matched < len(refs)`. |
| 5 | `medium` | **Bound the list with a configurable limit** | Performance | `store_mongo.go:34-38` → `cursor.All` at `pkg/mongoutil/collection.go:65` | Every flagged chat, each with a `members` array, decoded into one slice. This is the one `teams-*` job whose **cold-start set is the entire Teams chat corpus.** The `_id` sort already makes `WithLimit` deterministic across runs. |
| 6 | `medium` | **Mint a run correlation ID and stamp it on published messages** | Code quality | `main.go:36-48`; cf. `teams-hr-sync/main.go:91`, `teams-user-sync/main.go:74` | `natsutil.NewMsg` returns a nil `Header` (acknowledged at `publisher.go:33-35`), so **room-worker mints a fresh ID and the CronJob run cannot be traced to the rooms it created.** Replacing the hand-rolled header guard with `natsutil.NewMsgEncoded` fixes both this and the duplicated quirk-handling. |
| 7 | `medium` | **Bound both Mongo disconnects with a deadline** | Architecture | `main.go:55`, `:61` (both `context.Background()`) | An unresponsive node holds the deferred cleanup **past the pod's termination grace period.** `teams-room-verify/main.go:41-51` defines exactly this pattern correctly and documents why; this service took the fresh-context half and dropped the deadline. The NATS drain at `main.go:73` is bounded correctly. |
| 8 | `medium` | **Add `chatType` to the event** | Integration | source field `pkg/model/teams.go:85`; projection `store_mongo.go:36`; re-derivation `room-worker/teamsroomcreate.go:223-228` | The producer **has the authoritative value** and drops it, forcing room-worker to guess DM-ness from a chat-id string suffix. Any `oneOnOne` chat whose id does not end `@unq.gbl.spaces` **silently becomes a channel room.** |
| 9 | `medium` | **Guard against an empty `SiteID` at the publish site** | Integration | `runner.go:83` | `RoomTeamsCanonicalCreate("")` yields `chat.teams.room.canonical..create` — an invalid empty-token subject whose publish **fails every run forever**, leaving those chats permanently flagged. Upstream does guarantee non-empty, which is why this is medium — but the sibling defends and this one does not. |
| 10 | `medium` | **Fix the stale subject in both doc comments** | Maintainability / Integration | `main.go:4` and `pkg/model/teamsroom.go:6` name `chat.room.canonical.{siteId}.teams.create`; the real subject is `chat.teams.room.canonical.{siteID}.create` (`pkg/subject/subject.go:213`) | The `pkg/model` one is the worse of the two — **it misleads every future consumer of the shared type.** |

**Also worth doing.** Add a cancellation test (cancel after the first batch, assert the rest neither publish nor mark), a `BatchSize=1` and an exactly-`BatchSize` case, and a `MaxWorkers=1` case asserting serialized publishes — none of these are covered, and `run()` sits at 9.4%. Use `context.WithoutCancel` for `MarkRoomsCreated` (`runner.go:89`) so a SIGTERM between PubAck and flag-clear does not guarantee a whole-batch republish, and `break` the dispatch loop on `ctx.Err()`. Delete `chatIDs` (`runner.go:149-156`) — it allocates a full `[]string` per batch **used only for its length**, which equals `len(b.chats)` — and merge the three copying passes over the same heavy slice into one. Add a startup existence check for `ROOMS-TEAMS-{siteID}` per site so a misprovisioned site fails fast rather than warning forever. And record why migrated DMs use `idgen.DeterministicID` rather than `BuildDMRoomID`: deliberate, but it means migrated DMs will not dedup against natively created ones between the same two users.
