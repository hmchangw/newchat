...
- [high] The two binaries re-declare the same presence tunables with their own `envDefault`, so they can silently diverge — `user-presence-service/sync/main.go:24-25` vs `user-presence-service/main.go:42,44`. Both construct the *same* store via `presencestore.NewValkeyStoreFromClient(client, stale, connsTTL)` (`main.go:125`, `sync/main.go:109`), and the stale threshold is baked into `computeLua`'s pruning (`presencestore/store.go:77`). Raising `PRESENCE_STALE_THRESHOLD` on the service but not the cron makes the cron prune connections the service considers live (spurious offline/away + ZREM from the sweep index). CLAUDE.md §Configuration forbids exactly this: the knob belongs to `presencestore` as a named `TTLConfig`-shaped field.
- [high] `sync` re-declares `VALKEY_ADDRS`/`VALKEY_PASSWORD` and hand-rolls the dial — `user-presence-service/sync/main.go:30-31,101-103` — bypassing `valkeyutil.Config`/`ConnectRaw`. The main binary does it correctly (`main.go:53,120`), and `presencestore/store.go:185-190` documents in prose that "a bare `redis.NewClusterClient`+Ping of its own … is exactly the duplication valkeyutil exists to end". The cron therefore gets no instrumentation, no dial policy and no startup ping, contradicting a decision the code itself records.
- [medium] `PRESENCE_HEARTBEAT_INTERVAL` is dead config — declared at `main.go:41`, validated at `main.go:92`, logged at `main.go:99`, set to `15s` in `deploy/docker-compose.yml:19`, and read nowhere in the repo (grep: no other reference). It reads like a server-side heartbeat cadence; an operator tuning it changes nothing.
- [medium] The service's core logic is ~120 lines of Lua inside Go string constants (`presencestore/store.go:68-176`) with **0% unit coverage** — every function in `presencestore` is 0.0% in the provided profile (`store.go:36,198,230,242-341`). Lua in a string gets no compiler, vet, lint or formatting; the precedence ladder can only be exercised by a Docker-backed integration run. This is the single biggest barrier to a new engineer changing behaviour safely in a day.
- [medium] Dangling cross-service contract with no owner: `notification-worker/presence.go:61` requests `subject.PresenceSnapshot` (`pkg/subject/subject.go:1768`), and no service registers it — this service serves only the seven routes at `main.go:213-219`. Flagged in `docs/reviews/user-presence-service-readiness-2026-08-31.md:89` and still unresolved, so `pkg/subject` + `pkg/model` carry a surface a reader reasonably assumes this service owns.
- [low] Store integration tests are split across two packages with two near-identical constructors: `integration_test.go:22` (`newTestStore`) and `presencestore/integration_test.go:19` (`newStore`). The 174-line root `integration_test.go` tests only `presencestore.Store`, not the service — someone changing the Lua must know to edit both files.
- [low] Inconsistent clock ownership makes the store sleep-tested: `Store.mutate` calls `time.Now()` internally (`presencestore/store.go:231`) while `Sweep` takes `now` as a parameter (`store.go:317`). The result is a wall-clock `time.Sleep(40 * time.Millisecond)` to age state at `integration_test.go:115` — the only way to reach the staleness path.
- [low] Four near-identical 13-line handler bodies — validate account/connID, call one store method, publish on `changed` — at `handler.go:47-54,65-72,81-88,97-104`; their tests repeat the same shape non-table-driven (`handler_test.go:64-153`), against CLAUDE.md §4's table-driven preference. Every new fire-and-forget route copies the block twice.
- [nitpick] `sync/config_test.go:16` sets `MONGO_URI`, which `sync`'s `Config` does not declare — stale copy-paste from the main service's env set.
- [nitpick] `newTestReconciler` returns eight values (`sync/reconcile_test.go:35`); adding a ninth reconciler dependency edits every call site.

### Recommendations

- [high] Move `StaleThreshold`/`ConnsTTL` into a `presencestore.TTLConfig` with the env tags and defaults declared once, and mount it in both binaries (`main.go:42,44`, `sync/main.go:24-25`) — removes the silent-divergence class entirely.
- [high] Replace `sync`'s hand-rolled dial with `Valkey valkeyutil.Config` + `valkeyutil.ConnectRaw(ctx, cfg.Valkey, valkeyutil.Instrumented(sdk))` — `sync/main.go:30-31,101-103` — matching `main.go:120` and the rationale in `presencestore/store.go:185-190`.
- [medium] Either wire `PRESENCE_HEARTBEAT_INTERVAL` to something real or delete it from `main.go:41,92,99` and `deploy/docker-compose.yml:19`.
- [medium] Give `computeLua` unit-level coverage — a small table over the precedence ladder driven by miniredis or an extracted pure-Go reference implementation — so `presencestore/store.go:68-118` is not Docker-only. Pair it with injecting `now` into `mutate` (`store.go:231`) to delete the sleep at `integration_test.go:115`.
- [medium] Resolve the `PresenceSnapshot` contract: implement the responder here or delete `pkg/subject/subject.go:1768`, the `PresenceSnapshot*` model types and `notification-worker/presence.go:61`.
- [low] Fold the root `integration_test.go` into `presencestore/integration_test.go` (one helper, one place) and keep the service package's integration tests for service-level wiring only.
- [low] Extract the shared fire-and-forget body behind one helper taking the store call, and convert `handler_test.go:64-153` to a table — four routes become one place to change.
