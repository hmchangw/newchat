...
1. **[high] Purge/patch the coalescer's pending buffer on rewind & edit** — `broadcast-worker/store_mongo.go:108-133` + `coalescer.go:65-79`. Override `RewindRoomLastMessage`/`SetRoomLastMessageEdited` in `coalescingStore` to drop/patch `pending[roomID]` under the mutex. Why: the unguarded flush can resurrect a *deleted* message's full content into `rooms.lastMsg` (indefinitely in a quiet room) or persist a pre-edit body — the one true correctness+retention bug on the branch. Small, local fix.
2. **[high] Skip system messages on the create path** — `broadcast-worker/handler.go:188-199`. Gate `buildLastMessagePreview` on message type (use the same skip-set as the walker). Why: every membership event currently overwrites `rooms.lastMsg` with a system preview, violating the field's documented invariant and the client docs' promise. One-line gate + test.
3. **[high] Add a total-row cap to `scanFirstQualifying` (~250-500 rows → return nil)** — `history-service/internal/cassrepo/last_message.go:81-113`. Why: a mass-deleted tail in a busy bucket means minutes-long walks stacked by redeliveries; `rooms.go:19-20` already sets the 250-row precedent and nil is a handled response.
4. **[high] Gate the delete-path fetch on `room.LastMsgID == msg.ID`** — `broadcast-worker/handler.go:535-543`. Why: removes the RPC + Cassandra walk from the common case (deleting a non-latest message), which is also the cheapest mitigation for the 2s-RPC head-of-line blocking on the shared MaxWorkers semaphore and shrinks the new history-service availability coupling.
5. **[high] Guard the edit patch on `"lastMsg.messageId": editedMsgID`** — `broadcast-worker/store_mongo.go:173-186`. Why: first edit-of-latest on any pre-rollout room otherwise fabricates a schema-invalid partial `lastMsg` served to clients. One filter clause.
6. **[high, decision] Converge the two last-message lanes** — `history-service/internal/service/rooms.go:70-128` vs this branch. Adopt the system-skip semantics in `rooms.get`, reimplement `roomLastMessage` atop `GetLastRoomMessage`, converge on `LastMessagePreview`, align the sender-name cascades (`handler.go:1013` vs `last_message.go:56`). Why: today the same room preview flip-flops (content, trimming, system-message visibility) depending on which path refreshed it. Team/product decision — already flagged on PR #73.
7. **[medium] Truncate preview content (256 runes) before store/encrypt** — `history-service/internal/service/last_message.go:69` + `broadcast-worker/handler.go:1033`. Why: 20KB bodies otherwise land in the hot `rooms` collection, every `DeleteRoomEvent`, and the encrypt path; main's lane already trims.
8. **[medium, sign-off] Plaintext-at-rest + history-window bypass** — `broadcast-worker/handler.go:1027-1037`, `history-service/internal/service/last_message.go:17-20`. Why: `rooms.lastMsg.msg` is a new plaintext content surface in Mongo for non-E2E rooms, and survivor previews reach members whose history window excludes the message. Needs an explicit security ack, atrest treatment, or gating (truncation in #7 reduces but doesn't eliminate).
9. **[medium] Test the visible (TShow=true) thread-reply delete lane** — `broadcast-worker/handler_lastmsg_test.go`. Why: documented at docs/client-api.md:3167 as carrying `lastMessage`; only the hidden lane is covered today.
10. **[medium] Export a canonical `model.SystemMessageTypes` set** — `pkg/model/event.go:577-593` + `history-service/internal/cassrepo/last_message.go:24`. Why: the hand-enumerated skip-set silently leaks any future system type into previews; also fixes the mockgen-independent drift between the two lanes' skip rules once #6 lands.

---

# Status update — fixes applied (2026-07-16)

All simple/reasonable fixes from the action list were applied TDD-style (red tests first, then implementation), verified by lint + unit (race) + full integration on real containers.

1. **[high] Coalescer flush resurrection — ✅ FIXED.** `coalescingStore` now overrides `RewindRoomLastMessage`/`SetRoomLastMessageEdited` to purge/replace/patch the pending buffer under the mutex before delegating; `UpdateRoomLastMessage` no longer lets a nil-preview (system) update clobber a buffered user preview. Remaining exposure (documented in the store comment): a created-event redelivered AFTER its delete can still re-buffer — unchanged from pre-preview `lastMsgId` behavior. 8 new unit tests.
2. **[high] System-message previews — ✅ FIXED.** `handleCreated` gates on `model.IsSystemMessageType`: system messages advance `lastMsgAt`/`lastMsgId` (room sorting) but never become the stored preview. The resulting drift state `{lastMsgId: system, lastMsg: user}` is now first-class: the edit guard keys on `lastMsg.messageId` and the rewind got a phase-2 preview-only path. 2 new unit tests + 3 new integration tests.
3. **[high] Unbounded row scan — ✅ FIXED.** `lastMessageScanMaxRows = 250` (total across buckets, matching the rooms.get ponytail); budget exhaustion degrades to nil. New integration test with a 251-row deleted tail.
4. **[high] Delete-path RPC on every delete — ✅ FIXED.** `handleDeleted` skips the fetch when the stored preview already identifies the survivor (neither `lastMsgId` nor `lastMsg.messageId` targets the deleted message); legacy rooms (no `lastMsg` subdoc) and drift-deleted previews still fetch. Stored previews are served as-is (already trimmed + already `encMsg` in encrypted rooms — no key fetch). 4 new unit tests. This also shrinks the head-of-line-blocking surface and the history-service availability coupling to the "deleted message was the preview" case.
5. **[high] Legacy-room edit fabrication — ✅ FIXED.** The edit patch filter is now `{_id, "lastMsg.messageId": editedMsgID}` — existence + identity in one clause; also correctly patches drift rooms. Integration tests: legacy no-op (no partial subdoc fabricated) + drift patch.
6. **[high, decision] Lane convergence — ⏳ DEFERRED (team decision, flagged on PR #73).** Divergence reduced but not eliminated: both lanes now share `model.TrimPreview` (same 256-rune snippet) and the walker skip-set derives from `model.SystemMessageTypes`; still open: `rooms.get` doesn't skip system types, two wire shapes (`LastMessage` vs `LastMessagePreview`), sender-name cascades still differ (EngName→UserDisplayName→Account vs EngName→AppName→Account).
7. **[medium] Untrimmed preview content — ✅ FIXED.** `model.TrimPreview` (256 runes) applied at every producer: `buildLastMessagePreview` (create), the edit patch (with a separate trimmed-snippet ciphertext when the edit exceeds the cap — the event still seals the full body), and history-service's `lastMessagePreview`; `rooms.get`'s `previewContent` now delegates to the same helper. Docs updated (`LastMessagePreview.msg`/`encMsg` + the §6 crypto note). Also fixed en route: the fetcher's decode error no longer embeds reply fragments (message content) — `malformed reply (N bytes)` instead.
8. **[medium, sign-off] Plaintext-at-rest + history-window bypass — ⏳ OPEN (needs human ack).** Exposure reduced by the 256-rune trim (snippet, not full body, in Mongo/events), but the model question stands: `rooms.lastMsg.msg` remains plaintext-at-rest for non-E2E rooms, and survivor previews still bypass per-member history windows.
9. **[medium] TShow=true delete lane untested — ✅ FIXED.** New handler test asserts the visible thread-reply delete fetches the preview, rewinds, carries `lastMessage` on the event, and publishes the reply_deleted badge.
10. **[medium] Skip-set drift risk — ✅ FIXED.** `model.SystemMessageTypes` + `model.IsSystemMessageType` are the canonical set; the walker's skip-set is derived from it (+ repo-local `MessageTypeRemoved`) with a unit test enforcing the composition, and broadcast-worker's create gate uses the same set.

Also picked up from the lens chapters: Go-expert nitpicks (decrypt error wrapped locally as `decrypt last-message row`; walker query built once via if/else) — ✅ fixed. Not addressed (enhancements/pre-existing parity, not bugs): request-ID `attrs` on the RPC hop, rewind-miss counter, mockgen go1.25 tooling, CLAUDE.md sanction for in-process NATS unit tests, `historyFloor` clamp / `maxBuckets` exhaustion test gaps.
