...
### Recommendations
- `high` — Drop the per-page `CountDocuments` in favour of `EstimatedDocumentCount` when `q==""` — the filter is literally empty there (`store_mongo.go:118-119`), so the two are equivalent — or return `hasMore` from a `limit+1` fetch instead of `total`. That half is free.
- `high` — The scan itself needs care, and an earlier draft of this report was careless about it in two ways:
  - **Do not add a `users` index from this service.** `users` is user-service's collection; this service's own `EnsureIndexes` verifies `account_1` and **warns rather than creates**, with a comment saying so (`store_mongo.go:34-40`). An index for the search path is a change to make in the owning service, coordinated with it — creating one here repeats the ownership mistake this audit flags elsewhere.
  - **An index alone will not serve this query.** `$options: "i"` defeats ordinary index use even for an anchored pattern; only a case-insensitive collation index or a stored normalised (lower-cased) field is actually searchable, and a text index matches whole words rather than substrings. And anchoring the pattern is not a tuning change: today "mit" finds "smith", and after anchoring it does not. That is a product decision about the admin console's search, not a performance fix to make silently.
...
| 2 | `high` | Drop the per-page `CountDocuments`; take the scan itself to user-service, which owns `users` | Performance | `store_mongo.go:133`, `:137`, `:34-41`, `:118-119` | Two unindexed full scans of the all-sites `users` collection **per page view**, on the admin console's primary screen. The count is free to fix (the `q==""` filter is empty). The scan is not: `users` belongs to user-service, `$options:"i"` defeats a plain index anyway, and anchoring the pattern changes what the console finds — see the recommendations. |
