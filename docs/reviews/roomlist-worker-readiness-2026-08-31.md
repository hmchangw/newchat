...
|---|-----|--------|-----|----------|-----|
| 1 | `high` | **Lift the consume loop and its four siblings out of `main.go` into the files their tests already name** | Test coverage / Maint | `main.go:226-400` vs `consumeloop_test.go`, `config_test.go`, `main_test.go`; `handler.go:14-97` | Coverage is **65.9%**, under the 80% merge floor, and the shortfall is almost entirely unwired `main()`. Four test files describe source files that do not exist, and `handler.go` holds the pure derivation rather than the handler. One structural move fixes the coverage gate, the file-organisation deviation and the "where does this test go?" problem together. |
| 2 | `medium` | **Pick a sanctioned consumer pattern and commit to it** | Architecture | `main.go:154`, `:170-173`; rationale at `:327-331` | `cons.Messages()` with a single consume goroutine, no `chan struct{}` semaphore and no `MAX_WORKERS` is neither of CLAUDE.md's two patterns — it is a sequential consumer wearing the pull-iterator's clothes. The reasoning (no per-message I/O; the flusher owns the writes) is good enough to justify `cons.Consume()`; make that the code, or add the semaphore. |
| 3 | `medium` | **Test the transient failure in the third write stage** | Test coverage | `flush.go:176-178` | The `subscription mentions` early return is the one uncovered branch in the write pipeline, and it is a transient-failure path on a `MaxDeliver=-1` consumer — exactly where a wrong classification means an infinite retry rather than a dropped write. |
| 4 | `medium` | **Cover `requestSelfShutdown` and add a JetStream-side `MaxDeliver=-1` integration test** | Test coverage | `main.go:265-274`; `integration_test.go:64` | The service's central contract is "a MongoDB outage retries rather than drops". Today that contract is asserted only at the unit level; nothing exercises it against a real consumer, and the escalation path that gives up and re-signals is untested. |
| 5 | `medium` | **Add `Type` to the projection drift-guard, and set it in the fixture** | Integration | `projection_test.go:57-68`, fixture `:24-40` | The guard asserts 8 of the 9 decoded fields. The omitted one is the highest-consequence: room type drives the whole intent derivation, so a projection change that drops `Type` passes the test that exists to catch exactly that. |
| 6 | `medium` | **Assert the `subscriptions` index at startup** | Integration | `store_mongo.go:151`, `:55-61` | The service issues `(roomId, u.account)` filters on a shared collection **twice per flush at full message rate**, and `mongoutil.WarnMissingIndexes` appears nowhere in the service. It does not own the index, which is precisely why it should warn when it is gone. |
| 7 | `medium` | **Short-circuit `mention.Parse` on messages containing no `@`** | Performance | `handler.go:59`, `:84` | A full RE2 scan runs over every created and edited message, and the large majority contain no mention at all. A byte-level `IndexByte(content, '@') < 0` guard skips the regex entirely on the common case, on the service's hottest path. |
| 8 | `medium` | **Check the mention budget before the merge, and chunk `BulkSetMentions`** | Performance | `flush.go:66-69`; `batch.go:114-119`; `store_mongo.go:158-166` | The budget is enforced *after* merging, so a single message can overshoot it, and the bulk write never chunks — so the overshoot goes to Mongo as one oversized operation rather than being bounded where the budget says it is bounded. |
| 9 | `low` | **Split `main()`'s 157 lines of wiring into named stages** | Maintainability | `main.go:65-222` | Config validation → obs → Mongo → NATS → bootstrap → consumer → flusher → iterator → signal → consume → health → eight shutdown stages, straight-line. Each stage is independently nameable and several are independently testable once item 1 is done. |
| 10 | `nitpick` | **Bring comment volume down to what the code needs** | Maintainability | `flush.go` 127/299 lines (42%), `main.go` 130/400 (33%); e.g. `flush.go:34-51`, `:122-140`, `main.go:276-294` | Six blocks run 12–19 consecutive comment lines. The content is genuinely WHY-oriented and worth keeping — but as doc comments on the functions they describe, not as inline paragraphs a reader must scroll past to find the code. |

**Deliberately not flagged.** The absence of a read path, the `MaxDeliver=-1` setting, and the `msgbucket.NewerRow` tie-break are all load-bearing design decisions that CLAUDE.md documents and this service implements correctly — including the subtle one, that `roomlist-worker` and `broadcast-worker` touch disjoint halves of the room document with separate watermarks and must not be able to pick different "newest" messages at a same-millisecond tie. That invariant holds here.
