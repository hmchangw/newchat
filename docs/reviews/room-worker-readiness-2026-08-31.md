...
- `high` — **Do not** make the post-commit tail best-effort, and **do not** move the OUTBOX federate ahead of the Mongo commits — an earlier draft of this report recommended both, and each trades this divergence for a worse one. The guard above is what removes the reason for either: once `UpdateRoomName` is high-water-marked, a redelivered stale rename is a no-op on both documents, so returning an error after the commits is safe and the tail should keep doing it. The OUTBOX federate in particular must stay error-returning: it is a *local* JetStream publish into the durable retry buffer whose entire purpose is at-least-once cross-site delivery (`outbox-worker` does the forwarding and the retrying), so swallowing its failure drops the remote sites' rename with no heal path until the room is renamed again — the same permanent divergence, moved across the federation boundary. Publishing before the commits inverts it instead: a local write failure leaves the remote sites renamed and the origin not, and an OUTBOX event cannot be retracted.
...
|---|-----|--------|-----------|----------|-----|
| 1 | `high` | Guard `UpdateRoomName` with a monotonic `nameUpdatedAt` high-water mark — and leave the post-commit tail error-returning (see the note under §Integration recommendations) | Integration | `handler.go:2311`, `:2409`; `store_mongo.go:767` | **Permanent data divergence.** A redelivered rename re-applies a stale name to `rooms` while the guarded subscription write refuses to follow — the two documents disagree forever, with no heal path. The code's own comment identifies the hazard, then does the unsafe thing 17 lines later. The guard is the whole fix: it makes redelivery safe, so the NAK on the OUTBOX federate stops being a hazard and stays the thing that keeps the cross-site rename from being lost. |
