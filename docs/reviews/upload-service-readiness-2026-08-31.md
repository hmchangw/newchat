...
|---|-----|--------|-----|----------|-----|
| 1 | `critical` | **Allow-list `drive_host` against the configured base-URL map** | Integration | taken verbatim from the query string at `handler.go:342-358`; API token attached at `pkg/drive/uploader.go:136-140` | **The client chooses which host receives the service's Drive credential.** This is the one `critical` finding in the 35-service fleet. The allow-list already exists — `LoadBaseURLs` builds exactly the set of legitimate hosts — so the fix is to validate against it rather than to trust the parameter. Reject anything not in the map; do not fall back. |
| 2 | `high` | **Make `resolveMediaType` decide from bytes and extension, with the declared type as a tiebreak only** | Code quality / Integration | `mediatype.go:473` returns the client-declared type whenever it is anything but empty or `application/octet-stream`; `/upload/images` never consults `h.mimeFilter` at all (`handler.go:459` vs `:277`) | The byte-sniff, `looksLikeSVG` and extension logic **never run for a lying client** — which contradicts the function's own security comment. And the two upload endpoints enforce **disjoint type contracts**, so a type blocked on one route is accepted on the other. Run both routes through the same filter. |
| 3 | `high` | **Cap the request body before the multipart parse** | Performance | `c.MultipartForm()` at `handler.go:161`, `:237`; all limits are *post-parse* checks on `fh.Size` (`:169`, `:249`, `:254`); `MaxMultipartMemory = 1 MiB` at `main.go:181`; 15-minute timeout at `main.go:41` | With no `http.MaxBytesReader` anywhere, **an oversized request is spooled to the OS temp dir in full — for up to 15 minutes — and only then rejected.** Authenticated room-member disk exhaustion with a trivially cheap request. |
| 4 | `high` | **Fix the Drive HTTP transport** | Performance | bare `&http.Transport{TLSClientConfig: …}` at `pkg/drive/uploader.go:53-56`, never `DefaultTransport.Clone()`, no `WithMaxIdleConns` | It inherits `MaxIdleConnsPerHost = 2` **and throws away** `TLSHandshakeTimeout`, `IdleConnTimeout`, `ExpectContinueTimeout`, dial timeout/keepalive and `ForceAttemptHTTP2`. Every third concurrent upload or download proxy pays a fresh TCP+TLS handshake, idle connections never expire, **and a stalled handshake has no deadline at all.** |
| 5 | `high` | **Require the Drive URL and API token at startup** | Architecture | `pkg/drive/config.go:12-13` declares both with no `required`/`notEmpty`; two instances mounted at `main.go:103-105`; clients built unchecked at `:145-146` | **A pod with an empty `DRIVE_URL` starts healthy, passes `/healthz`, and fails every upload and download at request time.** `BOTPLATFORM_URL` (`main.go:90`) and `cfg.Pool.Validate` show the service already knows the fail-fast pattern. |
| 6 | `high` | **Cap the attachment `description`** | Integration | copied from the form with no length limit at `handler.go:313` | upload-service can mint an `Attachment` that **`message-gatekeeper` will then refuse** — so the file uploads successfully and the message that references it is rejected. Enforce the same cap the gatekeeper does, at the point the attachment is created. |
| 7 | `high` | **Replace `NewHandler`'s 11 positional parameters with a deps struct** | Maintainability / Quality | `handler.go:72-80`, 8 bare scalars/bools, **two arguments of the same interface type at opposite ends of the list**; 10 call sites already pass magic literals (`…, 0, nil, testCacheMaxAge, true, …`) | Trivially transposable with no compile error, and adding one knob is a 10-site edit. It also invites the latent bug already present: **a `nil` `mimeFilter` is constructible and panics on first use** (`mediatype.go:60-68`; the tests do pass `nil` at `handler_test.go:170`), safe only because `newMediaTypeFilter` never returns nil. **The right pattern already exists one file over** — `authDeps`. |
| 8 | `medium` | **Raise coverage from 76.5%, starting with `run()` and the file-upload rejection branches** | Test coverage | `run()` 0/70 (14.4% of the service), holding the DEV_MODE/OIDC guard at `main.go:159-161` and the bucket default at `:151-154`; uncovered branches at `handler.go:221-224`, `:227-230`, `:238-243`, `:262-265`, `:464-466` | Under the 80% floor. Note the asymmetry: **the identical rejection branches on the images twin *are* tested** — only the file twin's are not, which is the duplication of item 9 showing up as a coverage hole. And the stated "no orphaned Drive file" invariant is **unasserted**: dimensions are read before the Drive upload specifically so a read failure cannot orphan a file, and nothing proves `UploadGroupImages` is not called on that branch (`handler.go:283-290`). |
| 9 | `high` | **Extract the 31-line duplicated handler preamble** | Maintainability | byte-identical `handler.go:136-167` and `:213-244` — roomID check → `userFromContext` → email guard → `requireMembership` → `MultipartForm`, comment included | **Any fix to the auth/membership preamble must be made twice and can silently diverge** — which is exactly what happened to the coverage in item 8, and to the type filtering in item 2. The download-response header map is duplicated the same way (`:352-357`, `:419-427`), including a security-relevant CSP header. |
| 10 | `medium` | **Assert the subscriptions index at startup** | Performance | `MemberSiteID` filters `{roomId, u.account}` on **every upload and every download** (`store_mongo.go:26-32`, called at `handler.go:435`); `NewMongoStore` asserts nothing; peers use `mongoutil.WarnMissingIndexes` (`inbox-worker/main.go:562`, `user-service/mongorepo/subscriptions.go:75`) | A missing room-service-owned index degrades to **a COLLSCAN on the authorization path of every request** — silently. |

**Also worth doing.** Stop `LoadBaseURLs` failing open: a missing or malformed baseurls file logs a warning and installs an empty map (`pkg/drive/config.go:23-40`), after which every cross-site room's upload and download **silently routes at the local Drive**. It is also the only file-based config in the service, against CLAUDE.md §6 — and, once item 1 lands, it is the allow-list, so failing open there means failing open on the security control. Separate the client-facing stream deadline from the internal one: `driveTimeout` is documented as "sized for the internal leg only", but `http.Client.Timeout` covers reading the response body, which `GetGroupImage` hands straight to the client — so **client downloads are capped at 5m by a timeout that was never meant to bound them**, and `MinioDownloadTimeout` has the same shape. Stop collapsing Drive 404s into 503s (`handler.go:358-362`). Move `objectStore` and `driveClient` into `store.go` so mockgen covers them — today the tests hand-roll `fakeS3` and `fakeDrive`, which drift from the interfaces silently. Give `MINIO_BUCKET` a real `envDefault` or `required` rather than a silent in-code fallback. Drop the dead `upload.UserID` and `upload.ID` from the projection. Fix the broken internal references to this service's own section in `docs/client-api.md` (`:45`, `:882`, `:6355`). And convert `handler_test.go` — 70 top-level tests, exactly **one** `t.Run` — to the table-driven form, starting with the ~120 duplicated lines where `TestDownload_*` and `TestDownloadV3_*` cover the same six scenarios against an already-shared implementation.
