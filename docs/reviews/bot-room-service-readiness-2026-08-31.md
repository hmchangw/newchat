...
- `high` — **The code fix alone leaves every row already written still malformed, and this report originally stopped at the code fix.** `$setOnInsert` applies only on insert, and a re-execute against an existing row is a documented no-op (`store_mongo.go:98`), so neither the new fields nor a corrected `siteId` reaches any subscription this service has already created. Ship a one-off backfill **after** the code change (before it, new writes keep reintroducing the shape), with these recovery rules:
  - `joinedAt` ← the row's own `createdAt`. Every row this service writes binds it (`handler.go:254`, `:335`), so for this population it is exact, not a guess. For any row missing it, use the room's `createdAt` as a documented lower bound — **never `now()`**, which would sort a years-old member as the newest join under `member.list`'s `{roomId, joinedAt, _id}` pagination.
  - `roles` ← `["user"]` on channel member rows, matching `room-worker.newSub`.
  - `siteId` ← the owning room's `siteId`, for channel rows where the two disagree. That is the invariant every other writer already satisfies, so scoping the update by the disagreement itself both finds exactly the affected rows and leaves other writers' rows untouched. Read `rooms` first and update per room — no `$lookup` (CLAUDE.md §6).
...
|---|-----|--------|-----|----------|-----|
| 1 | `high` | **Write `joinedAt` and `roles` on every subscription — then backfill the rows already written** | Integration | `store_mongo.go:101-109`; every other writer sets both (`room-worker/handler.go:1510`, `inbox-worker/handler.go:322`); consumed at `room-service/store_mongo.go:140`, `:745-750` | room-service's `member.list` **projects `roles` and paginates on the `{roomId, joinedAt, _id}` index.** A subscription without them is a row that sorts wrong and reports no roles — in a collection four services read. **`$setOnInsert` never repairs an existing row**, so the code change fixes only future memberships; `joinedAt` recovers exactly from each row's `createdAt` (see §Integration recommendations). |
| 2 | `high` | **Use the room's site, not the member's, for channel subscriptions** | Integration | `handler.go:254`, `:335`; convention at `room-worker/handler.go:1170` (`SiteID: room.SiteID`) and `inbox-worker/handler.go:~310`; consumed at `user-service/service/subscriptions.go:237`, `:789-795` | user-service's `subscription.list` **groups rows by `sub.SiteID` to fetch room metadata from that site** — so a cross-site member's row sends the lookup to the wrong site and the room renders without metadata. **The DM and owner paths in this same service use `h.siteID` correctly**, which is what makes this a bug rather than a convention choice. Rows already written keep the wrong site — `$setOnInsert` does not update them — so this needs the same backfill as item 1. |
