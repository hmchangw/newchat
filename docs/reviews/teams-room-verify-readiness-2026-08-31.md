...
|---|-----|--------|-----|----------|-----|
| 1 | `high` | **Bound the flagged-chat scan** | Performance | `store_mongo.go:34-36`; `pkg/mongoutil/collection.go:60-73`; flag raised at `teams-room-creation/store_mongo.go:61` | `FindMany` accumulates the entire cursor into one slice, and `planBatches` re-copies every chat into per-site slices. **The backlog grows without bound whenever a site's inspector is down — so the failure mode this job exists to detect is the one that makes it OOM.** Paginate on `_id` using the sort that is already `_id`-ascending for exactly this reason. |
| 2 | `high` | **Close the 1.1-point coverage gap in `run()`** | Test coverage | 78.9% of 171 stmts; `main.go:main` 0%, `disconnect` 0%, `run` 33.3% | Under the 80% floor, so the dimension is floored at 2 — but the whole gap is wiring, and `run()`'s early-exit branches are table-drivable **with no Mongo at all.** Cheapest possible route back over the gate. |
| 3 | `medium` | **Stop dispatch on a canceled context** | Performance | `runner.go:78-86` | No `ctx.Err()` check and no `select` on `ctx.Done()` when acquiring the semaphore. After SIGTERM every remaining batch is still launched, each fails fast in resty, and the run reports `failedBatches` — **a routine pod eviction is indistinguishable from a total federation outage** in both logs and exit status. |
| 4 | `medium` | **Return and log `MarkVerified`'s `BulkWrite` result** | Code quality / Perf | `store_mongo.go:58`; counted `ok` at `runner.go:160-161`, summarized `:215` | It returns `nil` whether it cleared N chats or 0, and the runner has already counted those chats as `ok`. **`chats_ok` therefore over-reports every run in which member-sync re-wrote a chat mid-pass — precisely the case the CAS exists for.** `mongoutil.BulkResult` already carries the counts; log `chats_cas_missed`. |
| 5 | `medium` | **Add `LOG_LEVEL` and mint a run correlation ID** | Code quality | handler built with nil options `main.go:31`; no ID at `main.go:71`; `pkg/restyutil/restyutil.go:75`, `:97-99` | CLAUDE.md names log level as config that must have an `envDefault`, and requires an ID generated at the entry point. Today `restyutil`'s per-request `slog.Debug` is **unreachable in production without a rebuild**, and nothing correlates one pass's inspector calls, mismatch warnings and summary. `restyutil` already emits `request_id` when one is on the context — supply it. |
| 6 | `medium` | **Wire `pkg/obs.Init`** | Architecture | `main.go:30-36`; `pkg/mongoutil/mongo.go:41-42` | A CronJob **whose entire product is an audit signal exports zero metrics**, and `mongoutil.Connect` silently takes its uninstrumented path. All five `teams-*` jobs skip it, so fix the family in one PR. |
| 7 | `medium` | **Add `-tags=integration`, `make lint` and `make sast` stages to the Azure lane** | Test coverage | `deploy/azure-pipelines.yml:45` | The store tests run in the GitHub lane (`.github/workflows/ci.yml:119-140` auto-discovers them) but not the service's own pipeline, which also runs `go vet` rather than `make lint` and has no SAST stage — a blocking gate per CLAUDE.md §5. |
| 8 | `medium` | **Extract the classification rule from `verifyBatch`** | Maintainability | `runner.go:96-170` (75 lines, four abort paths) | `!RoomExists` → missing room; `SubscriptionCount != accountsPresent` → mismatch; else converged **is the service's entire domain logic**, and it is reachable only through batch plumbing, a mock store and a fake verifier. A `classify(chat, result) outcome` makes the matrix directly table-testable — and adding the fifth outcome `pkg/model/teams.go:135-137` already contemplates stops meaning surgery in the middle of this function. |
| 9 | `medium` | **Promote the verify endpoint path into `pkg/model`** | Integration | `client.go:13` and `teams-room-inspector/routes.go:9`; cf. `TeamsRoomVerifyMaxChatIDs` at `pkg/model/teams.go:110` | The batch cap is shared through `pkg/model` **precisely so both ends cannot drift** — the path deserves the same treatment. A rename on the inspector compiles cleanly and fails only at runtime, as 404s this client reports as a generic status error. |
| 10 | `medium` | **Size the shared resty client's idle pool to `MaxWorkers`** | Performance | `client.go:23`; `pkg/restyutil/restyutil.go:37-39` | One client serves up to 8 concurrent calls against the stdlib default of 2 idle conns per host, so **most calls in a pass re-handshake.** `restyutil.WithMaxIdleConns` exists for this and documents the cost. |

**Also worth doing.** Add `TestRunner_CanceledContextStopsDispatch` and a `MaxWorkers: 1` case — no test today asserts the semaphore ever blocks, because `TestRunner_BatchesLargeSites` produces 3 batches against a limit of 4. Drop the duplicated `"list chats needing verify: "` prefix at `runner.go:67`; the store already supplies it. Mount `mongoutil.PoolConfig`/`BreakerConfig` as named fields rather than running both clients on library defaults. Rename `store_mongo_test.go` → `integration_test.go` to match the per-service layout. And add one bounded retry for connection-level inspector failures, so a single blip does not defer a whole batch a full CronJob period.
