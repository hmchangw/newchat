...
|---|-----|--------|-----|----------|-----|
| 1 | `high` | **Refuse to advance a chat on an empty roster** | Integration | unconditional advance `syncer.go:184`; write `store_mongo.go:66-71`; downstream delete `room-worker/teamsroomcreate.go:153-158`, `:176-178` | A 200 with an empty `value` — e.g. after the app loses access to a chat — stores `members: []` and sets `needCreateRoom: true`. `room-worker` then **deletes every subscription not in that list.** One degraded Graph response silently empties a room. **A room is never legitimately zero-member**: log, count, and leave `needMemberSync` true. |
| 2 | `high` | **Count and log unresolved members; decide whether they block the advance** | Integration | empty `Account` persisted `store.go:44-47`, `syncer.go:100-106`; skipped downstream at `room-worker/teamsroomcreate.go:106-109`; counters are chats-only `syncer.go:160-163` | A member absent from `teams_user` gets an empty `Account`, the chat is marked `needMemberSync: false`, and it is **never retried** — so the person is permanently omitted from the room. This service emits **no log and no counter** for it; downstream emits one WARN and continues. Today the failure is invisible on both sides. |
| 3 | `high` | **Add `TestRun_EmptyGraphRosterIsRejected`** | Test coverage | no test covers a zero/empty roster | Write the test first, then the guard in item 1. Given the destructive downstream, **this is the single most important missing case** in the service. |
| 4 | `high` | **Flip `GRAPH_TLS_INSECURE_SKIP_VERIFY` to `envDefault:"false"`** | Code quality | `main.go:44`; the correct default at `teams-hr-sync/config.go:28`, `user-presence-service/sync/main.go:43` | Fail-open on the connection that POSTs `client_secret`. The `#nosec G402` + `//nolint:gosec` double-suppression in `pkg/msgraph` is correct — **the defect is the default, not the mechanism.** Also log a startup WARN when it is on, so an accidentally-insecure prod run is visible. |
| 5 | `medium` | **Move `teams_user` resolution to the primary client** | Integration | `main.go:91`, `store_mongo.go:26-28`; contrast `teams-chat-sync/store_mongo.go:16-21` | The sibling keeps **all** teams-collection access on the primary precisely because these collections are freshly populated. Replication lag here turns a just-created `teams_user` into the permanent empty-account outcome of item 2, and a stale `updatedAt` into a spurious `errSuperseded`. Keep the chat scan on the secondary; move the user lane. |
| 6 | `medium` | **Bound the backlog with a `MAX_CHATS_PER_RUN` limit** | Performance | `store_mongo.go:36-37`; flag re-raised on every large-chat re-sync at `teams-chat-sync/syncer.go:124` | `FindMany` runs `cursor.All` over every flagged chat with no limit, batching or pagination. A full re-sync makes **one run responsible for the entire backlog** with no ceiling — unlike `teams-room-verify`, which has a `VERIFY_BATCH_SIZE`. The projection keeps rows small, so this is a run-duration and Graph-quota risk more than a memory one. |
| 7 | `medium` | **Make dispatch cancellation-aware and classify `context.Canceled` separately** | Performance | `syncer.go:154-157`; error log per chat `:149` | No `select` on `ctx.Done()`, so SIGTERM drains the whole backlog with each chat failing fast, one `slog.Error` per chat, and a non-zero exit — **a cancelled run is indistinguishable from a broken one** in both logs and CronJob status. |
| 8 | `medium` | **Add `TestRun_BuildMembersFailureFailsChat`** | Test coverage | the one uncovered `syncChat` statement, `syncer.go:181-183` | `UsersByIDs` failure is tested directly but never *through* `run`, so nothing pins that a Mongo failure mid-chat marks the chat **Failed rather than Superseded** — and the two are handled very differently. |
| 9 | `medium` | **Let `run` own every counter** | Maintainability | `sum *summary` threaded into `syncChat` at `syncer.go:175`, `:187`; every other counter updated in the switch at `:141-150` | Two places own the tally, so adding a third counter means deciding again which side owns it. Return `len(members)` from `syncChat` and drop the parameter. |
| 10 | `medium` | **Add an integration case for a doc with no `updatedAt`** | Test coverage | conditional filter `store_mongo.go:53` | With a zero `updatedAt` the CAS filter never matches, **stalling that chat permanently as Superseded** — silently. Whatever the intended behaviour, skip-with-warning beats silent permanent stall, and nothing asserts either today. |

**Also worth doing.** Consider a sanity floor — refuse a roster that drops below some fraction of the currently stored member count — given that `DeleteSubscriptionsByAccounts` downstream is unconditional. Add a per-chat `context.WithTimeout` so one hung dependency cannot consume the whole CronJob window, and optionally prime `userRefCache` with a single union `UsersByIDs` before fan-out, removing the documented cold-start stampede entirely. Move `errSuperseded` out of `store.go` — it is a syncer-level control-flow concept. Fold `worker_test.go` and `log_test.go` into `syncer_test.go`; neither names a production file. And note the `teams-chat-sync`-owned index dependency in this service's package comment, since running this job first against a fresh cluster means a COLLSCAN.
