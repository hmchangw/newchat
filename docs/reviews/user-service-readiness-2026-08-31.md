...
- `medium` — Bound the phase-one `Find`, but **not with a plain `SetLimit` sized to the page** — an earlier draft of this report said exactly that, and it would silently return wrong lists. The `Find` carries **no sort** (`mongorepo/subscriptions.go:298-300`) and the ordering key is derived afterwards in Go from freshly-read room documents (`resolveSortKeys`, `buildListRows`, `sortListRows`); `sortListRows`' own comment calls the Find's order arbitrary. Truncating before the sort therefore drops an arbitrary subset — quite possibly the account's most recently active rooms — from every page, with no error. Two things that do work:
  - **Use the limit as an overflow detector, not a truncation**: `SetLimit(maxSubs + 1)` and, when the extra row comes back, fail loudly or flag the response as truncated. That bounds per-request cost while keeping a wrong page impossible.
  - **The durable fix is to denormalise the sort key onto the subscription document** so Mongo can sort and page server-side. Larger: it needs a writer to maintain the field and a backfill, and today the key lives on the room (`LastUserMsgAt`/`LastMsgAt`/`CreatedAt`).
...
| 9 | `medium` | Bound the phase-one subscription `Find` **as an overflow check, not a truncation**; project `ListApps` and the three unprojected reads | Performance | `mongorepo/subscriptions.go:298-300`, `:405-433` | Per-request memory and sort cost currently scale with an account's **total** subscriptions rather than page size. But the `Find` is unsorted and the order is derived in Go afterwards, so a plain page-sized `SetLimit` would drop arbitrary rooms from every page instead — see the recommendation. |
