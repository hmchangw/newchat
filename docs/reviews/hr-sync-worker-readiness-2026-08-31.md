...
| 3 | `high` | **Resolve the HR stream-ownership inversion** | Architecture | this consumer is the **only** creator of `HR-{siteID}` (`bootstrap.go:29-37`); the publisher creates nothing (`teams-hr-sync/publisher.go:38`, `:50`, `:67`); `search-sync-worker/main.go:306-307` states "HR is owned by hr-syncer … must not create their schemas" | **Two consumers hold contradictory beliefs about who owns HR.** Move creation to the declared owner and reduce this service to verify-only, or amend the two comments so one owner is named. |
