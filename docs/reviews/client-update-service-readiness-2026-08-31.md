...
|---|-----|--------|-----|----------|-----|
| 1 | `high` | **Make the read deadline configurable, default it to `HTTP_WRITE_TIMEOUT`, and set `ReadHeaderTimeout: 5s`** | Performance / Integration | `ReadTimeout: 30 * time.Second` at `main.go:80`; cap at `config.go:47`; the correct pattern **with its rationale** at `upload-service/main.go:38`, `:205-207` | `ReadTimeout` bounds the **entire request including the body**, so any upload over 30s is severed mid-body: ~570 Mbps sustained for a 2 GiB artifact, ~57 Mbps for 200 MiB. `HTTP_WRITE_TIMEOUT` governs the response and cannot help. This **silently breaks `admin-service`'s documented 10m relay budget** (`admin-service/config.go:78-81`). Add `ReadHeaderTimeout` in the same change — today `ReadTimeout` is the only slowloris guard, and raising it removes that. |
| 2 | `high` | **Extract `newServer(cfg, handler) *http.Server` out of `run()` and unit-test the timeout matrix** | Test coverage | 76.8% (276 stmts); `run()` 0%, **50 of ~64 uncovered statements** | Excluding `main.go` the package is ~93.8%, so this is not a testing-effort problem — server construction is inlined and therefore unassertable. **This single refactor clears the 80% floor and is the test that would have caught item 1.** |
| 3 | `medium` | **Add `minioutil.Config` and mount it as a named field in all three services** | Architecture | `client-update-service/config.go:19-24`; `upload-service/main.go:95-101`; `media-service/config.go:70-74`; `pkg/minioutil` exposes no `Config` | CLAUDE.md §6's shared-knob rule, and **the drift is already visible**: `MINIO_BUCKET` is `required` here, `envDefault:"avatars"` in media-service, and defaulted-to-empty in upload-service. Keep the bucket service-local; the buckets genuinely differ. |
| 4 | `medium` | **Bound the download cache by bytes, not entries** | Performance | `config.go:49-51`; `cache.go:31-37`; allocation at `version.go:173`; the same values shipped at `deploy/docker-compose.yml:26-28` | `CACHE_MAX_ENTRIES` 4 × `CACHE_MAX_OBJECT_BYTES` 512 MiB = **2 GiB of resident heap at steady state**, plus in-flight fills, with nothing coupling the defaults to the pod memory limit. Either add a total-bytes budget or document the product as required headroom in the deploy manifests. |
| 5 | `medium` | **Validate the two byte caps against each other** | Integration | `config.go:47` and `admin-service/config.go:86`, both defaulting to 2147483648 | Two independently-declared knobs that must agree, with nothing validating them: **raise one alone and a relayed upload becomes an opaque 400 at the far end.** Either derive both from one operator-facing knob, or have `admin-service` surface a 413 naming both limits. |
| 6 | `medium` | **Return the already-open reader for oversize objects** | Performance | `loadObject` opens and closes to learn the size `version.go:164-178`, then `streamObject` re-opens `:159`, `:181-195` | Two `GetObject`+`Stat` round trips on **precisely the latency-sensitive case.** A `loadOrStream` returning the still-open reader removes both the second `Stat` and one code path. |
| 7 | `medium` | **Unit-test `cancelReadCloser.Close` and the `Stat` not-found mapping** | Test coverage | `store_minio.go:47-49`, `:50`, `:93-97`; reachable only under `//go:build integration` | **Nothing asserts that `cancel()` actually runs on `Close`**, so a leaked download-timeout context would go unnoticed in `make test`. Cover the contract with a fake rather than a container. |
| 8 | `medium` | **Assert `run()`'s `bucketClient` type-assertion failure path** | Test coverage | `main.go:55-58` | A `minioutil` change that stops satisfying `bucketClient` fails **only at runtime startup** today. |
| 9 | `low` | **Set `X-Content-Type-Options: nosniff` on both serve paths** | Code quality / Perf | uploader-supplied type stored at `version.go:105`, echoed at `:199-203` | Download responses echo an uploader-supplied `Content-Type` back to **unauthenticated** GETs, with `Content-Disposition: attachment` as the only mitigation. A `text/html` part header stored today is served from this origin tomorrow. Add a modest rate limit on the same route while there — a cache miss turns each request into a MinIO round trip. |
| 10 | `low` | **Enforce the cross-service deadline invariant at startup** | Maintainability | documented only as prose in another service at `admin-service/config.go:78-80`; `admin-service` already has `checkHandlerTimeout` at `:124` | The invariant that makes item 1 correct exists **only as a comment in a different repo directory.** A startup check that the read deadline covers the artifact-transfer budget makes it enforced rather than described. |

**Also worth doing.** Replace `err != http.ErrServerClosed` with `errors.Is` at `main.go:101`. Close over `shutdownDone` on every `run` return path — an early `ListenAndServe` failure currently leaves the `shutdown.Wait` goroutine parked, harmless only because `main` then calls `os.Exit(1)`. Fix the stale comment at `config.go:57-59` claiming only one field is `int64`. Export the `chat.go/chat-versions/` object prefix (`version.go:21`) as a documented constant shared with `admin-service`, so the relay and the store agree by construction rather than by string. Soften the peak-heap assertion from a fixed 24 MiB to a ratio of the artifact size — a valuable test, but a load-sensitive one. Either rename `version.go` → `handler.go` or record the split as deliberate. And re-run `make sast-vuln` from an egress-permitted runner: `govulncheck` and the semgrep registry packs could not execute here.
