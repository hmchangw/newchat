...
|---|-----|--------|-----|----------|-----|
| 1 | `high` | **Move `botRateLimit` ahead of `requireBot`** | Performance | `routes.go:40` — `out := []gin.HandlerFunc{auth}` **before** `rateLimit`; positive-only session tier at `pkg/sessioncache/sessioncache.go:11-16`, uncached miss at `:90-95` | **An invalid token is never rate-limited**, and every bogus-token request is one uncapped MongoDB `FindOne`. Token spraying becomes an unmetered load generator **against the same Mongo the breaker exists to protect.** One-line reorder. |
| 2 | `high` | **Rate-limit `/api/v1/login`** | Performance | unauthenticated route with a full bcrypt verify at `handler.go:120`; no limiter in the global chain at `main.go:120-125`; `botRateLimit` attached only to bot routes at `routes.go:29`, `:40-42` | ~50–100 ms of CPU per request, on an unauthenticated endpoint, with nothing shedding load. `admin-service` and `auth-service` have the same shape — this is the fleet's unauthenticated-bcrypt pattern, and this is one of the three places to fix it. |
| 3 | `high` | **Cap the idempotency middleware's body read** | Performance / Quality | uncapped `io.ReadAll(c.Request.Body)` at `middleware_idempotency.go:60` | The **entire** request body is buffered in RAM before the handler runs, with no `http.MaxBytesReader` anywhere. Same defect class as items 1–2: unbounded work reachable without valid credentials. |
| 4 | `high` | **Inject all five handler dependencies through the constructor** | Architecture / Maint | `newHandler` takes only store and cfg (`handler.go:46`); `h.subs`, `h.forwarder`, `h.dmEnsurer` poked in at `main.go:107`, `:114`, `:116`; dereferenced unguarded at `bot_handlers.go:41`, `:51`, `:78` | CLAUDE.md §3 requires constructor injection. **Nothing forces those three to be set** — a dropped wiring line is a nil-panic on the first bot request, not a startup failure or a compile error. Adding a sixth dependency means remembering a fourth assignment site. |
| 5 | `high` | **Stop classifying errors by matching error text** | Code quality | `bindStrict` does `msg := err.Error()` then `strings.Contains(msg, "unknown field")` at `bot_handlers.go:210-212` | CLAUDE.md §3: never compare errors by string — use `errors.Is`/`errors.As`. This one silently changes behaviour whenever the stdlib rewords a decode error. |
| 6 | `high` | **Own `SESSIONS_MAX_PER_ACCOUNT` and `BCRYPT_COST` in one place — and delete `BcryptCost` or use it** | Architecture | re-declared with their own tags and defaults at `config.go:34`, `:37` and `admin-service/config.go:38`, `:44`, both operating on the same `sessions` collection and the same `pkg/pwhash` recipe; `BcryptCost` parsed and validated at `main.go:44` but **nothing in the service hashes a password** — `verifyPassword` calls `pwhash.Verify`, which takes no cost (`handler.go:227`) | CLAUDE.md §6's shared-knob rule, in its most consequential form: two services can silently disagree about how many sessions an account may hold. And **a validated-but-unread knob is worse than an absent one** — it fails startup on an operator value that has no effect. |
| 7 | `critical` | **Raise coverage from 56.5%, starting with the federation forwarders and cross-site routing** | Test coverage | below the 60% line; `dm_ensurer.go:31-74` **0% end to end**; all four room-management forwarders 0% at `bot_forwarder.go:57`, `:94`, `:98`, `:102`; **cross-site routing — the service's reason to exist — asserted nowhere** (`routes_bot_test.go:25-45`); `mongoSubscriptionStore` 0% with no integration test (`subscription_store.go:40-76`) | Every NATS federation path in the service is untested, and so is the routing decision that justifies the service existing. `requireBot`'s store-failure branch (`middleware.go:69-72`) — the one item 1 makes hot — is uncovered too. |
| 8 | `high` | **Fix the documented member endpoints** | Integration | code registers `POST …/members/add` and `…/members/remove` (`routes.go:69-73`); `docs/client-api.md:8584-8585` and the routing table at `:8466-8467` publish `POST …/members` and `DELETE …/members` | **Any SDK written against the documentation gets a 404 on both add and remove.** And the derived view (`docs/client-api/request-reply.md:256-261`) lists only login and auth-validate, stating "Emits: None — HTTP-only", **silently dropping all five bot endpoints** that canonical §10.3–10.7 documents. |
| 9 | `medium` | **Make the room-management timeout reachable and configurable** | Architecture / Perf / Maint | `forwardRoomMgmt` hardcodes `15*time.Second` at `bot_forwarder.go:75`, **ignoring the `timeout` field its constructor was given** at `:34`, `:132`; `ginutil.Timeout` already stamped a 10 s deadline (`main.go:124`, `pkg/ginutil/timeoutconfig.go:18`) | Two defects in one line. `newBotForwarder(nc, 3*time.Second)` silently configures **half** the forwarder; and the 15 s budget is unreachable anyway — the request is cancelled at 10 s and mapped to 503 `handler_timeout` first. **The idempotency TTLs are sized against 15 s**, so they are reasoning from a number that never applies. This is also the one budget that scales with batch size (up to 100 userIDs). |
| 10 | `medium` | **Collapse the three copies of the bot-RPC skeleton** | Maintainability | `forward` (`bot_forwarder.go:106-156`) and `forwardRoomMgmt` (`:57-92`) are ~85% identical — nil-session guard, `BotIdentity` marshal, `natsutil.NewMsg` + nil-header dance, header set, `WithTimeout`, the identical `nats.ErrTimeout` → `errcode.Unavailable` branch, the identical `errcode.Parse`; `dm_ensurer.go:35-74` is a third copy | Any change to bot RPC semantics must land in three places. **The duplication is why item 9 exists** — one copy took the timeout as a field, another as a literal. |

**A dead contract worth deciding on.** `model.BotIdentity` carries `EngName`/`ChineseName`/`AppID`/`AppName` (`pkg/model/bot.go:23-31`) and **downstream services read them** — `bot-room-service/handler.go:199-200`, `:281` persist the room owner's `appId`/`appName`, and `bot-message-handler/handler.go:113`, `:168` derives `UserDisplayName`. But this service, **the only producer**, constructs the header with `{ID, Account, SiteID}` only (`bot_forwarder.go:62`, `:114-118`, `dm_ensurer.go:43`). The enrichment is dead-empty in production across three services. Either populate it here or delete the fields and the downstream reads.

**Also worth doing.** Drop the log-AND-return in `denied` (`handler.go:180-181`). Return raw `fmt.Errorf` for infra failures rather than dressing them as `errcode.Internal(…, WithCause(err))` — about twelve sites across `middleware.go`, `middleware_idempotency.go`, `bot_forwarder.go`, `dm_ensurer.go`, `bot_handlers.go`. Move `subscriptionStore` into `store.go` so the `//go:generate mockgen` directive covers it — today one store surface is mocked mechanically and the other by hand (`fakeSubStore`), which is exactly the split that lets a signature change land untested. Wrap the bare `err` at `store_mongo.go:99`. Cover the uncovered login credential branches (malformed JSON `handler.go:93`, `tokenGen` failure `:132`, best-effort cap-delete failure `:152`) and the rate-limit failure branches (`middleware.go:96`, `:127`, `:150`). And either emit `dm_ensure_unavailable` or remove it from `docs/client-api.md:8542` — no such `Reason` exists in `pkg/errcode/codes_botplatform.go` and no path emits it.
