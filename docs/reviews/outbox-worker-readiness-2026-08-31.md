...
Statement coverage is 36.9% — far below the 60% line this audit scores as `critical` (CLAUDE.md §4 requires 80%) — and the uncovered mass is dominated by the durable-retry wiring (message disposition, dedup publish, lane pump), alongside the untestable `main()` startup and shutdown branches below, even though the handler-level tests that do exist are genuinely good.
...
|---|-----|--------|-----|----------|-----|
| 1 | `high` | **Fail fast when `ALL_SITE_IDS` yields no peers** | Integration / Arch / Quality | `envDefault:""` at `main.go:39`; warn-and-continue `:123-127`; health off the NATS check alone `:157-159`; **the service's own compose collapses to this** at `deploy/docker-compose.yml:18` | Found by three of six experts. Zero peers means **zero consumers**: the sole OUTBOX owner runs as a no-op while every federated event accumulates until retention deletes it — and the pod reports healthy throughout. Repo-consistent with `broadcast-worker/main.go:358-363`, but there it disables a feature; **here it disables the service's entire function.** |
| 2 | `high` | **Give each peer its own worker sub-budget** | Performance / Arch | one global `sem` at `main.go:114`, handed to every lane at `:140`, `:142`; 3s publish timeout `handler.go:31`, `:47`; the isolation argued at `main.go:118-124`, `:236-244` | The per-destination consumer split exists **specifically** so a down peer's parked forwards fill only their own ack-pending budget. That holds server-side and is then given back client-side: **one dead peer's burst can occupy all 100 slots**, each for the full publish timeout, delaying healthy peers. A per-peer sub-budget restores the property the design already argues for. |
| 3 | `high` | **Drain the ordered lane on shutdown** | Quality / Arch / Perf | `ocons.Consume(ctx, process)` tracked by nothing (`main.go:149`); `cc.Stop()` at `:173`; `wg.Wait()` at `:179` counts only `drainPool` (`:203-231`); `Drain()`/`Closed()` exposed at `o11y@v0.11.0/nats/jetstream.go:248-252`, neither used | Found by four of six experts. `Stop()` "discards buffered messages" and returns immediately, so **a membership forward in flight races `nc.Drain()` and fails.** Not data loss — `MaxDeliver=-1` redelivers — but it is an avoidable failed forward on every rolling restart, on the lane whose whole point is ordering. |
| 4 | `critical` | **Lift `process`, the publish closure and the lane loop out of `main()` and test them** | Test coverage / Maint | 36.9% (141 stmts); `main.go` holds 118 of them, 89 uncovered; `process` inlined at `:103-108`; publish closure `:90-96`; lane loop `:130-155` | Below the 60% critical line, and the uncovered code is **exactly the durable-retry machinery**: `jobguard.Run`'s Ack-on-panic poison drop, `logctx.ConsumeContext`, `jsretry.Settle`'s permanent→Ack / transient→NakWithDelay split, and **the only site that sets `jetstream.WithMsgID` — the entire cross-site idempotency guarantee.** The integration suite *reimplements* that closure rather than calling it (`integration_test.go:76-81`), so the production one is verified nowhere. Tellingly, `drainpool_test.go` and `consumer_config_test.go` already name files that do not exist. |
| 5 | `high` | **Set an explicit `Duplicates` window on INBOX** | Integration / Perf | neither `bootstrapStreams` (`bootstrap.go:43-49`) nor `pkg/stream.Inbox` (`stream.go:66-74`) sets it; server default 2m; `jsretry.DefaultBackoff` tail is 10m (`pkg/jsretry/jsretry.go:52-58`) under `MaxDeliver=-1` | `handler.go:56` states "Redelivery is idempotent (DedupID)". **That is true only inside the dedup window**, and the 3rd and 4th retries (2m, 10m) land outside it — so a 3s publish timeout on a forward the server actually accepted becomes a **duplicate INBOX event**. Set `Duplicates` to comfortably exceed the backoff tail. |
| 6 | `high` | **Bound the ordered lane's recovery latency** | Performance | `MaxAckPending=1` at `main.go:279`; `DefaultBackoff` at `:106` | One parked message blocks the **entire per-destination membership lane**, and the backoff walks to a 10-minute tail. After a peer heals, `member_added`/`member_removed`/`room_renamed` sit un-retried for up to 10 minutes and every subsequent event queues behind them. Recovery is bounded by the backoff schedule, **not by the outage.** A capped tail (or a peer-health probe that resets it) fixes it. |
| 7 | `medium` | **Prove `DedupID` actually dedups** | Test coverage | `integration_test.go:348-354` asserts arrival order and `State.Msgs == 2`, never re-forwards the same DedupID | The claim at `handler.go:56` is the service's central idempotency guarantee and **no test verifies it.** Re-forward the same DedupID and assert a single stored INBOX message — which also pins item 5. |
| 8 | `medium` | **Make a dead pump fatal, or restart it** | Code quality | non-`ErrMsgIteratorClosed` error kills that peer's lane at `main.go:208-216`; health stays green `:157-159` | A single iterator error **permanently kills one peer's concurrent pump** while the process stays up and healthy — the same silent-partial-failure shape as item 1, scoped to one destination. |
| 9 | `medium` | **Align prefetch with concurrency** | Performance | `2*MaxWorkers` per lane at `main.go:136`; global pool of `MaxWorkers`; `AckWait` 30s (`pkg/stream/consumer.go:19`) | With P peers, `2·P·MaxWorkers` already-delivered messages queue against `MaxWorkers` of concurrency. At ~3s per stalled forward, **a backlog only a few hundred deep re-delivers server-side** and the same event is forwarded twice — which then depends on item 5 to be harmless. |
| 10 | `medium` | **Unify the two undeliverable-event idioms; move the helpers to the files their tests name** | Maintainability | `errcode.Permanent` at `handler.go:41`, `:45` vs hand-rolled `slog.WarnContext` + `return nil` at `:50-55`; `drainPool` `main.go:203-228`, the three `build*ConsumerConfig` `:242-281`, `federationPeers` `:286-299` | A missing `Envelope`/`DedupID` is **equally unrecoverable** but bypasses `jsretry`'s poison accounting, so the two classes look different in logs and metrics for no stated reason. And moving the four helpers out of `main.go` is the same edit as item 4. |

**Documentation fix.** CLAUDE.md's OUTBOX producer list omits `bot-room-service`, which publishes `member_added`/`member_removed` through `outbox.Publish` (`bot-room-service/handler.go:676`, `:690`). That is drift in binding documentation about the federation contract this service owns, and it belongs in the same PR as any of the above.

**Also worth doing.** Replace the `time.Sleep(300ms)` synchronisation before the FIFO lane's `AckFloor`/`NumAckPending` assertions (`integration_test.go:335`) — CLAUDE.md forbids sleep-as-synchronisation, and a loaded CI runner will make that test flake. And trim `main.go`'s comment volume: 72 of 300 lines, with the per-destination-isolation rationale stated at `:117-122`, restated at `:230-241`, and stated a third time in CLAUDE.md §6 — three copies that will drift independently.
