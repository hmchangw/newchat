...
| message-worker retry | Transient handler errors use `jsretry.DefaultBackoff`. **`MaxDeliver` is 17, not the repo default 6** — `message-worker/main.go:360` wraps its settings in `stream.WithOutageRetryBudget`, which raises the cap until the schedule spans `stream.OutageRetryWindow` = 1 hour | A long outage exhausts the durable, but only after roughly two hours (17 deliveries = 1 + 16 redeliveries), not minutes. Handler-error exhaustion **is** counted today by `chat_nats_terminal_failures_total{reason="max_deliver"}`; advisories add the un-acked paths (crash, hang, `AckWait`) and attribution, not basic detection |
