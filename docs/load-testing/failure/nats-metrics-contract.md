...
|---|---|---|---|
| `chat.nats.consumer.loop.up` / `chat_nats_consumer_loop_up` | gauge | `service_name,site,stream,consumer` | 1 only while a live iterator/loop can receive messages |
| `chat.nats.consumer.messages` / `chat_nats_consumer_messages_total` | counter | `service_name,site,stream,consumer,event_type,outcome` | One terminal application disposition per delivery attempt |
...
| `chat.nats.terminal.failures` / `chat_nats_terminal_failures_total` | counter | `service_name,site,stream,consumer,event_type,reason` | Work that will receive no further application attempt |
| `chat.nats.requests` / `chat_nats_requests_total` | counter | `service_name,site,operation,outcome` | Request/reply result classified by bounded outcome |
...
| OTel instrument / Prometheus family | Type | Labels | Semantics |
|---|---|---|---|
| `chat.nats.client.connected` / `chat_nats_client_connected` | gauge | resource only | 1 while the process holds a live broker connection |
| `chat.nats.client.connection.events` / `chat_nats_client_connection_events_total` | counter | `event` | Lifecycle transitions: `connected`, `disconnected`, `reconnected`, `closed`, `async_error` |
