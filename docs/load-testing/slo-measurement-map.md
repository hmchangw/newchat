...
|---|---|---|---|---|
| **1a** persist | `message_worker_persistence_total{result="success"}` | `message_gatekeeper_messages_total{result="accepted"}` | ⚠️ **approximate indicator now** — computable, but both sides count attempts, so a redelivery can push it **over 100%**. Not a hard gate | Logical-outcome dedup (P7), **or** loadgen authoritative read-back. **Not advisories** — they prove the broker stopped delivering, not that the numerator stopped double-counting; see the runbook §5 |
| **1b** channel enqueue | per-target, wrong unit | no `broadcast_path` slice | ❌ | a `broadcast_path`-labelled denominator at the **gatekeeper** publish site (upstream, so a dead worker drops the ratio instead of vanishing) plus a per-message enqueue counter on broadcast-worker — [`p2-implementation-task.md`](p2-implementation-task.md) §3 |
