...
| **Resilience / fault injection** | a dependency dies/slows — what degrades? | fixed load + injected fault | ⏳ later | [`../failure/overview.md`](../failure/overview.md) |
...
width — isolated keyspace) · dependency fault injection (→ [`../failure/overview.md`](../failure/overview.md))
...
- [`../failure/overview.md`](../failure/overview.md) — the failure-testing program: dependency criticality matrix, fault campaigns, and recovery reconciliation. [`resilience-test-plan.md`](resilience-test-plan.md) is a pointer into it.
