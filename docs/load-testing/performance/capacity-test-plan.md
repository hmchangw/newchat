...
| **Valkey** | ⚠️ conditional | Self-hosted, best-effort cache, not correctness-critical, so the Elasticsearch blockers above do **not** apply to it. Before a result is accepted it needs a run-scoped key namespace or ownership marker, an expiry, and cleanup verified after teardown - the evidence-retention window is not a cleanup mechanism (§7). Until those exist, a Valkey capacity run evicts pre-existing keys and the refill lands as extra Mongo and Elasticsearch load, which contaminates neighbouring measurements |
