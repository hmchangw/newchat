...
| **G4** ⬆ | **P2 J1 counters — narrowed** (gatekeeper `messages_canonical_published_total`; broadcast-worker `broadcast_canonical_consumed_total{broadcast_path}`, `broadcast_channel_enqueue_total{result}`, `broadcast_channel_enqueue_age_seconds` from the JetStream metadata timestamp, `_age_invalid_total{reason}`). Implementable brief: [`p2-implementation-task.md`](p2-implementation-task.md) | Hard-gating SLO-1a/1b/2 in B1. Until then every J1 verdict is loadgen-L1 observational | App |
