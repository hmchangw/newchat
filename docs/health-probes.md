...
Rolling that ownership out: the previous `message-worker` repaired
`thread_rooms.parentMessageId` itself, so during a rolling deploy an old
replica and the new `room-service` are both repairers. A repairer only drops
a conflicting index, so this is inert unless that index is non-unique at
deploy time; when it is, roll `message-worker` fully before `room-service`
(or repair the index by hand first), because an old replica's stale
drop-and-recreate can briefly remove the index a new replica has already
confirmed.

A gate confirms once and then costs an atomic load, so it does **not** notice
an index dropped afterwards — by that stale repairer, or by an operator. That
is why a conflicting index is repaired as a deliberate migration before these
workers roll, not concurrently with them: a gate is a startup-ordering
guard, not a continuous constraint monitor. MongoDB enforces the constraint
itself once the index exists, so the exposure is only the window in which the
index is absent while a gate still believes it is there.
...
window must be at least `stream.OutageRetryWindow`, or a redelivery after the
window republishes push batches that were already accepted. `notification-worker`'s
bootstrap sets it in dev; in production the stream is ops-owned, so the worker
reads the deployed window at startup and refuses to start while it is shorter.
Raise it on the stream before rolling the worker out. Rejected credentials and
a cancelled startup context are the two ping failures that stay fatal. A pod
that started degraded because MongoDB was unreachable may hold bad credentials
too; the first operation after recovery is refused, and `mongoutil` ends the
process with a nonzero exit on that first post-start rejection, so the
