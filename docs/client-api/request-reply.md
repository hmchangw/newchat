...
| `updatedWithinDays` | number | no | `rooms`-type only. Filters to rooms whose `lastUserMsgAt` (falling back to `lastMsgAt` for rooms that predate it) is within N days. Non-negative. |
