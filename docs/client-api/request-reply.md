...
| `GET /v1/admin/rooms` | synchronous HTTP | List the rooms homed at this site, paged; each row projected to `id`/`name`/`type`/`userCount`/`restricted`/`externalAccess`, plus a server-derived `onDuty` (both flags set) (§9.18). |
