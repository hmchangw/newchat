...
| `GET /v1/admin/rooms` | synchronous HTTP | List this deployment's rooms (no `siteId` predicate — each site has its own database), paged and optionally narrowed by a `q` holding one exact room id; each row projected to `id`/`name`/`type`/`userCount`/`restricted`/`externalAccess`, plus a server-derived `onDuty` (both flags set) (§9.18). |
| `GET /v1/admin/rooms/:roomId/members` | synchronous HTTP | List every account subscribed to the room, unpaged. Same subscriptions the duty toggle validates `ownerAccount` against, and unlike the client-facing `member.list` RPC it needs no membership (§9.19). |
| `POST /v1/admin/rooms/:roomId/onduty` | synchronous HTTP | Toggle a channel's on-duty state: maps the boolean onto `restricted` + `externalAccess` via room-service's restrict RPC, with `ownerAccount` required when turning on. Emits a `room_restricted` room event; no system message, so nothing is displayed. Writes one `admin_audit` row (`room.onduty.set` / `room.onduty.unset`) naming the room and the designated owner (§9.12). |
...
followed threads (at most +1 per room; muted excluded). The thread check is
subscription-local: a room counts when its sub's `threadUnread` is non-empty — no
per-site thread RPC.
...
state and, concurrently, `$unset` `threadUnread` on every one of that user's
subscriptions that still has unread threads (no alert coupling); each remote site
converges the user's home replica via one `thread_read_all` inbox event.
