...
Note: in DM/botDM rooms hidden thread-reply deletes arrive on the same
`chat.user.{recipient}.event.room` subject as visible deletes, and the payload
carries no thread marker — distinguish them from your local cache: if the deleted
`messageId` is a thread reply not shown in the room, keep the current preview;
otherwise apply the visible-delete rule above.
