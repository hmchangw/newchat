...
| `message_worker_history_write_failures_total` | counter | message-worker | on first failed history write | none — the driver sees a query fail, not that the site's history is now incomplete | the leading indicator for destruction: a rising `class="request"` rate is a migration-induced wave, visible *before* the retry window elapses and anything is dropped. Labelled by error class only (`infra`/`request`) |
| `message_worker_history_dropped_total` | counter | message-worker | on first drop | none | **any non-zero value is data destruction** and alerts as such. Labelled by CQL code, never error text |
| `message_worker_history_drop_suppressed_total` | counter | message-worker | on first withheld drop | none — the brake is application state | whether releasing the kill switch is safe: `reason="rate_limited"` means the unattended cap is the only thing holding the feed together, `reason="disabled"` means the switch is still engaged. Without it the brake is invisible |
| `message_worker_history_degraded` | gauge | message-worker | at startup | none — the marker is a Mongo document this service owns | degraded-marker state and duration. A marker stuck set is one of the two things that keep a retry loop running, so it is read beside the ack-floor age below, not alone |
| `message_worker_consumer_num_pending` | gauge | message-worker | at startup | **yes, exactly**: `jetstream_consumer_num_pending` from the NATS exporter is the same number | consumer-backlog alert. Emitted anyway because the ack-floor age below needs a `ConsumerInfo` poll the exporter cannot substitute for, and the depth rides along in that same sample — so the alert reads one process's consistent view instead of joining two label schemes. If that alert is ever built on the exporter series, delete this row and the gauge |
| `message_worker_ack_floor_age_seconds` | gauge | message-worker | at startup | none — the exporter publishes the ack floor's *sequence*, not its timestamp, so age is not derivable from it | the stuck-retry-loop alert. Retry-with-no-cap trades loud loss for quiet lag, and oldest-unacked age is the lag; the repo had no such metric anywhere before this |
...
| `bot_control_bypassed_total` | counter | botplatform-service | on the first bot request admitted without a Valkey-backed control | none — Valkey exporters show the outage, not which of our controls we chose to skip during it | **the alert this family exists for.** The controls fail open, so a Valkey outage leaves bot traffic unthrottled and undeduplicated while every other bot signal stays green; this is the only durable evidence. The `control` label (`rate_limit_caller`, `rate_limit_global`, `idempotency`) says which one. Deliberately independent of the log, which throttles to one WARN per 30s |
| `room_key_absent_errors_total` | counter | room-worker | on first occurrence | none | distinguishes "no key" from "store broken" |
| `bot_requests_shed_total` | counter | botplatform-service | on the first bot request rejected by the admission cap | partial and ambiguous: the o11y Gin middleware counts 429s via `http.server.request.duration{http.response.status_code="429"}`, but on the bot routes `botRateLimit` also answers 429, so the platform metric cannot separate "cap rejected it before the session lookup" from "caller exceeded its rate". On the login routes that ambiguity does not exist — nothing else there returns 429 — so those caps deliberately emit no counter and rely on the platform metric | **admission-cap health for the pre-auth control.** The cap runs ahead of `requireBot` because an invalid token misses the positive-only session cache and reaches MongoDB on every attempt; this counter is the evidence it is doing that. Non-zero in steady state means legitimate bot traffic is being shed and `BOT_MAX_CONCURRENCY` is too low. Unlabelled — the cap is process-wide, so there is no bounded dimension to slice it by |
