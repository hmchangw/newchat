...
| `loadgen_mongo_up`, `loadgen_mongo_probe_timestamp_seconds` | loadgen soak | Loadgen's latest bounded primary Ping and its completion time. These identify harness-side Mongo blindness; they are not service readiness or proof that every command is authorized | Existing |
