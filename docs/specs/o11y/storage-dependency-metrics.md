...
| `loadgen_failure_invalidations_total{reason}` | loadgen soak | Ledger capacity or WAL failures that invalidate evidence. `reconcile_capacity` is recorded once at startup when the run begins below the reconciliation floor: nothing has failed yet, but every message will expire unverified for want of a claim, so the window is inconclusive from the first second rather than degraded partway through | Existing |
