...
| `loadgen_failure_invalidations_total{reason}` | loadgen soak | Ledger capacity or WAL failures that invalidate evidence. `reconcile_capacity` invalidates the ledger once at startup when the run begins below the reconciliation floor: nothing has failed yet, but every message will expire unverified for want of a claim, so the window is inconclusive from the first second rather than degraded partway through. Every reason here comes from a ledger invalidation, so the counter and the run's own `InvalidReason` always agree | Existing |
