...
// testPreviewLookback mirrors the MESSAGE_PREVIEW_LOOKBACK_ROWS default.
...
// Budget boundary, inclusive side: a survivor at the lookback edge (testPreviewLookback-1
// tombstones above it) is still found — the budget covers the tombstones plus the survivor.
...
// Budget boundary, exclusive side: testPreviewLookback tombstones exhaust the lookback
// before the survivor — empty preview (the next real message self-heals it).
...
// messages_by_room holds only main-timeline rows (non-TShow thread replies live in
// thread_messages_by_thread), so a thread-only reply must never surface as last message.
...
// A deleted tail longer than the row-scan budget aborts the walk with (nil, nil) —
// matching rooms.get's ponytail precedent — not draining deep partitions per delete.
...
// A qualifying row OLDER than floor inside the floor bucket must not leak out:
// DESC order proves nothing newer survives, so the walk answers "no preview".
...
// The pointer tracks the newest surviving row of ANY type (system included),
// while the preview skips system types — one walk, two answers.
...
// re-pointing lastMsgId at the just-deleted message undoes the walk's rewind.
