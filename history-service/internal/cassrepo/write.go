...
func (r *Repository) countAndSetParentTcount(ctx context.Context, msg *models.Message) (*int, *time.Time, error) {
	...
	parent := threadcount.NewParent(msg.ThreadParentID, msg.RoomID, msg.ThreadRoomID,
		*msg.ThreadParentCreatedAt, r.bucket)
	res, err := threadcount.Maintain(ctx, r.session, parent, r.threadPolicy, -1, nil, false)
	if err != nil {
