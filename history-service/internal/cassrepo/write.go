...
func (r *Repository) readEncryptedFields(ctx context.Context, msg *models.Message) (atrest.EncryptedFields, *cassmodel.QuotedParentMessage, *cassmodel.ForwardedMessage, error) {
	...
	err := r.session.Query(
		`SELECT enc_payload, enc_meta, msg, attachments, card, card_action, quoted_parent_message, forwarded_message
		 FROM messages_by_id WHERE message_id = ?`,
		msg.MessageID,
	).WithContext(ctx).Scan(&encPayload, &encMeta, &msgText, &attachments, &card, &cardAction, &quoted, &forwarded)
	if err != nil {
		if errors.Is(err, gocql.ErrNotFound) {
			return atrest.EncryptedFields{}, nil, nil, ErrMessageNotFound
		}
		return atrest.EncryptedFields{}, nil, nil, fmt.Errorf("read existing fields for message %s: %w", msg.MessageID, err)
	}
...
// whereArgs...); encQ binds (encPayload, encMeta, quotedMeta[, forwardedMeta],
// editedAt, editedAt, whereArgs...) — forwardedMeta is bound only when
// withForwarded is true, since thread_messages_by_thread has no
// forwarded_message column.
...
func (r *Repository) editInMessagesByID(ctx context.Context, msg *models.Message, ep editPayload, editedAt time.Time) error {
	return r.editOne(ctx, editMsgByID, editMsgByIDEncrypted, ep, editedAt, true, msg.MessageID)
}
...
func (r *Repository) editInMessagesByRoom(ctx context.Context, msg *models.Message, ep editPayload, editedAt time.Time) error {
	...
	return r.editOne(ctx, editMsgByRoom, editMsgByRoomEncrypted, ep, editedAt, true, msg.RoomID, b, msg.CreatedAt, msg.MessageID)
}
...
func (r *Repository) editInThreadMessagesByThread(ctx context.Context, msg *models.Message, ep editPayload, editedAt time.Time) error {
	return r.editOne(ctx, editThreadMsg, editThreadMsgEncrypted, ep, editedAt, false, msg.ThreadRoomID, msg.CreatedAt, msg.MessageID)
}
...
func (r *Repository) editInPinnedMessagesByRoom(ctx context.Context, msg *models.Message, ep editPayload, editedAt time.Time) error {
	return r.editOne(ctx, editPinnedMsg, editPinnedMsgEncrypted, ep, editedAt, true, msg.RoomID, *msg.PinnedAt, msg.MessageID)
}
