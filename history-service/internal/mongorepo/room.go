...
func (r *RoomRepo) applyPreviewFields(ctx context.Context, roomID, what string, fields bson.M) (bool, error) {
	if r.cipher == nil {
		return false, nil
	}
	pipeline := mongo.Pipeline{{{Key: "$set", Value: fields}}}
	res, err := r.rooms.Raw().UpdateOne(ctx, bson.M{"_id": roomID}, pipeline)
	if err != nil {
...
func (r *RoomRepo) SetPreviewMessage(ctx context.Context, roomID string, pvw model.PreviewMessage, forMsgID string, asOf int64) error {
	...
	_, err = r.applyPreviewFields(ctx, roomID, "store room preview", preview.GuardedSetFields(sealed, asOf))
	return err
}
...
func (r *RoomRepo) InvalidatePreviewKey(ctx context.Context, roomID, msgID string) error {
	...
	_, err := r.applyPreviewFields(ctx, roomID, "invalidate room preview key",
