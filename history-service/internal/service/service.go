...
type RoomRepository interface {
	...
	// SetPreviewMessage seals and stores a walk-resolved preview on the room doc,
	// guarded by asOf so it fills empty docs but never regresses a newer write.
	// forMsgID is the freshness key: the newest message id the walk OBSERVED in
	// Cassandra, never the lastMsgId read out of Mongo (see the repo doc).
	// Callers treat errors as best-effort (log, never fail the read).
	SetPreviewMessage(ctx context.Context, roomID string, pvw models.PreviewMessage, forMsgID string, asOf int64) error
	// ClearPreview removes the stored preview under the same watermark guard,
	// for a mutation that leaves the room with no eligible message at all.
