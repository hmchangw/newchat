...
// newSubL2 builds the same tier-backed access-check adapter main.go wires, so
// the test exercises the real chain. valkey is nil here (fail-open to the
// loader) — the point is that the tier runs the breaker, not that a Valkey
// server is present.
func newSubL2(breaker *circuitbreaker.Breaker, loader subauthcache.Loader) subAuthReadThrough {
	...
	tier := subauthcache.NewTierWithLoader(nil, 90*time.Minute, cachemetrics.For("subauth", "l2"), guarded)
	return func(ctx context.Context, account, roomID string) (*time.Time, bool, error) {
		auth, subscribed, err := tier.Resolve(ctx, roomID, account)
