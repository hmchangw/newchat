...
func run() error {
	...
	// Armed before the lanes bind: a dying lane raises SIGTERM on this process
	// through its guard, and a signal raised before the handler exists is fatal.
	sig := shutdown.Signals()

	// The lanes carry one loop guard each, so a dead lane fails readiness by
	// name rather than hiding behind its live sibling.
