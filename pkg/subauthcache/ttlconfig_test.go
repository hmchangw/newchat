...
// Mounted under an envPrefix, as both consumers do, so the assertions are about
// the prefixed name an operator actually sets.
type ttlHolder struct {
	Cfg TTLConfig `envPrefix:"GATEKEEPER_"`
}
...
func TestTTLConfig_EnvName(t *testing.T) {
	t.Setenv("GATEKEEPER_SUB_L2_TTL", "45m")
...
func TestTTLConfig_Default(t *testing.T) {
	testutil.UnsetEnv(t, "GATEKEEPER_SUB_L2_TTL")
