...
//
// context.Canceled is ALWAYS exempt, without being listed: cancellation only
// happens when someone calls cancel(), so it is evidence about the caller, not
// the downstream — the call may not even have been issued. Counting it would
// let a burst of client disconnects fence a database that is perfectly healthy.
//
// context.DeadlineExceeded is deliberately NOT exempt, and the asymmetry is the
// important part. The MongoDB driver bounds every operation's server selection
// with context.WithTimeout(parent, MONGO_SERVER_SELECTION_TIMEOUT) — see
// internal/csot.WithServerSelectionTimeout, applied at x/mongo/driver's
// operation.go — and on expiry returns a topology.ServerSelectionError wrapping
// that context's Err(). So an unreachable MongoDB reports itself as an error
// matching context.DeadlineExceeded, and it is the single most important error
// for this breaker to count: exempting it leaves the breaker permanently closed
// against the exact outage it exists to fence, and every caller keeps paying
// the full timeout.
//
// A caller whose own deadline is tighter than the downstream's is therefore
// counted too. That is the safe direction to be wrong in: the cost is a breaker
// that opens on a healthy database and re-closes on its next probe, against a
// breaker that never opens at all. Cancellation, which is unambiguous, covers
// the disconnect-storm case that actually motivated the exemption.
//
