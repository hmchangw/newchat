...
// A CANCELLED caller says nothing about the downstream's health: the call may
// never have been issued, and a burst of client disconnects would otherwise
// spend the shared failure budget and fence a database that is perfectly
// healthy. Every predicate needs this, so FailureExcept owns it rather than
// five call sites each remembering to pass it.
func TestFailureExcept_CancellationIsNeverABreakerFailure(t *testing.T) {
	notFound := errors.New("not found")
	tests := []struct {
		name string
		err  error
	}{
		{"cancelled caller", context.Canceled},
		{"wrapped cancellation", fmt.Errorf("find room: %w", context.Canceled)},
		{"server selection abandoned by the caller", serverSelectionErr{wrapped: context.Canceled}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.False(t, FailureExcept(notFound)(tt.err),
				"a cancelled caller says nothing about the downstream's health")
		})
	}
}

// serverSelectionErr mirrors what the MongoDB driver returns when server
// selection gives up: topology.ServerSelectionError, whose Unwrap exposes the
// context error of an INTERNAL deadline. The driver builds that deadline with
// context.WithTimeout(parent, MONGO_SERVER_SELECTION_TIMEOUT)
// (internal/csot.WithServerSelectionTimeout), so an unreachable MongoDB
// surfaces as an error matching context.DeadlineExceeded while the caller is
// still waiting. Reproduced here rather than imported: the real type lives
// under the driver's x/ tree, and this package must not depend on MongoDB.
type serverSelectionErr struct{ wrapped error }

func (e serverSelectionErr) Error() string { return "server selection error: " + e.wrapped.Error() }
func (e serverSelectionErr) Unwrap() error { return e.wrapped }

// The single most important error for the breaker to count. Exempting
// DeadlineExceeded wholesale leaves the breaker permanently closed against the
// exact outage it exists to fence.
func TestFailureExcept_ServerSelectionTimeoutIsABreakerFailure(t *testing.T) {
	notFound := errors.New("not found")
	tests := []struct {
		name string
		err  error
		...
		{"wrapped deadline", fmt.Errorf("get subscription: %w", context.DeadlineExceeded)},
		{"driver server selection timeout", serverSelectionErr{wrapped: context.DeadlineExceeded}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.True(t, FailureExcept(notFound)(tt.err),
				"a server-selection timeout is how an unreachable MongoDB reports itself")
		})
	}
}

// The outage the whole design exists for: MongoDB stops answering, every call
// times out in server selection, and the breaker must fence it so the next
// caller fast-fails instead of paying the timeout again.
func TestFailureExcept_ServerSelectionTimeoutBurstOpensTheBreaker(t *testing.T) {
	b := New(5, time.Minute, WithFailurePredicate(FailureExcept()))

	down := serverSelectionErr{wrapped: context.DeadlineExceeded}
	for range 5 {
		require.Error(t, b.Do(func() error { return down }))
	}
	assert.Equal(t, StateOpen, b.State(),
		"an unreachable MongoDB must fence, or every caller keeps paying the full server-selection timeout")
}

// The exemption must hold under load: a run of cancellations cannot open the
// breaker, while a real infrastructure error still does.
func TestFailureExcept_CancellationBurstLeavesTheBreakerClosed(t *testing.T) {
	b := New(2, time.Minute, WithFailurePredicate(FailureExcept()))

	for range 20 {
		require.ErrorIs(t, b.Do(func() error { return context.Canceled }), context.Canceled)
	}
	assert.Equal(t, StateClosed, b.State(), "cancelled callers must not fence a healthy downstream")
