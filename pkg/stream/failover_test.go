...
func (f *fakeFailoverJS) create(_ context.Context, cfg *jetstream.StreamConfig) error {
	f.created = append(f.created, cfg.Name)
	return f.createErr
}

func (f *fakeFailoverJS) info(_ context.Context, name string) (*jetstream.StreamInfo, error) {
...
func TestEnsureFailoverStream_BootstrapCreatesWithoutAsserting(t *testing.T) {
	js := &fakeFailoverJS{}
	cfg := MessagesCanonicalFailover("site-a")

	err := ensureFailoverStream(context.Background(), js, cfg, true, "site-b")
...
func TestEnsureFailoverStream_ProductionVerifiesAndAsserts(t *testing.T) {
	cfg := MessagesCanonicalFailover("site-a")
	js := &fakeFailoverJS{streamInfo: &jetstream.StreamInfo{
		Config:  jetstream.StreamConfig{Name: cfg.Name},
		Cluster: &jetstream.ClusterInfo{Name: "site-b"},
	}}

	err := ensureFailoverStream(context.Background(), js, cfg, false, "site-b")
