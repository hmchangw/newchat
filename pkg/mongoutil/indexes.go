...
func warnMissingIndexes(ctx context.Context, coll *mongo.Collection, requireUnique bool, names ...string) {
	...
	warnIndexDependencies(ctx, coll.Name(), have, requireUnique, names...)
}

// warnIndexDependencies warns per absent name and, only when required, per present-but-non-unique one.
func warnIndexDependencies(ctx context.Context, collName string, have map[string]bool, requireUnique bool, names ...string) {
	for _, n := range names {
		unique, ok := have[n]
		switch {
		case !ok:
			slog.WarnContext(ctx, "mongo: depended-on index missing; its owner service must create it",
				"collection", collName, "index", n)
		case requireUnique && !unique:
