...
// terminateProcess ends a degraded pod whose credentials were rejected after recovery. A plain nonzero exit,
// as a healthy start with the same credentials makes: a graceful self-signal would return 0 through main and
// present the misconfiguration to a restart-on-failure supervisor as success. Consumers are at-least-once.
var terminateProcess = func() { os.Exit(1) }
...
func startCredentialWatch(p *credentialProbe, uri string) (stop func()) {
	...
	// Waits for the watcher to exit, so a rejection recorded after Disconnect cannot still end the
	// process — a stop that only signals leaves the two racing. In production the rejection branch
	// exits the process, so there is nothing for this to wait behind.
	return sync.OnceFunc(func() { close(done); <-exited })
}
