...
// stop must not return until the watcher is gone, so a rejection recorded after Disconnect
// cannot still end the process. Repeated because a stop that only signals leaves the watcher
// racing the rejection: it would pass about half the time and prove nothing.
func TestStartCredentialWatch_StopSilencesALaterRejection(t *testing.T) {
	terminated := make(chan struct{}, 1)
	orig := terminateProcess
	terminateProcess = func() { terminated <- struct{}{} }
	t.Cleanup(func() { terminateProcess = orig })
	...
	for range 20 {
		p := newCredentialProbe()
		stop := startCredentialWatch(p, unreachableMongo)
		stop()
		close(p.rejected) // the rejection a stopped watch must ignore
		select {
		case <-terminated:
			t.Fatal("a stopped watch must not end the process")
		case <-time.After(20 * time.Millisecond):
		}
	}
}

// Disconnect must stop the watch AND forget it, or a long-lived process accumulates one
// dead entry per reconnect.
func TestConnect_DegradedStart_DisconnectStopsTheCredentialWatch(t *testing.T) {
	...
	_, watched := credentialWatches.Load(client)
	require.True(t, watched, "a degraded client must be watched")

	Disconnect(context.Background(), client)

	_, watched = credentialWatches.Load(client)
	assert.False(t, watched, "Disconnect must stop and forget the watch")
}
