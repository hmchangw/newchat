...
// Done reports whether the watcher goroutine has exited — because the bind
// landed, or because the context was cancelled. Exported for the test that
// pins the termination path: a watcher that outlived its context would be a
// goroutine leak per pod, invisible until it accumulated.
...
// PageBudget is the byte ceiling a reply on conn must fit under, or the
// disabled zero Budget when page trimming is switched off. Shared because a
// service with a failover lane sizes one budget per connection — the two
// brokers can advertise different max_payloads — and a hand-copied version
// that forgets MaxPayload's not-yet-dialing floor sizes against zero.
func PageBudget(trimming bool, maxResponseBytes int64, conn *o11ynats.Conn) pagefit.Budget {
	if !trimming {
		return pagefit.Budget{}
	}
	return pagefit.Resolve(maxResponseBytes, MaxPayload(conn), pagefit.DefaultReserve)
}
