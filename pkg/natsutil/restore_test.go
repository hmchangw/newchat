...
func TestRestoreTracker_ZeroUntilMarked(t *testing.T) {
	var tr natsutil.ReconnectTracker
	assert.True(t, tr.LastReconnectAt().IsZero(),
...
func TestRestoreTracker_MarkAndRead(t *testing.T) {
	...
	tr.MarkReconnect(at)
	assert.Equal(t, at, tr.LastReconnectAt())
}
...
func TestRestoreTracker_LatestWins(t *testing.T) {
	...
	tr.MarkReconnect(first)
	tr.MarkReconnect(second)
	assert.Equal(t, second, tr.LastReconnectAt(),
...
func TestRestoreTracker_EarlierStampIsIgnored(t *testing.T) {
	...
	tr.MarkReconnect(later)
	tr.MarkReconnect(later.Add(-time.Hour))
	assert.Equal(t, later, tr.LastReconnectAt())
}
...
func TestRestoreTracker_ConcurrentAccess(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		for i := 0; i < 1000; i++ {
			tr.MarkReconnect(time.Now())
		}
	}()
	for i := 0; i < 1000; i++ {
		_ = tr.LastReconnectAt()
	}
	wg.Wait()
}

func TestTrackRestores_NilConnYieldsAnInertTracker(t *testing.T) {
	tr := natsutil.TrackReconnects(context.Background(), nil, false)
	require.NotNil(t, tr)
	assert.True(t, tr.LastReconnectAt().IsZero())
}
...
func TestTrackRestores_StopsOnContextCancel(t *testing.T) {
	...
	tr := natsutil.TrackReconnects(ctx, conn, false)
	require.NotNil(t, tr)
	assert.True(t, tr.LastReconnectAt().IsZero(), "a connection that never dropped is not restored")
...
func TestTrackRestores_StampsAReconnect(t *testing.T) {
	...
	t.Cleanup(cancel)
	tr := natsutil.TrackReconnects(ctx, conn, false)
	require.True(t, tr.LastReconnectAt().IsZero())

	ns.restart(t)

	require.Eventually(t, func() bool { return !tr.LastReconnectAt().IsZero() },
...
func TestTrackRestores_AssumeOutageOpensTheWindowAtStartup(t *testing.T) {
	...
	assert.False(t, tr.LastReconnectAt().IsZero())
	assert.WithinDuration(t, time.Now(), tr.LastReconnectAt(), 5*time.Second)
}
