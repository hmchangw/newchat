package natsutil

import (
	"context"
	"log/slog"
	"sync/atomic"
	"time"

	o11ynats "github.com/flywindy/o11y/nats"
	"github.com/nats-io/nats.go"
)

// ReconnectTracker records when a connection last came back after being lost.
//
// Room-event routing needs this: for a while after home reconnects, publishers
// must emit to BOTH subject roots, because clients revert to their home site on
// a slower clock than servers do (see subject.LaneRouter.Mode). The zero value
// is valid and reads as "never lost", keeping a service that has had no outage
// out of the grace window.
//
// The stamp is an atomic UnixNano rather than a mutex-guarded time.Time because
// it is read once per room-event publish from up to MAX_WORKERS goroutines,
// while writes happen only on a reconnect.
type ReconnectTracker struct {
	nanos atomic.Int64
}

// MarkReconnect stamps a restoration. A later stamp replaces an earlier one, so a
// second outage restarts the grace window rather than inheriting the first; an
// earlier stamp is ignored, so an out-of-order mark cannot cut a window short.
func (t *ReconnectTracker) MarkReconnect(at time.Time) {
	n := at.UnixNano()
	for {
		cur := t.nanos.Load()
		if n <= cur {
			return
		}
		if t.nanos.CompareAndSwap(cur, n) {
			return
		}
	}
}

// LastReconnectAt returns the last reconnect, or the zero time if the
// connection has never been lost.
func (t *ReconnectTracker) LastReconnectAt() time.Time {
	n := t.nanos.Load()
	if n == 0 {
		return time.Time{}
	}
	return time.Unix(0, n).UTC()
}

// TrackReconnects watches conn and stamps a tracker every time it reaches
// CONNECTED after not being connected: a reconnect after a drop, and — on a
// lazily dialed home (BuddyDialer.ConnectHome) — the first connect of a pod
// that booted during the outage, which IS the recovery from where it stands.
//
// assumeOutage stamps the tracker at startup as well. A publisher that restarts
// after home has already recovered sees no reconnect, but clients can still be
// on peers for up to their probe backoff; with a buddy configured a restart is
// indistinguishable from a recovery, so it opens the window rather than
// narrowing delivery. The cost is one grace period of dual publishing per
// restart, which is cheap; the alternative was silent same-site events.
//
// StatusChanged rather than nats.ReconnectHandler: Connect already installs a
// ReconnectHandler for logging, and same-kind nats.Options overwrite rather than
// compose, so adding one here would silently drop the other. StatusChanged is an
// independent listener registry — the same reason Drain uses it.

func TrackReconnects(ctx context.Context, conn *o11ynats.Conn, assumeOutage bool) *ReconnectTracker {
	tr := &ReconnectTracker{}
	if assumeOutage {
		tr.MarkReconnect(time.Now().UTC())
	}
	if conn == nil {
		return tr
	}
	nc := conn.NatsConn()
	ch := nc.StatusChanged(nats.CONNECTED)

	go func() {
		defer nc.RemoveStatusListener(ch)
		for {
			select {
			case <-ctx.Done():
				return
			case _, ok := <-ch:
				if !ok {
					return
				}
				now := time.Now().UTC()
				tr.MarkReconnect(now)
				slog.InfoContext(ctx, "nats connection restored; entering room-route grace window",
					"restored_at", now)
			}
		}
	}()
	return tr
}
