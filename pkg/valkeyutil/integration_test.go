...
// permanently once it passes the ceiling, long after Valkey recovered.
...
// missing TTL must not turn the fixed window into a sliding one — an increment
// against a key that already has an expiry leaves that expiry alone, or a caller
// could hold its window open indefinitely by keeping up a steady request rate.
