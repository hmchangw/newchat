...
func TestSystemMessageTypes_CoversAllSystemConstants(t *testing.T) {
	...
	set := model.SystemMessageTypeSet()
	assert.Len(t, set, len(want),
		"SystemMessageTypeSet must enumerate exactly the system-message constants")
	for _, typ := range want {
		_, ok := set[typ]
		assert.True(t, ok, "system type %q missing from SystemMessageTypeSet", typ)
	}
}

func TestSystemMessageTypeSet_ReturnsIsolatedClone(t *testing.T) {
	first := model.SystemMessageTypeSet()
	n := len(first)
	// Mutating the returned set must not affect the canonical set or later calls.
	first["not_a_real_type"] = struct{}{}
	delete(first, model.MessageTypeRoomCreated)

	second := model.SystemMessageTypeSet()
	assert.Len(t, second, n, "a later call must return the full canonical set")
	assert.True(t, model.IsSystemMessageType(model.MessageTypeRoomCreated),
		"membership test must be unaffected by a caller mutating a returned clone")
	_, leaked := second["not_a_real_type"]
	assert.False(t, leaked, "a caller's mutation must not leak into the canonical set")
}
