...
type ThreadRoom struct {
	...
	// ParentSubscribed is true once the parent author's thread subscription has
	// been written. Server-side only, never sent to clients. Replies keep
	// re-writing the subscription while this is false. Rooms created before this
	// field have no value, which reads as false, so their next reply repairs them.
	ParentSubscribed bool `json:"-" bson:"parentSubscribed,omitempty"`
}
