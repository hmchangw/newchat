...
// Verb semantics, not just verb spelling. The naming guard in rpcmethod_test.go
// checks the first token is in a closed set; it cannot read a handler's response
// type, so which verb is *right* is a rule for authors:
//
...
//
// This follows Google AIP-131/132/190, where List's total_size is an optional
// response field and never the thing that makes a method a List. The rule exists
// because three history-service methods returning []Message with limit/cursor
// were spelled get_ purely because they lacked a total, which left one golden
// file carrying two contradictory precedents for the same shape.
//
...
var rpcMethods = []RPCMethod{
	...
	// history-service
	MethodListChannelMessages, MethodListThreadMessages, MethodListNextMessages,
	MethodListSurroundingMessages, MethodGetMessage, MethodBatchGetMessages,
