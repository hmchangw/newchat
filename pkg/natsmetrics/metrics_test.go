...
func TestPublishAndRequestMetrics(t *testing.T) {
	...
	p.RecordRPCClientCall(context.Background(), MethodGetPresenceSnapshot, 25*time.Millisecond, nats.ErrTimeout)
...
func TestPublishAndRequestLabelsAreBounded(t *testing.T) {
	...
	p.RecordRPCClientCall(context.Background(), RPCMethod("another.dynamic.operation"), time.Millisecond, nil)
...
func TestZeroValuePublisherDoesNotAffectBusinessFlow(t *testing.T) {
	...
	publisher.RecordRPCClientCall(context.Background(), MethodGetPresenceSnapshot, time.Millisecond, errors.New("request failed"))
}
