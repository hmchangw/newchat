...
// rpc.method as the logical method name, which for a subject-routed RPC is what
// the registered pattern derives to — never the subject itself, which carries
// room and account identifiers.
func rpcMethod(value string) attribute.KeyValue {
	return attribute.String(rpcMethodKey, value)
}
