...
func setupBenchThread(b *testing.B) (*gocql.Session, *Parent) {
	...
	return s, &Parent{MessageID: "bench-parent", RoomID: "bench-room", Bucket: 0, ThreadRoomID: "bench-thread"}
}
...
func BenchmarkScan(b *testing.B) {
	...
	for _, k := range []int{100, 250, 500, 1000, 2000} {
		b.Run(fmt.Sprintf("rows=%d", k), func(b *testing.B) {
			b.ReportAllocs()
			for i := 0; i < b.N; i++ {
				if _, _, _, err := countReplies(ctx, sess, "bench-thread", k); err != nil {
					b.Fatal(err)
				}
			}
		})
...
func BenchmarkStampParent(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		if err := writeToBothRows(ctx, sess, p, i, &now, true); err != nil {
			b.Fatal(err)
		}
	}
}
...
func BenchmarkMaintainPastLimit(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		if _, err := Maintain(ctx, sess, p, pol, +1, &now, false); err != nil {
			b.Fatal(err)
		}
	}
}
