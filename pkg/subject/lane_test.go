...
func TestEffectiveRouteMode(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			configured:      subject.RouteLocal,
			lane:            subject.LaneHome,
			lastReconnectAt: now.Add(-5 * time.Minute),
		...
		},
		{
			...
			configured:      subject.RouteLocal,
			lane:            subject.LaneHome,
			lastReconnectAt: now.Add(-31 * time.Minute),
			want:            subject.RouteLocal,
		},
		{
		...
		},
		{
			...
			configured:      subject.RouteLocal,
			lane:            subject.LaneHome,
			lastReconnectAt: now.Add(-30 * time.Minute),
			want:            subject.RouteLocal,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			r := subject.NewLaneRouter(tt.configured, tt.lane,
				func() time.Time { return tt.lastReconnectAt }, grace)
			assert.Equal(t, tt.want, r.Mode(now))
		})
	}
}
...
// A nil lastReconnectAt must not panic — the failover router has no tracker, and a
