...
type stubBatch struct{ err error }

func (b stubBatch) Messages() <-chan o11ynats.FetchedMessage {
	ch := make(chan o11ynats.FetchedMessage)
	close(ch)
	return ch
}
...
// recordingSource drives runConsumer and captures every recovery decision.
...
func (s *recordingSource) Fetch(context.Context, int, ...jetstream.FetchOpt) (msgBatch, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	i := s.calls
	s.calls++
	if i < len(s.fetchErrs) && s.fetchErrs[i] != nil {
		return nil, s.fetchErrs[i]
	}
	if i < len(s.batches) {
...
func newLoopHandler(t *testing.T) *Handler {
	t.Helper()
	ctrl := gomock.NewController(t)
	store := NewMockStore(ctrl)
	return NewHandler(store, newMessageCollection("msgs-v1", "site-a", time.Time{}, false), 500)
}
...
func runLoop(t *testing.T, src fetchSource, stopCh chan struct{}) {
	...
	go runConsumer(context.Background(), src, newLoopHandler(t), 10, 500, time.Minute, stopCh, doneCh)
	select {
	case <-doneCh:
...
func TestRunConsumer_ExitsWhenRecoveryGivesUp(t *testing.T) {
	src := &recordingSource{
		fetchErrs: []error{jetstream.ErrConnectionClosed},
		keepGoing: false,
	}
...
func TestBackoffStep_ClampsPastTheSchedule(t *testing.T) {
	assert.Equal(t, jsiter.RebuildBackoff[0], backoffStep(0))
	last := jsiter.RebuildBackoff[len(jsiter.RebuildBackoff)-1]
	assert.Equal(t, last, backoffStep(len(jsiter.RebuildBackoff)))
