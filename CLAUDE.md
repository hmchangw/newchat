...
- **Tier 3 — specialist, you'll know when.** Don't use these in ordinary request/reply handlers:
  ...
  - `errcode.Parse` — **only** in a site that decides nothing from the reply (e.g. `client_update.go`, which has already classified by HTTP status and just lifts display text). Anything that decides success or failure must use `FromReply`: `Parse` answers "does this fit *this build's* `Error` struct", so one added or retyped field makes it report "not an envelope" and the reply falls past every `!Code.Valid()` guard. A positive success check downstream is not a substitute — `status == "ok"`, `ack.OK` and a non-nil collection each only hold for a reply that omits that field. semgrep blocks it everywhere else, so pair it with a `nosemgrep` naming the reason.
