...
- Create indexes in the store constructor or a dedicated `EnsureIndexes` method at startup. The one exception is a degradable service confirming an index it depends on before a write, through `mongoutil.IndexGate` (see the sole-creator rule below)
...
- **Degraded start.** Eight services pass `mongoutil.WithDegradedStart()` so an unreachable MongoDB at startup warns instead of exiting: `broadcast-worker`, `history-service`, `message-gatekeeper`, `message-worker`, `notification-worker`, `search-service`, `tcard-service`, `user-presence-service`. Every other service MUST keep failing fast — MongoDB is their job. See `docs/health-probes.md`.
- **A degradable service must not be the sole creator of an index whose absence is a correctness hazard for its own write path.** It starts with MongoDB down, so its `EnsureIndexes` is skipped exactly when an outage makes that most likely, and the damage lands on recovery. Such an index must also be owned by a fail-fast service, which creates it ahead of its read-path indexes so a shared startup budget reaches it. The degradable service either keeps creating it with the identical spec via the non-destructive `mongoutil.EnsureIndex` (idempotent; closes the fresh-site window before the owner has run; never repairs, because two repairers racing on one dirty index can drop each other's replacement — a conflicting index closes the gate until the owner repairs) **and gates the write that relies on it on the index being confirmed** — a startup-only attempt is not enough, because a degraded pod resumes consuming the moment MongoDB returns, before the crashlooping owner has restarted; `message-worker`'s `mongoutil.IndexGate`s confirm `thread_rooms.parentMessageId` before `EnsureThreadRoom` (whose upsert filters on that key, so without it two concurrent first replies both miss and both insert) and `thread_subscriptions.(threadRoomId,userAccount)` before the subscription inserts and upserts, and NAK until they do; both are owned by `room-service` — or, for an index it only reads, verifies it with `mongoutil.WarnMissingUniqueIndexes` (which checks the `unique` option, not just the name). A performance-only index may stay: its absence costs collection scans until the next healthy restart, not integrity.
...
short for work that waits on a dependency.
