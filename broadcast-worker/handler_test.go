...
func TestHandler_HandleMessage_Errors(t *testing.T) {
	...
	t.Run("get room meta fails", func(t *testing.T) {
		...
		us.EXPECT().FindUsersByAccounts(gomock.Any(), []string{"sender"}).Return(nil, nil) // combined lookup runs before GetRoomMeta
		store.EXPECT().GetRoomMeta(gomock.Any(), "room-1").Return(roommetacache.Meta{}, errors.New("db error"))
