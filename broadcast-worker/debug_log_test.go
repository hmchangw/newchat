...
func TestHandler_DMFanout_DebugBreadcrumbs(t *testing.T) {
	...
	pub := &mockPublisher{}
	store.EXPECT().UpdateRoomLastMessage(gomock.Any(), "dm-1", "msg-1", msgTime, false, gomock.Any()).Return(nil).AnyTimes()
	...
	h := NewHandler(store, us, pub, NewMockRoomKeyProvider(ctrl), defaultParentFetcher, defaultLastMsgFetcher, false)
...
func TestHandler_DMFanout_NoContentLeak(t *testing.T) {
	...
	store.EXPECT().ListSubscriptions(gomock.Any(), "dm-1").Return(testDMSubs, nil).AnyTimes()
	us.EXPECT().FindUsersByAccounts(gomock.Any(), []string{"alice"}).Return([]model.User{testUsers[0]}, nil).AnyTimes()
	h := NewHandler(store, us, &mockPublisher{}, NewMockRoomKeyProvider(ctrl), defaultParentFetcher, defaultLastMsgFetcher, false)
