...
// nopLastMsgFetcher is a stateless LastMessageFetcher for handlers whose test doesn't assert on the fetch.
...
// ptrOf derives the pointer a preview implies, mirroring the production fetcher when the reply carries none.
...
// stubLastMsgFetcher is a LastMessageFetcher double returning a fixed preview/pointer (or error) and counting calls.
...
// become the stored rooms.lastMsg preview (newest non-system message only).
...
// The delete-path RPC is skipped when the stored preview already identifies the
// survivor: neither pointer targets the deleted message, so the preview IS the survivor.
func TestHandleDeleted_StoredPreviewIdentifiesSurvivor_SkipsFetch(t *testing.T) {
	...
	room := &model.Room{ID: "r1", Type: model.RoomTypeChannel, SiteID: "site-a", LastMsgID: "m-last", LastMsgAt: &stored.CreatedAt, LastMsg: stored}
	fetcher := &stubLastMsgFetcher{}
...
// carries the surviving preview AND the thread badge still updates (both per docs).
...
// When the newest survivor is a system notice, the rewind gets the system pointer
// and the user-message preview separately — sorting keeps the notice, the event previews the user message.
...
// The fetch ceiling is the delete-event time: the stored lastMsgAt can lag
// coalesced creates, so a survivor in that window must stay findable.
...
// An encrypted-room edit that clears the text still publishes an envelope
// (clients apply the clear), while the store patch carries no preview ciphertext.
...
// Drift skip-path: deleting an unrelated message while the room is in drift must
// rewind with the SYSTEM pointer from lastMsgId/lastMsgAt, not regress it to the preview.
func TestHandleDeleted_DriftSkipPath_PointerFromStoredPointerNotPreview(t *testing.T) {
	ctrl := gomock.NewController(t)
	store := NewMockStore(ctrl)
	us := NewMockUserStore(ctrl)
	pub := &mockPublisher{}
	...
	deletedAt := time.Date(2026, 7, 1, 12, 10, 0, 0, time.UTC)
	sysAt := deletedAt.Add(-time.Minute)
	stored := &model.LastMessagePreview{MessageID: "m-user", SenderAccount: "bob", Msg: "older user msg", CreatedAt: deletedAt.Add(-time.Hour)}
	room := &model.Room{ID: "r1", Type: model.RoomTypeChannel, SiteID: "site-a", LastMsgID: "m-sys", LastMsgAt: &sysAt, LastMsg: stored}
	fetcher := &stubLastMsgFetcher{}

	store.EXPECT().GetRoom(gomock.Any(), "r1").Return(room, nil)
	store.EXPECT().RewindRoomLastMessage(gomock.Any(), "r1", "msg-1",
		&model.LastMessagePointer{MessageID: "m-sys", CreatedAt: sysAt}, stored, deletedAt).Return(nil)

	h := NewHandler(store, us, pub, keyStore, defaultParentFetcher, fetcher, false)
	require.NoError(t, h.HandleMessage(context.Background(), makeDeleteEvent("r1", deletedAt)))

	assert.Equal(t, 0, fetcher.calls, "unrelated delete with a stored preview skips the RPC")
}

// A room with a preview but no lastMsgAt (malformed/legacy) must NOT take the
// skip shortcut — fall through to the authoritative fetch.
func TestHandleDeleted_StoredPreviewWithoutPointer_Fetches(t *testing.T) {
	ctrl := gomock.NewController(t)
	store := NewMockStore(ctrl)
	us := NewMockUserStore(ctrl)
	pub := &mockPublisher{}
	...
	survivor := survivorPreview(deletedAt.Add(-2 * time.Hour))
	fetcher := &stubLastMsgFetcher{preview: survivor}
	stored := &model.LastMessagePreview{MessageID: "m-last", SenderAccount: "bob", Msg: "x", CreatedAt: deletedAt.Add(-time.Minute)}
	room := &model.Room{ID: "r1", Type: model.RoomTypeChannel, SiteID: "site-a", LastMsgID: "m-last", LastMsg: stored} // no LastMsgAt
	...
	assert.Equal(t, 1, fetcher.calls, "a preview without a pointer can't be trusted — fetch instead")
}
