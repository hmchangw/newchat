...
func main() {
	...
	// Checks, not Check: the lanes carry one loop guard each, so a dead lane
	// fails readiness by name instead of hiding behind its live sibling.
	...
	// The lanes mark their own guards inside StopHooks, which the caller appends
...
// lane consumes on; see failoverlane.BuildHandler.
func (d *broadcastDeps) newLaneHandler(nc *o11ynats.Conn, js o11ynats.JetStream, lane subject.Lane) *Handler {
	...
	return NewHandler(d.store, d.users, publisher, d.keys,
		...
		subject.NewLaneRouter(d.routeMode, lane, d.lastReconnectAt, d.cfg.Revert.FailoverRevertGrace),
...
// guardedHandler keeps a handler panic from crash-looping the worker: jobguard
// Acks the message instead, dropping the poison pill. Both lanes are wrapped,
// because a failover-lane message is still this site's message.
func guardedHandler(process messageProcessor) func(context.Context, jetstream.Msg) {
	return jobguard.Handler(process)
}

// guardedProcessor is guardedHandler for natsmetrics.Consume, which hands back
// its own message type. Only the integration test binds that loop directly.
func guardedProcessor(process messageProcessor) natsmetrics.ProcessMessage {
	return jobguard.Handler(func(ctx context.Context, msg *natsmetrics.Message) { process(ctx, msg) })
}
