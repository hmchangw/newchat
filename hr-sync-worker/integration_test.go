...
func TestWorker_EndToEnd(t *testing.T) {
	...
	var alice bson.M
	require.NoError(t, db.Collection(usersCollection).FindOne(ctx, bson.M{"account": "alice"}).Decode(&alice))
	...
	var carol bson.M
	require.NoError(t, db.Collection(usersCollection).FindOne(ctx, bson.M{"account": "carol"}).Decode(&carol))
