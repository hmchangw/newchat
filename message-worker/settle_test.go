...
func TestHandler_Settle(t *testing.T) {
	...
	t.Run("infra-class failure naks indefinitely", func(t *testing.T) {
		...
		assert.True(t, msg.naked)
		assert.False(t, msg.acked)
		assert.Equal(t, 2*time.Minute, msg.nakDelay, "backoff tail")
	...
	t.Run("the production default window drops on the 34th delivery", func(t *testing.T) {
		// Pins the documented mapping: 1h of accumulated backoff is first reached
		// on delivery 34 (36s + 30 × 2m). If the backoff schedule is retuned this
		// number moves, which is exactly why the config is a duration.
		under := newDegradeStateHandler(t, nil, noopPublish, false, testDropPolicy())
		msg33 := &fakeJetStreamMsg{numDelivered: 33}
		under.settle(context.Background(), msg33, requestClassErr())
		assert.True(t, msg33.naked)
		assert.False(t, msg33.acked)

		over := newDegradeStateHandler(t, nil, noopPublish, false, testDropPolicy())
		msg34 := &fakeJetStreamMsg{numDelivered: 34}
		over.settle(context.Background(), msg34, requestClassErr())
		assert.True(t, msg34.acked)
		assert.False(t, msg34.naked)
	})
}
...
func TestDeliveriesToDrop(t *testing.T) {
	tests := []struct {
		...
		{name: "the production default", window: time.Hour, want: 34},
...
func TestHandler_Settle_DropLogNamesTheMessageWithoutLeakingIt(t *testing.T) {
	...
	assert.Equal(t, "1h12m36s", fields["retried_for"], "40 deliveries: 36s + 36 × 2m of accumulated backoff")
