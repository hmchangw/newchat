...
func TestDeliveriesToDrop(t *testing.T) {
	tests := []struct {
		...
		{name: "the production default", window: time.Hour, want: 17},
		{name: "the first redelivery covers a sub-second window", window: time.Millisecond, want: 2},
		{name: "a window inside the third guaranteed wait", window: 6 * time.Second, want: 4},
		{name: "just past it, same delivery", window: 7 * time.Second, want: 4},
...
func TestHandler_Settle_DropLogNamesTheMessageWithoutLeakingIt(t *testing.T) {
	...
	assert.Equal(t, "2h56m18s", fields["retried_for"], "40 deliveries of guaranteed (min-jitter) accumulated backoff")
