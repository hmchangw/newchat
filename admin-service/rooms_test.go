...
func TestHandler_listRooms(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(m *MockAdminStore) {
				m.EXPECT().ListRooms(gomock.Any(), "", 2, 10).
					Return([]model.Room{{
		...
		},
		{
			...
			query: "",
			setupMock: func(m *MockAdminStore) {
				m.EXPECT().ListRooms(gomock.Any(), "", 1, 20).
		...
		},
		{
			...
			query: "?limit=100000",
			setupMock: func(m *MockAdminStore) {
				m.EXPECT().ListRooms(gomock.Any(), "", 1, 100).
					Return([]model.Room{}, int64(0), nil)
			},
			wantStatus: http.StatusOK,
		},
		...
		},
		{
			...
			setupMock: func(m *MockAdminStore) {
				m.EXPECT().ListRooms(gomock.Any(), "aB3xY9kLmN2pQ7rS4", 1, 20).
					Return([]model.Room{{
