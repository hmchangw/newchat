...
// heartbeatIterator raises a missing-heartbeat error before ending for real.
// nats.go returns ErrNoHeartbeat from Next without invalidating the iterator,
// so the loop must retry rather than treat it as the consumer's death.
type heartbeatIterator struct{ calls int }

func (h *heartbeatIterator) Next(...jetstream.NextOpt) (context.Context, jetstream.Msg, error) {
	h.calls++
	if h.calls == 1 {
...
// A heartbeat gap is a stall on a healthy connection — a JetStream leader
// election or a rolling server upgrade produces one. The guard's restart hook
// raises SIGTERM, so escalating it would restart every pull-iterator worker in
// the fleet at the same moment, which is the storm this package exists to stop.
func TestConsumeLoop_MissingHeartbeatDoesNotRequestRestart(t *testing.T) {
	store := &stubStore{}
	f := newFlusher(store, 0, 0)
	iter := &heartbeatIterator{}

	var restarts int
	state := loopguard.New("consume-loop", func() { restarts++ })
	probe := state.Check().Probe
	...
	assert.Equal(t, 2, iter.calls, "the loop must call Next again after a heartbeat gap")
	assert.Equal(t, 1, restarts, "only the terminal error may request a restart")

	err := probe(context.Background())
	require.Error(t, err, "the terminal error must still fail readiness")
	assert.NotContains(t, err.Error(), jetstream.ErrNoHeartbeat.Error(),
		"readiness must name the real cause, not the heartbeat gap")
}
