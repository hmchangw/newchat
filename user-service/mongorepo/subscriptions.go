...
// deletedRoomNamePrefix is what room-service prepends to a room's name when it
// soft-deletes the room; the list drops local rows whose room starts with it.
// The regex below is derived from it so the marker is written down once.
...
type SubscriptionRepo struct {
	...
	// The *Secondary handles serve reads that tolerate stale data. The list must
	// show a subscription the caller just changed, and GetAppSubscription guards
	// against duplicates, so both stay on the primary.
...
// NewSubscriptionRepo builds a SubscriptionRepo over db. The deleted-room filter
// works off the room's name ("Del-"), so a subscription whose room has no local
// document — one owned by another site — is kept. sortKeyCacheSize and
	...
	stages := bson.A{
		...
		bson.M{"$lookup": bson.M{
			...
			"pipeline": bson.A{
				...
				// createdAt is gone: only the sort key this pipeline used to
				// compute read it, and the list now sorts in Go.
...
// has no messages — plus a hasMore flag from reading one row extra. Rooms
// soft-deleted here ("Del-") are left out. favorite narrows to favorited rows
...
// originFilter leaves Teams-migrated rooms (origin "teams") out of account's
// results, or returns nil when SHOW_TEAMS_ROOM is on or the account is
// allowlisted. It reads the subscription's own origin, not the room's, which is
// what makes it work for rooms owned by another site — those have no local room
// document — and lets it sit anywhere in a pipeline.
...
// originFilterStage is originFilter as an aggregation stage, for the pipelines
// that still join through $lookup. Empty when the account may see Teams rooms.
func (r *SubscriptionRepo) originFilterStage(account string) bson.A {
	f := r.originFilter(account)
	if f == nil {
		return bson.A{}
	}
	return bson.A{bson.M{"$match": bson.M{"origin": f}}}
}
...
// mongoutil.AggregatePagedHasMore does. When a fresh read drops a row (its room
// was renamed "Del-", or the subscription was deleted), the next candidate takes
// its place, so the page and hasMore describe rows that really exist rather than
