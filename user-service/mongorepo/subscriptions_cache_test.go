...
// Soft-deleting a room must drop the row right away, even with the old name
// still cached: the page is filled from a fresh read that drops "Del-" rooms.
...
// When the fresh read drops rooms soft-deleted after their keys were cached,
// the page pulls in the next candidates. The client gets a full page of rows
// that exist and a matching HasMore — not a short page whose HasMore sends the
// next request to an offset that skips rows.
func TestAggregateSubscriptions_PageRefillsPastFreshSoftDeletes_Integration(t *testing.T) {
	...
	// The two rows the first page would serve are soft-deleted elsewhere.
