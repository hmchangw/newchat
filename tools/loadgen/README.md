...
| `SOAK_MESSAGE_PREFIX` | `[LoadTest] ` | Label prepended to every body the run sends or edits, so a real user sharing the environment can tell load traffic from a colleague. Absorbed into the sampled payload size, never added on top, so the percentiles above still bound the wire. Set it to `none` to run unlabelled — an empty value takes the default instead, because `caarlos0/env` applies `envDefault` to a variable that is set but empty. |
