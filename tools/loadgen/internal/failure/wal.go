...
func OpenWAL(path string, options ...WALOption) (*WAL, error) {
	...
	// #nosec G304 -- the loadgen caller builds this path from SOAK_LEDGER_DIR, SOAK_RUN_ID and SOAK_LEDGER_EPOCH, all operator-set config, never request data.
...
func (w *WAL) Compact(events []Event) error {
	...
	// #nosec G304 -- temporaryPath is w.path, already validated by OpenWAL, plus a constant suffix.
	// nosemgrep: gosec.G304-1
