...
func TestFailureLedger_ARecoveredJournalLandsTheVerdictOnTheNextWrite(t *testing.T) {
	...
	operation := testLedgerOperation("m1", now)
	operation.LifecycleState = OperationJournaled
...
func journaledInvalidationReasons(journal *flakyInvalidationJournal) []string {
	...
	for i := range journal.events {
		if journal.events[i].Type == EventInvalidated {
...
func (j *flakyInvalidationJournal) Append(event *Event) error {
	if event.Type == EventInvalidated && j.failures > 0 {
		j.failures--
...
func TestFailureLedger_ACapacityVerdictFromReplayIsOwedToTheJournal(t *testing.T) {
	...
	for _, id := range []string{"m1", "m2"} {
		operation := testLedgerOperation(id, now)
		operation.LifecycleState = OperationJournaled
		journal.events = append(journal.events, Event{
			Type: EventStarted, Operation: operation, At: now,
		})
	}
...
func TestFailureLedger_AFailedAppendDoesNotImmediatelyRetryTheJournal(t *testing.T) {
	...
	require.Error(t, ledger.Start(testLedgerOperation("m1", now)))
...
func (j *countingFailureJournal) Append(event *Event) error {
	j.attempts++
	if j.fail {
		return errors.New("no space left on device")
	}
...
func (j *reasonFailingFailureJournal) Append(event *Event) error {
	if event.Type == EventInvalidated && event.InvalidReason == j.refuse {
		return errors.New("no space left on device")
	}
	return j.memoryFailureJournal.Append(event)
}
...
func TestFailureLedger_NoEvidenceIsWrittenWhileAVerdictIsOwed(t *testing.T) {
	...
	t.Run("a start is refused too", func(t *testing.T) {
		next := testLedgerOperation("m2", now)
		next.LifecycleState = OperationJournaled
	...
	t.Run("a recovered journal settles the debt and accepts the record", func(t *testing.T) {
		...
		assert.Equal(t, EventActivated, journal.events[len(journal.events)-1].Type)
	})
}
...
func TestFailureLedger_AStartSettlesAVerdictRaisedWhileItWasWriting(t *testing.T) {
	...
	require.NoError(t, ledger.Start(testLedgerOperation("m1", now)))
...
func (j *startRacingFailureJournal) Append(event *Event) error {
	if event.Type == EventStarted && !j.raised {
		j.raised = true
		j.refuseNext = true
