...
func TestFailureLedger_CloseWaitsForInFlightStartersBeforeTheFinalFlush(t *testing.T) {
	...
	go func() { startErr <- ledger.Start(testLedgerOperation("in-flight", now)) }()
	<-journal.entered
...
func awaitLedgerClosed(t *testing.T, ledger *Ledger, now time.Time) {
	t.Helper()
	for range 1_000_000 {
		err := ledger.Start(testLedgerOperation("probe", now))
		if err != nil && errors.Is(err, ErrLedgerClosed) {
			return
		}
		runtime.Gosched()
	}
...
func (j *blockingStartFailureJournal) Append(event *Event) error {
	if event.Type == EventStarted && event.Operation != nil &&
...
func (j *closeTrackingFailureJournal) Append(event *Event) error {
	if j.closed {
