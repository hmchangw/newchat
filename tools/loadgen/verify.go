...
func (r *verifyRun) openOracleConn(cfg *config) error {
	...
	// "general" pool, no disconnect hook: this is a request/reply conn, never a
	// probe recipient, so losing it is an oracle-query failure (tolerated, §10)
	// rather than a dropped recipient.
	var metrics *Metrics
	if r.env.collector != nil {
		metrics = r.env.collector.m
	}
	nc, err := connectWithCredsHealth(cfg.NatsURL, "loadgen-verify-oracle", cfg.NatsCredsFile, "general", metrics)
	if err != nil {
