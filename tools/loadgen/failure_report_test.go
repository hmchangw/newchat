...
func TestFailureEvidenceReport_BuildsEveryVerdictGateAndRetainsSidecars(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			report := buildSoakFailureEvidenceReport(
				&testRun, tc.ledger, soak, tc.health, tc.sidecars, start.Add(time.Minute),
			)
	...
	nilRun := buildSoakFailureEvidenceReport(nil, failureLedgerSnapshot{}, nil, failureObserverHealthSnapshot{}, nil, start)
