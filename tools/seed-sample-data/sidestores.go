...
// valkeyutil.Client.Set with restrictedCacheTTL), filtered to the rows that
// belong in site's database.
func writeSideStores(ctx context.Context, keys roomkeystore.RoomKeyStore, client valkeyutil.Client, site string) (sideStoreCounts, error) {
	...
	for _, k := range filterBySite(BuildRoomKeys(), site, roomKeyHomeSite) {
		if _, err := keys.Set(ctx, k.RoomID, k.KeyPair); err != nil {
			return c, fmt.Errorf("set room key for %s: %w", k.RoomID, err)
		}
		c.RoomKeys++
	}
	for _, e := range filterBySite(BuildRestrictedCache(), site, restrictedCacheHomeSite) {
