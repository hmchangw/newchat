...
// scoped to site's rows — or every row, when site is empty (the single-site
// path). users and hr_employee are never scoped: the full directory goes into
// every site's database so either portal can resolve any account and tell a
// client where its home site is — this mirrors what HR replication does in
// production.
func upsertAll(ctx context.Context, db *mongo.Database, site string) (mongoCounts, error) {
	...
	res, err = rooms.BulkUpsertByID(ctx, scopeToSite(BuildRoomsWithLastMsg(), site, roomHomeSite), func(r model.Room) string { return r.ID })
	if err != nil {
	...
	res, err = subs.BulkUpsertByID(ctx, scopeToSite(BuildSubscriptions(), site, subscriptionHomeSite), func(s model.Subscription) string { return s.ID })
	if err != nil {
	...
	res, err = members.BulkUpsertByID(ctx, scopeToSite(BuildRoomMembers(), site, memberHomeSite), func(m model.RoomMember) string { return m.ID })
	if err != nil {
	...
	res, err = msgs.BulkUpsertByID(ctx, scopeToSite(BuildMessages(), site, messageHomeSite), func(m model.Message) string { return m.ID })
	if err != nil {
	...
	res, err = trs.BulkUpsertByID(ctx, scopeToSite(BuildThreadRooms(), site, threadRoomHomeSite), func(t model.ThreadRoom) string { return t.ID })
	if err != nil {
	...
	res, err = tsubs.BulkUpsertByID(ctx, scopeToSite(BuildThreadSubscriptions(), site, threadSubscriptionHomeSite), func(t model.ThreadSubscription) string { return t.ID })
	if err != nil {
