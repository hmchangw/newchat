...
func main() {
	...
	healthStop, err := health.Serve(cfg.HealthAddr, 5*time.Second,
		jsiter.Check("oplog-collections-transformer", func() bool { c := consumer.Load(); return c != nil && c.IsUp() }))
	if err != nil {
	...
	cc, err := jsiter.NewSupervisor(ctx, "oplog-collections-transformer", jsiter.ConsumeFrom(resolve, process))
	if err != nil {
