...
// setupCassandraForRun returns the keyspace name and host so the caller can
// build a config for run() to dial its own session against, plus a
// keyspace-pinned session for seeding test rows (production queries use
// unqualified table names, so the seed insert needs the same pinning). Schema
// setup is shared with messagesource_integration_test.go's setupCassandra via
// newTestCassandraSession in cassandra_test_helpers_test.go.
func setupCassandraForRun(t *testing.T, prefix string) (keyspace, hostAddr string, pinned *gocql.Session) {
	t.Helper()
	return newTestCassandraSession(t, prefix)
}
