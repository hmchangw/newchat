...
// fakeStreamManager is a minimal in-memory streamManager, ported unchanged from
// broadcast-worker/bootstrap_test.go (same interface shape).
...
func TestBootstrapStreams(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !tc.enabled {
				assert.Empty(t, fake.created, "the disabled (production) path must never call CreateOrUpdateStream")
			}
		})
	}
}
