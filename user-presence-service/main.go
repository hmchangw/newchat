...
func main() {
	...
	routers, err := failoverlane.BindRouters(ctx, nc, nil, dialer,
		func(_ context.Context, conn *o11ynats.Conn, _ o11ynats.JetStream, _ subject.Lane) (*natsrouter.Router, error) {
			...
			handler := NewHandler(store, userDir, peer, natsutil.CorePublishFunc(conn, publishMetrics), cfg.SiteID, cfg.Presence.BatchMax)
	...
	sweeper := NewSweeper(store, natsutil.CorePublishFunc(nc, publishMetrics), cfg.SiteID, cfg.Presence.SweepInterval)
