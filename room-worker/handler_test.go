...
func TestHandler_ProcessAddMembers_WithOrgs(t *testing.T) {
	...
	store.EXPECT().ListOrgRoomMembers(gomock.Any(), gomock.Any(), []string{"eng"}).Return(nil, nil)
...
func TestHandler_ProcessAddMembers_RoomEventCarriesEnrichedMembers(t *testing.T) {
	...
	subIDByAccount := map[string]string{}
	for _, s := range createdSubs {
		subIDByAccount[s.User.Account] = s.ID
	}
	byAccount := map[string]model.RoomMember{}
	for _, m := range evt.Members {
		byAccount[m.Member.Account] = m
	}

	acceptedAt := time.UnixMilli(req.Timestamp).UTC()
	bob := byAccount["bob"]
	assert.Equal(t, subIDByAccount["bob"], bob.ID,
		"individual-only rooms mirror member.list's subscriptions fallback: envelope id is the subscription id")
	assert.Equal(t, "r1", bob.RoomID)
	assert.True(t, bob.Ts.Equal(acceptedAt), "ts must be the request-acceptance time, got %v", bob.Ts)
	assert.Equal(t, model.RoomMemberIndividual, bob.Member.Type)
	assert.Equal(t, "u2", bob.Member.ID)
	assert.Equal(t, "Bob", bob.Member.EngName)
	assert.Equal(t, "鮑", bob.Member.ChineseName)
	assert.Equal(t, "Cardiology", bob.Member.SectName)
	assert.Equal(t, "E-1", bob.Member.EmployeeID)
	assert.False(t, bob.Member.IsOwner, "newly added members are plain members")

	carol := byAccount["carol"]
	assert.Equal(t, subIDByAccount["carol"], carol.ID)
	assert.Equal(t, "Radiology", carol.Member.SectName)
	assert.Equal(t, "E-2", carol.Member.EmployeeID)
...
func TestHandler_ProcessAddMembers_WithOrgs_RoomEventMembersEnrichment(t *testing.T) {
	...
	store.EXPECT().ListOrgRoomMembers(gomock.Any(), "r1", []string{"eng"}).Return(nil, nil)
	...
	var orgDoc, bobDoc *model.RoomMember
	for _, doc := range createdRoomMembers {
		switch {
		case doc.Member.Type == model.RoomMemberOrg:
			orgDoc = doc
		case doc.Member.Account == "bob":
			bobDoc = doc
		}
	}
	require.NotNil(t, orgDoc)
	require.NotNil(t, bobDoc)

	org := evt.Members[0]
	assert.Equal(t, model.RoomMemberOrg, org.Member.Type, "org entries come first, mirroring member.list's sort")
	assert.Equal(t, orgDoc.ID, org.ID, "envelope id is the org room_members doc written by this add")
	assert.Equal(t, "r1", org.RoomID)
	...
	assert.Equal(t, 2, org.Member.MemberCount)
	assert.Empty(t, org.Member.Account)

	bob := evt.Members[1]
	assert.Equal(t, model.RoomMemberIndividual, bob.Member.Type)
	assert.Equal(t, "bob", bob.Member.Account)
	assert.Equal(t, bobDoc.ID, bob.ID,
		"org-tracking rooms mirror member.list's room_members path: envelope id is the room_members doc id")
	assert.Equal(t, "u2", bob.Member.ID)
	assert.Equal(t, "Engineering", bob.Member.SectName)
	assert.Equal(t, "E-1", bob.Member.EmployeeID)
...
func TestHandler_ProcessAddMembers_OrgWithNoUsersFallsBackToOrgID(t *testing.T) {
	...
	store.EXPECT().ListOrgRoomMembers(gomock.Any(), gomock.Any(), []string{"ghost-org"}).Return(nil, nil)
	...
	org := evt.Members[0]
	assert.Equal(t, model.RoomMemberOrg, org.Member.Type)
	assert.Equal(t, "ghost-org", org.Member.OrgName, "no directory rows → name falls back to the orgID, matching member.list")
	assert.Empty(t, org.Member.OrgDescription)
	assert.Zero(t, org.Member.MemberCount)
}
...
func TestHandler_ProcessAddMembers_OrgReAddReusesPersistedEnvelope(t *testing.T) {
	// An org re-add (or redelivery) must carry the persisted room_members id/ts and not attempt a duplicate insert.
	ctrl := gomock.NewController(t)
	store := NewMockSubscriptionStore(ctrl)
	...
	persistedTs := time.Date(2026, 4, 1, 9, 0, 0, 0, time.UTC)
	store.EXPECT().GetRoomMeta(gomock.Any(), "r1").Return(&model.Room{ID: "r1", Type: model.RoomTypeChannel, SiteID: "site-a"}, nil)
	// Every org member is already subscribed → nothing to write anywhere.
	store.EXPECT().ListAddMemberCandidates(gomock.Any(), []string{"eng"}, nil, "r1").
		Return([]AddMemberCandidate{}, nil)
	store.EXPECT().HasOrgRoomMembers(gomock.Any(), "r1").Return(true, nil)
	store.EXPECT().FetchOrgDisplayUsers(gomock.Any(), []string{"eng"}).Return([]orgdisplay.User{
		{SectID: "eng", SectName: "Engineering"},
	}, nil)
	store.EXPECT().ListOrgRoomMembers(gomock.Any(), "r1", []string{"eng"}).Return([]model.RoomMember{
		{ID: "rm-eng-existing", RoomID: "r1", Ts: persistedTs,
	...
	store.EXPECT().ApplyMemberCountDelta(gomock.Any(), "r1", 0, 0, gomock.Any()).Return(false, nil)
	// No write expectations: the org row already exists.
	...
	require.Len(t, evt.Members, 1)
	org := evt.Members[0]
	assert.Equal(t, "rm-eng-existing", org.ID,
		"re-added org must reuse the persisted room_members doc id — member.list returns that id")
	assert.True(t, org.Ts.Equal(persistedTs), "re-added org must carry the persisted ts, got %v", org.Ts)
	assert.Equal(t, "Engineering", org.Member.OrgName)
...
func TestHandler_ProcessAddMembers_BackfillRunsOnFirstOrgTransition(t *testing.T) {
	...
	store.EXPECT().ListOrgRoomMembers(gomock.Any(), gomock.Any(), []string{"o1"}).Return(nil, nil)
...
func TestHandler_ProcessAddMembers_BackfillSubscriptionAccountsErrorFailsHard(t *testing.T) {
	...
	store.EXPECT().FetchOrgDisplayUsers(gomock.Any(), []string{"o1"}).Return(nil, nil)
	store.EXPECT().ListOrgRoomMembers(gomock.Any(), roomID, []string{"o1"}).Return(nil, nil)
...
func TestHandler_ProcessAddMembers_BackfillSkippedWhenRoomAlreadyHasOrgs(t *testing.T) {
	...
	store.EXPECT().ListOrgRoomMembers(gomock.Any(), gomock.Any(), []string{"o_new"}).Return(nil, nil)
