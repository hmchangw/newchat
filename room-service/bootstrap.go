...
// instance where the streams it publishes to do not yet exist. In production
// streams are pre-provisioned by ops/IaC and Bootstrap.Enabled must remain
// false; the service only publishes to existing streams.
type bootstrapConfig struct {
	// Enabled (BOOTSTRAP_STREAMS) toggles whether the service calls
	// CreateOrUpdateStream at startup for the streams it publishes to.
...
// bootstrapStreams handles the JetStream ROOMS stream this service publishes
// to. When enabled (dev/integration), it creates the stream via
...
func bootstrapStreams(ctx context.Context, js streamManager, siteID string, enabled bool) error {
	...
	// broken before the first publish.
