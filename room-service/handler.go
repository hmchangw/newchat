...
func (h *Handler) getRoomKey(c *natsrouter.Context) (*model.RoomKeyGetResponse, error) {
	...
	// Bot key delivery is out of scope for this PR: fanOutKey omits bots
	// push-side, and this closes the pull side so behavior stays consistent.
	if model.IsBot(requesterAccount) || model.IsPlatformAdminAccount(requesterAccount) {
		return nil, errcode.Forbidden("bots cannot fetch room keys")
	}
