...
func main() {
	...
	// One handler per lane; see failoverlane.BuildRouter for why nothing that
	...
	routers, err := failoverlane.BindRouters(ctx, nc, js, dialer,
		func(ctx context.Context, conn *o11ynats.Conn, laneJS o11ynats.JetStream, lane subject.Lane) (*natsrouter.Router, error) {
			laneHandler := buildHandler(laneJS, conn,
				subject.NewLaneRouter(roomRouteMode, lane, homeRestores.LastReconnectAt, cfg.Revert.FailoverRevertGrace))
			laneHandlers = append(laneHandlers, laneHandler)
	...
	for _, h := range laneHandlers {
		if failoverPublish != nil {
			h.SetFailoverPublisher(failoverPublish)
		}
	}
