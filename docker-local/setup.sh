...
#
# Each server runs JetStream STANDALONE and the two are joined by a LEAFNODE
# link, not a gateway. Both a cluster{} block and a gateway{} block give the
# server a cluster identity, which puts JetStream into clustered mode; its Raft
# meta-group then needs the system account carried over intra-cluster routes,
# and with one server per site there are none, so the server refuses to start
# JetStream:
#
#   JetStream cluster requires configured routes or solicited leafnode
#   for the system account
#
# A gateway cannot avoid this — the gateway NAME *is* the cluster name, so a
# gateway-only config still logs "Cluster: <site>" and still demands cluster
# connectivity. Leafnodes are the supported way to join two independent
...
# do not use.
write_fed_nats_conf() {
  local site="$1" peer="$2" out="$3" role="$4"
  local leaf_block
  if [ "$role" = "hub" ]; then
    leaf_block="leafnodes {
  port: 7422
}"
  else
    leaf_block="leafnodes {
...
write_fed_nats_conf site-local  site-remote "$SCRIPT_DIR/nats-site-local.conf" hub
write_fed_nats_conf site-remote site-local  "$SCRIPT_DIR/nats-site-remote.conf" spoke
