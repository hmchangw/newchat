...
func main() {
	...
	lanes, err := failoverlane.BindLanes(ctx, nc, js, dialer, &failoverlane.LanesSpec{
		...
		Buddy: &failoverlane.BuddyLane{
			...
			PublishesTo: []stream.Config{stream.MessagesCanonicalFailover(cfg.SiteID)},
		...
		return guardedLaneHandler(gatekeeperHandler(laneHandler(conn, laneJS, lane))), nil
	})
	if err != nil {
...
// consumes on; see failoverlane.BuildHandler for why each lane needs its own.
...
//
// Unguarded on its own: both lanes bind it through guardedLaneHandler.
...
// guardedLaneHandler wraps a lane's per-message handler in jobguard's panic
// recovery, dropping the poison message instead of taking the process down.
//
...
func guardedLaneHandler(handle func(context.Context, jetstream.Msg)) func(context.Context, jetstream.Msg) {
	return jobguard.Handler(handle)
}
...
// independent cursors.
//
// It takes the home lane's outage retry budget for the same reason: the message
// body is identical, and a displaced client's send is no more droppable than a
// home-lane one when a cold-cache lookup NAKs.
