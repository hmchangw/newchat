...
func TestBootstrapStreams(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "disabled - fails when MESSAGES stream missing",
			enabled:    false,
			existing:   map[string]bool{"MESSAGES-CANONICAL-test": true},
			wantErrSub: "verify MESSAGES stream",
		},
		{
			name:       "disabled - fails when MESSAGES-CANONICAL stream missing",
			enabled:    false,
			existing:   map[string]bool{"MESSAGES-test": true},
			wantErrSub: "verify MESSAGES-CANONICAL stream",
		},
		{
			name:        "enabled - creates MESSAGES and MESSAGES-CANONICAL",
			enabled:     true,
			existing:    map[string]bool{},
			wantCreated: []string{"MESSAGES-test", "MESSAGES-CANONICAL-test"},
		},
		{
			name:       "enabled - wraps MESSAGES creator error",
			enabled:    true,
			existing:   map[string]bool{},
			failOn:     "MESSAGES-test",
