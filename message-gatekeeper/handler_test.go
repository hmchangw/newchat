...
// A transient infra failure Naks for redelivery, and the Nak must carry a
// backoff delay: nats-server queues a delay-less NAK for immediate
// redelivery, which burns MaxDeliver in milliseconds and drops the accepted
// message instead of riding out the dependency outage.
func TestHandleJetStreamMsg_InfraError_NaksWithDelay(t *testing.T) {
	ctrl := gomock.NewController(t)
	store := NewMockStore(ctrl)
	store.EXPECT().
		GetSubscription(gomock.Any(), "alice", "room-1").
	...
	assert.False(t, msg.acked, "infra failure must not Ack — that would drop the message")
	// Metadata() is nil on the fake, so jsretry falls back to the head of the schedule.
	assert.Equal(t, jsretry.LowLatencyBackoff[0], msg.nakDelay, "the nak must carry the jsretry backoff delay")
}
